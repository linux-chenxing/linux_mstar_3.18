...
#if defined(CONFIG_MP_DEBUG_TOOL_PMU_TIMER_MODE)

/* pmu_enable_flag for local_timer check */
u32 armpmu_enable_flag[CONFIG_NR_CPUS] = {0};
...
#if defined(CONFIG_MP_DEBUG_TOOL_PMU_TIMER_MODE)
        /* pmu_enable_flag set for local_timer check pmu running */
        armpmu_enable_flag[smp_processor_id()] |= (u32)(0x1 << idx);
...
#if defined(CONFIG_MP_DEBUG_TOOL_PMU_TIMER_MODE)
        /* pmu_enable_flag clear for local_timer check pmu running */
        armpmu_enable_flag[smp_processor_id()] &= (u32) ~(0x1 << idx);
...
irqreturn_t armpmu_handle_irq(int irq_num, void *dev)
{
    return armv8pmu_handle_irq(irq_num,dev);
}
...
static void armv8pmu_reset(void *info)
{
    ...
    /* MStar Kernel Patch Begin 
       This Patch is for mantis 0942239 which WKSZ apk
       encountered undefined instruction error when attempting
       to read PMCCNTR_EL0
    */
