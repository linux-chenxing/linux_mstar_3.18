...
int __init mstar_device_pool_init(struct device *cma_dev);
#endif
...
struct dma_pool pci_atomic_pool = {
	.size = 0,
};
#endif
...
static int __init atomic_pool_init(void)
{
    ...
    if(pci_cma_device)
    {
        pci_atomic_pool.size = ( (pci_cma_device->cma_area->count) << PAGE_SHIFT );
        dev_set_drvdata(pci_cma_device, &pci_atomic_pool);
        mstar_device_pool_init(pci_cma_device);
    }
#endif
...
int __init mstar_device_pool_init(struct device *cma_dev)
{
	struct dma_pool *pool = (struct dma_pool *)dev_get_drvdata(cma_dev);
	pgprot_t prot=__pgprot_modify(pgprot_kernel,L_PTE_MT_MASK,L_PTE_MT_WRITEBACK);
	gfp_t gfp = GFP_KERNEL | GFP_DMA;
	unsigned long nr_pages = pool->size >> PAGE_SHIFT;
	unsigned long *bitmap;
	struct page *page;
	...
	if (cma_dev)
		ptr = __alloc_from_contiguous(cma_dev, pool->size, prot, &page,NULL);
    else
        goto no_cma;
 ...
 * CMA is activated by core_initcall, so we must be called after it.
 */
void *mstar_device_alloc_from_pool(struct device *cma_dev,size_t size, struct page **ret_page)
{
	struct dma_pool *pool = (struct dma_pool *)dev_get_drvdata(cma_dev);
	unsigned int count = PAGE_ALIGN(size) >> PAGE_SHIFT;
	unsigned int pageno;
...
bool in_mstar_device_pool(struct device *cma_dev,void *start, size_t size)
{
	struct dma_pool *pool = (struct dma_pool *)dev_get_drvdata(cma_dev);
	void *end = start + size;
	void *pool_start;
...
int mstar_device_free_from_pool(struct device *cma_dev,void *start, size_t size)
{
	struct dma_pool *pool = (struct dma_pool *)dev_get_drvdata(cma_dev);
	unsigned long pageno, count;
	unsigned long flags;

	if (!in_mstar_device_pool(cma_dev,start, size))
		return 0;
...
EXPORT_SYMBOL(mstar_device_pool_init);
EXPORT_SYMBOL(mstar_device_alloc_from_pool);
EXPORT_SYMBOL(in_mstar_device_pool);
EXPORT_SYMBOL(mstar_device_free_from_pool);
#endif
