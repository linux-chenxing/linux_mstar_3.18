...
int Si2151_demoLoop (L1_Si2151_Context *Si2151, char* choice) {
    ...
    else if (stricmp(entry, "ATVTune"       )==0)
    {
        ...
        if ((errcode=Si2151_ATVTune (Si2151, freq,  Si2151->prop->atv_video_mode.video_sys, Si2151->prop->atv_video_mode.invert_spectrum)) != NO_Si2151_ERROR)
  ...
  else if (stricmp(entry, "DTVTune"       )==0)
  {
    ...
    if ((errcode=Si2151_DTVTune (Si2151, freq, Si2151->prop->dtv_mode.bw, Si2151->prop->dtv_mode.modulation, Si2151->prop->dtv_mode.invert_spectrum)) != NO_Si2151_ERROR)
...
int  main(int argc, char *argv[]) {
    ...
    for (fe=0; fe<FRONT_END_COUNT; fe++) {
      ...
      Si2151_L1_API_Init(front_end, Si2151_BASE_ADDRESS+(2*fe));
      if (fe==0)
      {
