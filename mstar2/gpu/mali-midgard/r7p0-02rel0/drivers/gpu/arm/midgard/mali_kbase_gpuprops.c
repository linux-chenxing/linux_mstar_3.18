...
/* MSTAR patch begin */
#if MSTAR_DISABLE_GPU_CORES
extern int mstar_platform_get_efuse_disable_gpu_cores(void);
#endif
/* MSTAR patch end */
...
/* MSTAR patch begin */
#if MSTAR_DISABLE_GPU_CORES
	shader_present = gpu_props->raw_props.shader_present;
	efuse_disable_gpu_cores = mstar_platform_get_efuse_disable_gpu_cores();
	num_cores = hweight64(gpu_props->raw_props.shader_present);

	//printk(KERN_DEBUG"shader_present = 0x%x, num_cores = %d, efuse_disable_gpu_cores=%d, \n", shader_present, num_cores, efuse_disable_gpu_cores);
	KBASE_DEBUG_ASSERT(efuse_disable_gpu_cores < num_cores);

	if (efuse_disable_gpu_cores != 0)
	{
		for (i = 1; i <= efuse_disable_gpu_cores; i++)
			shader_present &= ~(0x1 << (num_cores - i));
		gpu_props->raw_props.shader_present = shader_present;
	}
#endif
/* MSTAR patch end */
