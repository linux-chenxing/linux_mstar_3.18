...
static int os_allocate(void *ctx, ump_dd_mem *descriptor)
{
...
#ifdef MSTAR
		bus_to_phys_address_adjust(&descriptor->block_array[pages_allocated].addr);
#endif
	...
	if (left) {
		...
		while (pages_allocated) {
			pages_allocated--;
#ifdef MSTAR
			phys_to_bus_address_adjust(&descriptor->block_array[pages_allocated].addr);
#endif
			if (!is_cached) {
...
#ifdef MSTAR
		phys_to_bus_address_adjust(&descriptor->block_array[i].addr);
#endif
