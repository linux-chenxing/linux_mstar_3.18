...
static int _mdrv_alsa_pcm_new(struct MStar_Device_Struct *madchip, int device_no, int playback_substreams, int capture_substreams)
{
	...
	if (card_no == 0)
		sprintf(pcm->name, "%s CARD0 No.%02d", _MAD_PCM_NAME, device_no); /* 80 bytes */
	if (card_no == 1)
		sprintf(pcm->name, "%s CARD1 No.%02d", _MAD_PCM_NAME, device_no); /* 80 bytes */
