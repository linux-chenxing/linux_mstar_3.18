...
ST_DRV_LD_DMA_INFO  stDrvLdMDAInfo;
...
static MS_U16 MDrv_LD_PreInitVariable(void)
{
    ...
    memset((void *)&stDrvLdMspiInfo,0,sizeof(MSPI_config));
    memset((void *)&stDrvLdMDAInfo,0,sizeof(ST_DRV_LD_DMA_INFO));
...
MS_U16 MDrv_LD_InitLDB(void)
{
...
#if LD_HW_PINGPONG_MODE

    LDB_mem_size = LD_MAX_BLOCK;
    if(stDrvLdMiscInfo.u8ClkHz == 120)
    {
    ...
    MHal_LD_SetLDBFrameBufBaseAddr_1((u32LDBBaseAddr + LDB_mem_size)/ psDrvLdInfo->u16PackLength);
    if(stDrvLdMiscInfo.u8ClkHz == 120)
    {
    MHal_LD_SetLDBFrameBufBaseAddr_R0((u32LDBBaseAddr + 2*LDB_mem_size) / psDrvLdInfo->u16PackLength);
    MHal_LD_SetLDBFrameBufBaseAddr_R1((u32LDBBaseAddr + 3*LDB_mem_size) / psDrvLdInfo->u16PackLength);
    }
#else
    if(stDrvLdMiscInfo.u8ClkHz == 120)
    {
...
MS_U16 MDrv_LD_InitMSPI(void)
{
    LD_INFO("Start MSPI Init\n");

    MDrv_MSPI_Init(stDrvLdMspiInfo.eChannel);
    MDrv_MSPI_DCConfig(stDrvLdMspiInfo.eChannel ,&stDrvLdMspiInfo.tMSPI_DCConfig);
    MDrv_MSPI_SetMode(stDrvLdMspiInfo.eChannel , stDrvLdMspiInfo.eMSPIMode);

    MDrv_MSPI_SetCLKByINI(stDrvLdMspiInfo.eChannel ,stDrvLdMspiInfo.tMSPI_ClockConfig.u32Clock);

    MDrv_MSPI_FRAMEConfig(stDrvLdMspiInfo.eChannel, &stDrvLdMspiInfo.tMSPI_FrameConfig);

    //set how many spi write buffer should use
    MDrv_MSPI_SetWriteBufferSize(stDrvLdMspiInfo.eChannel,stDrvLdMspiInfo.u8MspiBuffSizes);

    LD_INFO("MSPI init done\n");
    return E_LD_STATUS_SUCCESS;
}
MS_U16 MDrv_LD_InitLDMA(void)
{
   LDMA_TrigDelay_st stLDMDelay;
   MS_U32 u32dataoffset = MHal_LD_GetLDFFrameBufBaseAddr_0() + LD_BIN_LENGTH - MHal_LD_GetLDBFrameBufBaseAddr_0();

   stLDMDelay.u16Delay0Cnt0 = stDrvLdMDAInfo.u16DMADelay[0];
   stLDMDelay.u16Delay0Cnt1 = stDrvLdMDAInfo.u16DMADelay[1];
   stLDMDelay.u16Delay1Cnt0 = stDrvLdMDAInfo.u16DMADelay[2];
   stLDMDelay.u16Delay1Cnt1 = stDrvLdMDAInfo.u16DMADelay[3];

   LD_INFO("Start LDMA Init\n");
   MDrv_LDM_DMA_Init(stDrvLdMDAInfo.eLDMAchanel);
   MDrv_LDM_DMA_SetSpiTriggerMode( stDrvLdMDAInfo.eLDMAchanel,stDrvLdMDAInfo.eLDMATrimode);

   MDrv_LDM_DMA_SetSPICommandFormat(stDrvLdMDAInfo.eLDMAchanel,stDrvLdMDAInfo.u8cmdlength,stDrvLdMDAInfo.u16MspiHead);

   MDrv_LDMA_SetCheckSumMode( stDrvLdMDAInfo.eLDMAchanel,stDrvLdMDAInfo.eLDMACheckSumMode);

   MDrv_LDM_DMA_SetMenuloadNumber(stDrvLdMDAInfo.eLDMAchanel,stDrvLdMDAInfo.u16LedNum);
   MDrv_LDMA_SetTrigDelay(stDrvLdMDAInfo.eLDMAchanel,&stLDMDelay);
   MDrv_LDMA_EnableCS(stDrvLdMDAInfo.eLDMAchanel,TRUE);

   MHal_LD_SetMIUPackOffset(stDrvLdMDAInfo.eLDMAchanel,0 );
   MHal_LD_SetMIUPackLength(stDrvLdMDAInfo.eLDMAchanel, 0x001F);
   MHal_LD_SetYoffEnd(stDrvLdMDAInfo.eLDMAchanel,(stDrvLdMDAInfo.u8BLHeight-1));

   //0:depack spi pack using 8bits mode;1:depack spi pack using 16bits mode
   if(stDrvLdMDAInfo.u8DataPackMode == 1)
   {
     stDrvLdMDAInfo.u16DMAPackLength = 32;
   }
   else
   {
     stDrvLdMDAInfo.u16DMAPackLength = 16;
   }
   MHal_LD_SetBlDepack16BitMode(stDrvLdMDAInfo.u8DataPackMode);

   MHal_LD_SetSpiDataInv(stDrvLdMDAInfo.u8DataInvert);   //spi data invert mode.0 : [15:0], 1: {[7:0], [15:8]}, 2: {[0:15]}

   MHal_LD_SetDmaEnable(stDrvLdMDAInfo.eLDMAchanel, TRUE);

   MHal_LD_SetBlHeightDMA((stDrvLdMDAInfo.u8BLHeight-1));
   if(stDrvLdMiscInfo.u8SPIBits== E_LD_SPI_DATA_12_BIT)
   {
       stDrvLdMDAInfo.u8BLWidth = (stDrvLdMDAInfo.u8BLWidth*3)/2 ;
   }
   MHal_LD_SetBlWidthDMA((stDrvLdMDAInfo.u8BLWidth-1));

   MHal_LD_SetLEDBufBaseOffset(u32dataoffset/MHAL_LD_PACKLENGTH);

   LD_INFO(" LDMA init done\n");
   return E_LD_STATUS_SUCCESS;
}


MS_U16 MDrv_LD_InitPWM(void)
{
     MDrv_PWM_Init(E_PWM_DBGLV_ALL);

     MDrv_PWM_Oen(LOCAL_DIMMING_PWM_CH, 0);
     MDrv_PWM_Period(LOCAL_DIMMING_PWM_CH, LOCAL_DIMMING_PWM_VSYNC_Period);
     MDrv_PWM_DutyCycle(LOCAL_DIMMING_PWM_CH, LOCAL_DIMMING_PWM_VSYNC_DutyCycle);

     MDrv_PWM_Div(LOCAL_DIMMING_PWM_CH, LOCAL_DIMMING_PWM_VSYNC_Div);
     MDrv_PWM_Vdben(LOCAL_DIMMING_PWM_CH,TRUE);
     MDrv_PWM_Dben(LOCAL_DIMMING_PWM_CH, FALSE);
     MDrv_PWM_ForceShitfEn(LOCAL_DIMMING_PWM_CH,TRUE);
