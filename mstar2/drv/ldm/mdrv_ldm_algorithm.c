...
//=============================================================================
static MS_S8 MDrv_LD_SendMsg2RT51(MS_BOOL bIsCommand, MS_PHY argv0, MS_U16 argv1);
static MS_U16 MDrv_LD_ConvertLumaData2SPIData(MS_U16* pu16Addr, MS_U16 u16LEDNum);
...
static void MDrv_LD_AlgoPostProcess(const MS_U16* IN_blocks, MS_U16* SPI_blocks, MS_U8* LDB_blocks)
{
    ...
    for (block_idx = 0; block_idx < n_leds; block_idx++)
    {
     ...
     if(stDrvLdMiscInfo.u8MirrorPanel == 1)
     {
         MS_U8 u8LDBtemp = 0;
...
static void MDrv_LD_AlgoGlobalDimmingStrength(MS_U16* BL_blocks)
{
    ...
    MDrv_LD_FlushDRAMData((unsigned long)pSPI_blocks, ld_addr_base+SPI_BUFF_OFFSET, n_leds);

   if(NULL != mbx_fp && gu8MbxTime<20)
   {
    ReEntrance = 1;
    ret = MDrv_LD_SendMsg2RT51(FALSE, (MS_PHY)(ld_addr_base+SPI_BUFF_OFFSET),n_leds);
    ReEntrance = 0;
     gu8MbxTime++;
    if(0 != ret)
    {
...
void MDrv_LD_MarqueeDemo(void)
{
    ...
    for(;u16Counter<u16Leds;u16Counter++)
    {
        ...
        #if 0   //don't have ld panel to test real bl, using digital bl
        //u16SPIBlocks[u16Counter] = 0xffff;
        memcpy(pSPI_blocks,u16SPIBlocks,u16Leds);
        Chip_Flush_Memory();
        LD_INFO(" E_DEMO_PATTERN_MARQUEE u16Led:%d, u16Counter:%d, BL DATA:%d\n", u16Leds,u16Counter,pSPI_blocks[u16Counter]);
        ret = MDrv_LD_SendMsg2RT51((MS_PHY)(ld_addr_base+SPI_BUFF_OFFSET), u16Leds*sizeof(MS_U16));
        if(0 != ret)
        {
            LD_ERROR(" send addr_base to rt51 fail ret:%d\n",ret);
        }
...
void MDrv_LD_Main(void)
{
    ...
    if((NULL == pLDF_blocks) || (NULL == pSD_blocks) || (NULL == pTF_blocks) ||
        ...
        (NULL == pLuma_blocks) ||(FALSE == bLdOn)||(NULL == pGamma_blocks))
    {
...
static MS_U16 MDrv_LD_ConvertLumaData2SPIData(MS_U16* pu16Addr, MS_U16 u16LEDNum)
{
    ...
    switch(stDrvLdMiscInfo.u8SPIBits)
    {
        ...
        case E_LD_SPI_DATA_12_BIT:
         ...
         case E_LD_SPI_DATA_12_BIT_HL:
            ...
            {
                u8LEDData[i] = (pu8Addr[i+1]>> 4);                                  //high 8 bit
                u8LEDData[i+1] = ((pu8Addr[i+1]&0x0f)<<4) |((pu8Addr[i]&0xf0)>>4);  //low 8 bit
                i = i + 2;
            }
            u16DataByteNum = i; //the data length
            break;
        case E_LD_SPI_DATA_12_BIT_LH:
            for(i=0;i<u16DataByteNum;)
            {
                u8LEDData[i] = ((pu8Addr[i+1]&0x0f)<<4) |((pu8Addr[i]&0xf0)>>4); //low 8 bit
                u8LEDData[i+1] = (pu8Addr[i+1]>> 4);                          //high 8 bit
                i = i + 2;
            }
            u16DataByteNum = i; //the data length
            break;
        case E_LD_SPI_DATA_16_BIT_HL:
            //16-bit --> 16-bit:  AB CD --> CD AB
            for(i=0,j=0;j<u16DataByteNum;j++)
            {
                u8LEDData[i++] = pu8Addr[j+1];   //high 8 bit
                u8LEDData[i++] = pu8Addr[j++];   //low 8 bit
            }
            break;
        default:
    ...
    //only for mirror panel,the u8MirrorPanel is default to be 0
    if(stDrvLdMiscInfo.u8MirrorPanel == 1)
    {
        MS_U8 u8SPItemp = 0;
    ...
    memcpy((MS_U8 *)pSPI_blocks,u8LEDData,u16DataByteNum);
...
//is command, argv[0]=0, argv[1]=command, IF not, argv[0]=addr, argv[1]=led numbers
static MS_S8 MDrv_LD_SendMsg2RT51(MS_BOOL bIsCommand, MS_PHY argv0, MS_U16 argv1)
{
    MBX_Result enMbxResult = E_MBX_UNKNOW_ERROR;
    //return enMbxResult = 0;
    MBX_Msg stMbxCommand;
    ...
    if(TRUE == bIsCommand)
    {
        stMbxCommand.u8ParameterCount = 2;
        stMbxCommand.u8Parameters[0] =  (MS_U8)(argv1>>8);
        stMbxCommand.u8Parameters[1] =  (MS_U8)(argv1&0xFF);
    }
    else
    {
        bIs64bits = (8 == sizeof(MS_PHY))?TRUE:FALSE;
        if(TRUE == bIs64bits)
        {
            stMbxCommand.u8ParameterCount = 10;
            stMbxCommand.u8Parameters[0] =  (MS_U8)(((MS_U64)argv0)>>56);
            stMbxCommand.u8Parameters[1] =  (MS_U8)(((MS_U64)argv0)>>48);
            stMbxCommand.u8Parameters[2] =  (MS_U8)(((MS_U64)argv0)>>40);
            stMbxCommand.u8Parameters[3] =  (MS_U8)(((MS_U64)argv0)>>32);
            stMbxCommand.u8Parameters[4] =  (MS_U8)(((MS_U64)argv0)>>24);
            stMbxCommand.u8Parameters[5] =  (MS_U8)(((MS_U64)argv0)>>16);
            stMbxCommand.u8Parameters[6] =  (MS_U8)(((MS_U64)argv0)>>8);
            stMbxCommand.u8Parameters[7] =  (MS_U8)(((MS_U64)argv0)&0xFF);
            stMbxCommand.u8Parameters[8] =  (MS_U8)(argv1>>8); //led numbers
            stMbxCommand.u8Parameters[9] =  (MS_U8)((argv1)&0xFF);
        }
        else
        {
            stMbxCommand.u8ParameterCount = 6;
            stMbxCommand.u8Parameters[0] =  (MS_U8)(argv0>>24);
            stMbxCommand.u8Parameters[1] =  (MS_U8)(argv0>>16);
            stMbxCommand.u8Parameters[2] =  (MS_U8)(argv0>>8);
            stMbxCommand.u8Parameters[3] =  (MS_U8)(argv0&0xFF);
            stMbxCommand.u8Parameters[4] =  (MS_U8)(argv1>>8); //led numbers
            stMbxCommand.u8Parameters[5] =  (MS_U8)(argv1&0xFF); //led numbers
        }
    }
...
void MDrv_LD_Suspend(void)
{
    ...
    MDrv_MBX_UnRegisterMSG((MS_U64)mbx_fp, E_MBX_CLASS_PM_NOWAIT, PM_MBX_QUEUESIZE);
    if(NULL != mbx_fp)
    {
...
void MDrv_LD_Enable(MS_BOOL bOn, MS_U8 u8Luma)
{
    ...
    // ld status:if false go into MDrv_LD_SendConstantLuma and ld sw not work, if true go into MDrv_LD_Main
    bLdOn = bOn;
    ...
    if(FALSE == bOn)
    {
        ...
        if((0<=u8Luma)&&(256>u8Luma))
        {
            gu16LedLuma = (u8Luma << 8) | u8Luma; //gu16LedLuma using constant luma
        }
        else
        {
            gu16LedLuma = 0xffff;                  //gu16LedLuma using constant luma
        }
    }
    else
    {
...
MS_S8 MDrv_LD_PrepareBuffs(void)
{
    ...
    pSPI_blocks = (MS_U16*)(u64Vaddr+SPI_BUFF_OFFSET); //mmap baseaddress + 0x80000
    pLDF_blocks = (MS_U8*)((MS_U8*)pSPI_blocks+LD_MAX_BLOCK); //SPI + 0X2800
    ...
    memset(((void *)pSPI_blocks),0xFF,LDM_LEN/2);    //follow spi to the end of buffer
    MDrv_LD_FlushDRAMData((unsigned long)pSPI_blocks, (unsigned long)(ld_addr_base+SPI_BUFF_OFFSET), LDM_LEN/2);
    ...
    switch(u8Pattern)
    {
        case E_LD_DEMO_PATTERN_SWITCH_SINGLE_LED:
        {
            ...
            if(TRUE == bOn)
            {
                ...
                bLdOn = FALSE;            //set ld status is false so go into MDrv_LD_SendConstantLuma
                gu16LedLuma = u16LEDNum; //gu16LedLuma using for led number
                gbMarqueue = TRUE;
            }
            else
            {
...
void MDrv_LD_DumpDRAM(EN_LD_GET_DATA_TYPE type)
{
    ...
    u8Buff = kmalloc(LD_MAX_BLOCK_SIZE*2, GFP_KERNEL);
    if (!u8Buff)
    {
        LD_ERROR("malloc memory fail \n");
    }

    memset(u8Buff,0,LD_MAX_BLOCK_SIZE*2);
    ...
    switch(type)
    {
        ...
        case E_LD_DATA_TYPE_SPI:
        {
            ...
            spin_lock_irq(&spinlock_ld);
            memcpy(u8Buff,(MS_U8 *)pSPI_blocks,n_leds);
            spin_unlock_irq(&spinlock_ld);
...
void MDrv_LD_CleanupModule(void)
{
    ...
    //send to 51 command, to satisfy power off timing
    do
    {
        ret = MDrv_LD_SendMsg2RT51(TRUE, 0, E_LD_PM_CMD_POWER_DOWN_SPI_PWM);
    }while((0 != ret) && (++u8Counter < 10));
