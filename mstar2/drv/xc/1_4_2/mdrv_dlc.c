...
///@return FALSE :fail
//---------------------------------------------------------------------------------------------
MS_BOOL MDrv_DLC_SetBlePointChangeInfo(ST_KDRV_XC_DLC_BLE_INFO *pstBleInfo)
{
#ifdef SUPPORT_XC_DLC
    MS_U8 i;

    _stBleInfo.bBLEPointChange =  pstBleInfo->bBLEPointChange;

    for (i=0; i<6; i++)
    {
        _stBleInfo.u16BLESlopPoint[i] = pstBleInfo->u16BLESlopPoint[i];
    }
#endif
