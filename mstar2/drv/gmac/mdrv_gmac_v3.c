...
#elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
        skb_addr = (u32)__virt_to_phys(skb->data) - GMAC_MIU0_BUS_BASE;
#endif
...
//        Chip_Inv_Cache_Range_VA_PA((u32)(&(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index].addr)),(&(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index].addr)) - LocPtr->RAM_VA_PA_OFFSET, sizeof(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index].addr));
        ...
        if (pktlen > GMAC_MTU)
        {
            GMAC_DBG("Packet RX too large!!(pktlen = %d)", pktlen);
    ...
    while(LocPtr->rx_desc_free_number < RX_DESC_NUMBER)
    {
        ...
        {
#if defined(CONFIG_MIPS)
            LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr = (u32)virt_to_phys(LocPtr->rx_desc_sk_buff_list[LocPtr->rx_desc_free_index]->data) - GMAC_MIU0_BUS_BASE;
#elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
            LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr = (u32)__virt_to_phys(LocPtr->rx_desc_sk_buff_list[LocPtr->rx_desc_free_index]->data) - GMAC_MIU0_BUS_BASE;
#endif
        ...
        #elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
//            Chip_Flush_Cache_Range_VA_PA((u32)(&(LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr)),(u32)(&(LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr)) - LocPtr->RAM_VA_PA_OFFSET ,sizeof(LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr));
        #else
            //#ERROR
        #endif
#endif
        ...
        {
#if defined(CONFIG_MIPS)
            LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr = (u32)virt_to_phys(LocPtr->rx_desc_sk_buff_list[LocPtr->rx_desc_free_index]->data) + GMAC_DESC_WRAP - GMAC_MIU0_BUS_BASE;
#elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
            LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr = (u32)__virt_to_phys(LocPtr->rx_desc_sk_buff_list[LocPtr->rx_desc_free_index]->data) + GMAC_DESC_WRAP - GMAC_MIU0_BUS_BASE;
#endif
...
static u32 MDev_GMAC_HW_Reg_init(struct net_device *dev)
{
    ...
    if (!LocPtr->ThisUVE.initedGMAC)
    {
        MHal_GMAC_write_phy(LocPtr->phyaddr, MII_BMCR, 0x9000UL);
        ...
        // ETH_CFG Register -----------------------------------------------------
        word_ETH_CFG = 0x00000c00UL;        // Init: CLK = 0x2
...
static int MDev_GMAC_init(void)
{
    ...
    gmac_dev->base_addr     = (long) GMAC_REG_ADDR_BASE;
    gmac_dev->irq           = MAC_IRQ;
