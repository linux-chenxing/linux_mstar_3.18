...
#define _MDrv_HWI2C0_Send_Byte HAL_HWI2C0_Send_Byte
#define _MDrv_HWI2C0_Recv_Byte HAL_HWI2C0_Recv_Byte

BOOL MDrv_HWI2C0_Send_Byte(U16 u16PortOffset, U8 u8Data)
{
	return HAL_HWI2C0_Send_Byte(u16PortOffset, u8Data);
}

BOOL MDrv_HWI2C0_Recv_Byte(U16 u16PortOffset, U8 *pData)
{
	return HAL_HWI2C0_Recv_Byte(u16PortOffset, pData);
}


BOOL MDrv_HWI2C0_NoAck(U16 u16PortOffset)
{
    return HAL_HWI2C0_NoAck(u16PortOffset);
}


BOOL MDrv_HWI2C0_Reset(U16 u16PortOffset, BOOL bReset)
{
    return HAL_HWI2C0_Reset(u16PortOffset, bReset);
}


BOOL _MDrv_HWI2C0_GetPortRegOffset(U8 u8Port, U16 *pu16Offset)
{
    HWI2C_DBG_FUNC();
    ...
    return HAL_HWI2C0_SetPortRegOffset((HAL_HWI2C_PORT)g_HWI2CPort[u8Port],pu16Offset);
}

BOOL _MDrv_HWI2C0_ReadBytes(U8 u8Port, U16 u16SlaveCfg, U32 uAddrCnt, U8 *pRegAddr, U32 uSize, U8 *pData)
{
    ...
    while (u16Dummy--)
    {
        ...
        {	
            HAL_HWI2C0_Start(u16Offset);
            //add extral delay by user configuration
            MsOS_DelayTaskUs(g_u32StartDelay);

            if (!_MDrv_HWI2C0_Send_Byte(u16Offset,HWI2C_SET_RW_BIT(FALSE, u8SlaveID)))
                goto end;

            while(uAddrCnt--)
            {
                if (!_MDrv_HWI2C0_Send_Byte(u16Offset,*pRegAddr))
                    goto end;
                pRegAddr++;
            }
            ...
            {
                HAL_HWI2C0_Stop(u16Offset);
                //add extral delay by user configuration
                MsOS_DelayTaskUs(g_u32StopDelay);
            }
			else
			{
				HAL_HWI2C0_Reset(u16Offset,TRUE);
				HAL_HWI2C0_Reset(u16Offset,FALSE);
			}
        }

        //Very important to add delay to support all clock speeds
        //Strongly recommend that do not remove this delay routine
        HAL_HWI2C0_ExtraDelay(g_u16DelayFactor[u8Port]);
        HAL_HWI2C0_Start(u16Offset);

        //add extral delay by user configuration
        MsOS_DelayTaskUs(g_u32StartDelay);

        if (!_MDrv_HWI2C0_Send_Byte(u16Offset,HWI2C_SET_RW_BIT(TRUE, u8SlaveID)))
            goto end;
        ...
        while(uSize)
        {
            ...
            uSize--;
            if (uSize==0)
                HAL_HWI2C0_NoAck(u16Offset);
            if (_MDrv_HWI2C0_Recv_Byte(u16Offset,pData)==FALSE)
                goto end;
            pData++;
        }
        bComplete = TRUE;
...
BOOL _MDrv_HWI2C0_WriteBytes(U8 u8Port, U16 u16SlaveCfg, U32 uAddrCnt, U8 *pRegAddr, U32 uSize, U8 *pData)
{
    U8 u8SlaveID = LOW_BYTE(u16SlaveCfg);
    U16 u16Offset = 0x00;
    ...
    while(u16Dummy--)
    {
        ...
        while(uSize)
        {
            if (!_MDrv_HWI2C0_Send_Byte(u16Offset, *pData))
...
static BOOL _MDrv_HWI2C0_SelectPort(HWI2C_PORT ePort)
{
    U16 u16Offset = 0x00;
    U8 u8Port = 0x00;
    BOOL bRet=TRUE;
    ...
    if(HAL_HWI2C0_GetPortIdxByPort((HAL_HWI2C_PORT)ePort,&u8Port))
    {
    ...
    HAL_HWI2C0_SelectPort((HAL_HWI2C_PORT)ePort);

    //(3) configure port register offset ==> important
    bRet &= _MDrv_HWI2C0_GetPortRegOffset(u8Port,&u16Offset);

    //(4) master init
    bRet &= HAL_HWI2C0_Master_Enable(u16Offset);

    return bRet;
}

static BOOL _MDrv_HWI2C0_SetClk(U8 u8Port, HWI2C_CLKSEL eClk)
{
    U16 u16Offset = 0x00;
    ...
    return HAL_HWI2C0_SetClk(u16Offset,(HAL_HWI2C_CLKSEL)eClk);
}

static BOOL _MDrv_HWI2C0_SetReadMode(U8 u8Port, HWI2C_ReadMode eReadMode)
{
    HWI2C_DBG_FUNC();
...
static BOOL _MDrv_HWI2C0_InitPort(HWI2C_UnitCfg *psCfg)
{
    ...
    for(u8PortIdx=0; u8PortIdx < HWI2C_PORTS; u8PortIdx++)
    {
        ...
        {
            if(_MDrv_HWI2C0_SelectPort(stPortCfg.ePort))
            {
                //set clock speed
                bRet &= _MDrv_HWI2C0_SetClk(g_HWI2CPortIdx, stPortCfg.eSpeed);
                //set read mode
                bRet &= _MDrv_HWI2C0_SetReadMode(g_HWI2CPortIdx, stPortCfg.eReadMode);
                //get port index
                bRet &= HAL_HWI2C0_GetPortIdxByPort((HAL_HWI2C_PORT)stPortCfg.ePort,&u8Port);
                //configure port register offset ==> important
                bRet &= _MDrv_HWI2C0_GetPortRegOffset(u8Port,&u16Offset);
                //master init
                bRet &= HAL_HWI2C0_Master_Enable(u16Offset);
    ...
    if(_MDrv_HWI2C0_SelectPort(psCfg->ePort))
    {
        //set clock speed
        bRet &=_MDrv_HWI2C0_SetClk(g_HWI2CPortIdx,psCfg->eSpeed);
        //set read mode
        bRet &=_MDrv_HWI2C0_SetReadMode(g_HWI2CPortIdx,psCfg->eReadMode);
        //get port index
        bRet &= HAL_HWI2C0_GetPortIdxByPort((HAL_HWI2C_PORT)psCfg->ePort,&u8Port);
        //configure port register offset ==> important
        bRet &= _MDrv_HWI2C0_GetPortRegOffset(u8Port,&u16Offset);
        //master init
        bRet &= HAL_HWI2C0_Master_Enable(u16Offset);
...
BOOL MDrv_HWI2C0_Init(HWI2C_UnitCfg *psCfg)
{
    ...
    HAL_HWI2C0_SetIOMapBase(u32BaseAddr);
    ...
    bRet &= HAL_HWI2C0_Init_Chip();
    //(3) Initialize all port
    bRet &= _MDrv_HWI2C0_InitPort(psCfg);
    ...
    HWI2C_DBG_INFO("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&MDrv_HWI2C0_Init&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n");
    return bRet;
}

void MDrv_HW_IIC0_Init(void)
{
    ...
    for(j = 0 ; j < 1 ; j++)
    {
        ...
        pHwbuscfg[0].sCfgPort[j].eSpeed= E_HAL_HWI2C_CLKSEL_NORMAL; //E_HAL_HWI2C0_CLKSEL_VSLOW;//pIIC_Param->u8ClockIIC;//
	...
	pHwbuscfg[0].eSpeed = E_HAL_HWI2C_CLKSEL_NORMAL; //E_HAL_HWI2C0_CLKSEL_VSLOW;//pIIC_Param->u8ClockIIC;//
    ...
    MDrv_HWI2C0_Init(&pHwbuscfg[0]);
}

EXPORT_SYMBOL(MDrv_HW_IIC0_Init);
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_Start
/// @brief \b Function  \b Description: send start bit
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_Start(U16 u16PortOffset)
{
    HWI2C_DBG_FUNC();
    return HAL_HWI2C0_Start(u16PortOffset);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_Stop
/// @brief \b Function  \b Description: send stop bit
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_Stop(U16 u16PortOffset)
{
    HWI2C_DBG_FUNC();
    return HAL_HWI2C0_Stop(u16PortOffset);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_GetPortIndex
...
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_GetPortIndex(HWI2C_PORT ePort, U8* pu8Port)
{
    BOOL bRet=TRUE;
    ...
    //(1) Get port index by port number 
    bRet &= HAL_HWI2C0_GetPortIdxByPort((HAL_HWI2C_PORT)ePort, pu8Port);
    HWI2C_DBG_INFO("ePort:0x%02X, u8Port:0x%02X\n",(U8)ePort,(U8)*pu8Port);
...
//###################
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_SelectPort
/// @brief \b Function  \b Description: Decide port index and pad mux for port number
/// @param ePort        \b IN: port number
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_SelectPort(HWI2C_PORT ePort)
{
    HWI2C_DBG_FUNC();
    if(ePort >= E_HWI2C_PORT_NOSUP)
        return FALSE;
    return _MDrv_HWI2C0_SelectPort(ePort);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_SetClk
/// @brief \b Function  \b Description: Set HW I2C clock
/// @param eClk         \b IN: clock rate
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_SetClk(HWI2C_CLKSEL eClk)
{
    HWI2C_DBG_FUNC();
    return _MDrv_HWI2C0_SetClk(g_HWI2CPortIdx, eClk);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_SetReadMode
/// @brief \b Function  \b Description: Set HW I2C Read Mode
/// @param eClk         \b IN: ReadMode
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_SetReadMode(HWI2C_ReadMode eReadMode)
{
    return _MDrv_HWI2C0_SetReadMode(g_HWI2CPortIdx, eReadMode);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_ReadByte
...
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_ReadByte(U16 u16SlaveCfg, U8 u8RegAddr, U8 *pData)
{
    HWI2C_DBG_FUNC();
    return MDrv_HWI2C0_ReadBytes(u16SlaveCfg, 1, &u8RegAddr, 1, pData);
}
...
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_WriteByte(U16 u16SlaveCfg, U8 u8RegAddr, U8 u8Data)
{
    HWI2C_DBG_FUNC();
    return MDrv_HWI2C0_WriteBytes(u16SlaveCfg, 1, &u8RegAddr, 1, &u8Data);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_WriteBytes
/// @brief \b Function  \b Description: Init HWI2C driver
/// @param u16SlaveCfg  \b IN: [15:8]: Channel number [7:0]:Slave ID
...
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_WriteBytes(U16 u16SlaveCfg, U32 uAddrCnt, U8 *pRegAddr, U32 uSize, U8 *pData)
{
    BOOL bRet;
    U8 u8Port;
    ...
    bRet = _MDrv_HWI2C0_WriteBytes(u8Port,u16SlaveCfg,uAddrCnt,pRegAddr,uSize,pData);
    HWI2C_MUTEX_UNLOCK(u8Port);
    return bRet;
}
EXPORT_SYMBOL(MDrv_HWI2C0_WriteBytes);

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function  \b Name: MDrv_HWI2C0_ReadBytes
/// @brief \b Function  \b Description: Init HWI2C driver
/// @param u16SlaveCfg  \b IN: [15:8]: Channel number [7:0]:Slave ID
/// @param uAddrCnt     \b IN: register address count
/// @param pRegAddr     \b IN: pointer to targert register address
...
/// @return             \b TRUE: Success FALSE: Fail
////////////////////////////////////////////////////////////////////////////////
BOOL MDrv_HWI2C0_ReadBytes(U16 u16SlaveCfg, U32 uAddrCnt, U8 *pRegAddr, U32 uSize, U8 *pData)
{
    BOOL bRet;
    U8 u8Port;
    ...
    HWI2C_MUTEX_LOCK(u8Port);
    bRet = _MDrv_HWI2C0_ReadBytes(u8Port,u16SlaveCfg,uAddrCnt,pRegAddr,uSize,pData);
    HWI2C_MUTEX_UNLOCK(u8Port);
    return bRet;
}
EXPORT_SYMBOL(MDrv_HWI2C0_ReadBytes);
