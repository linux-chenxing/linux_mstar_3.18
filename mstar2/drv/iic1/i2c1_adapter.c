...
*/
static int
ms_i2c1_xfer_read(struct i2c_msg *pmsg, u8 *pbuf, int length)
{
    BOOL ret = FALSE;
    u32 u32i = 0;
    U8 u8Port = 0;
   ...
   /* ***** 1. Send start bit ***** */
   if(!MDrv_HWI2C1_Start(u16Offset))
   {
	   printk(KERN_INFO
   ...
   if (!MDrv_HWI2C1_Send_Byte(u16Offset, ((pmsg->addr & I2C_BYTE_MASK) << 1) |
   ...
   for (u32i = 0; u32i < length; u32i++)
   {
	  /* ***** 6. Read byte data from slave ***** */
	   if ((length-1) == u32i)
	   {
		   MDrv_HWI2C1_NoAck(u16Offset);
	   }
	   ret = MDrv_HWI2C1_Recv_Byte(u16Offset, pbuf);
	   pbuf++;
   }
...
*/
static int
ms_i2c1_xfer_write(struct i2c_msg *pmsg, u8 *pbuf, int length)
{
    u32 u32i = 0;
    U8 u8Port = 0;
    U16 u16Offset = 0x00;
    ...
    for (u32i = 0; u32i < length; u32i++)
    {
        ...
        if(MDrv_HWI2C1_Send_Byte(u16Offset, *pbuf))
        {
            pbuf++;
        }
        else
    ...
    MDrv_HWI2C1_Reset(u16Offset,TRUE);
    MDrv_HWI2C1_Reset(u16Offset,FALSE);
...
*/
static int
ms_i2c1_xfer(struct i2c_adapter *padap, struct i2c_msg *pmsg, int num)
{
    int i, err;
    U8 u8Port = 0;
    U16 u16Offset = 0x00;
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (pmsg->len && pmsg->buf) /* sanity check */
        {
            if (pmsg->flags & I2C_M_RD)
                {
                //return;
                err = ms_i2c1_xfer_read(pmsg, pmsg->buf, pmsg->len);
                }
            else
                err = ms_i2c1_xfer_write(pmsg, pmsg->buf, pmsg->len);
            if (err)
                return err;
...
+------------------------------------------------------------------------------
*/
static u32 ms_i2c1_func(struct i2c_adapter *padapter)
{
    return I2C_FUNC_I2C | I2C_FUNC_SMBUS_EMUL;
}

/* implement the i2c transfer function in algorithm structure */
static struct i2c_algorithm sg_ms_i2c1_algorithm =
{
    .master_xfer = ms_i2c1_xfer,
    .functionality = ms_i2c1_func,
};
...
+------------------------------------------------------------------------------
*/
static int  ms_i2c1_probe(struct platform_device *pdev)
{
...
+------------------------------------------------------------------------------
*/
static int ms_i2c1_remove(struct platform_device *pdev)
{
    struct i2c_adapter *padapter;
    int err;
...
*/
static
int ms_i2c1_suspend(struct platform_device *pdev, pm_message_t mesg)
{
    return 0;
}

static int ms_i2c1_resume(struct platform_device *pdev)
{
    /* initialize MIIC controller */
    MDrv_HW_IIC1_Init();
    return 0;
...
#else /* CONFIG_PM */
#define ms_i2c1_suspend      NULL
#define ms_i2c1_resume       NULL
#endif  /* CONFIG_PM */

/* work with "modprobe ms_i2c" from hotplugging or coldplugging */
MODULE_ALIAS("hw_i2c1");

/* i2c driver sturcut, used to register to kernel */
static struct platform_driver sg_ms_i2c1_driver =
{
    .probe      = ms_i2c1_probe,
    .remove     = ms_i2c1_remove,
    .suspend    = ms_i2c1_suspend,
    .resume   = ms_i2c1_resume,
    .driver     =
    {
        .name   = "hw_i2c1",
        .owner  = THIS_MODULE,
...
static struct platform_device sg_ms_i2c1_device =
{
    .name = "hw_i2c1",
    .id = 0,
    .dev =
...
+------------------------------------------------------------------------------
*/
static int __init ms_i2c1_init(void)
{
    int ret = 0;
    i2c_register_board_info(1, i2c_devs1, ARRAY_SIZE(i2c_devs1));

    printk(KERN_INFO "ms_i2c_init\n");
    ret = platform_driver_register(&sg_ms_i2c1_driver);
    if (!ret)
    {
        /* register driver sucess */
        ret = platform_device_register(&sg_ms_i2c1_device);
        /* if do not register a device, the probe() won't be called */

        if (ret)    /* if register device fail, then unregister the driver. */
            platform_driver_unregister(&sg_ms_i2c1_driver);
    }
    return ret;
}
...
+------------------------------------------------------------------------------
*/
static void __exit ms_i2c1_exit(void)
{
    platform_driver_unregister(&sg_ms_i2c1_driver);
}

subsys_initcall(ms_i2c1_init);
module_exit(ms_i2c1_exit);

MODULE_AUTHOR("Mstar");
MODULE_DESCRIPTION("MSTAR HW I2C1 driver");
MODULE_LICENSE("GPL");
