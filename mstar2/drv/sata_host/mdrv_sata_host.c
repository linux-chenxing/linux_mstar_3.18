...
static int sata_mstar_scr_read(struct ata_link *link, unsigned int sc_reg, u32 *val)
{
...
static int sata_mstar_scr_write(struct ata_link *link, unsigned int sc_reg_in, u32 val)
{
...
static unsigned int sata_mstar_dev_classify(struct ata_port *ap)
{
    ...
    temp = ahci_reg_read(PORT_SIG + SATA_GHC_1_ADDRESS_START);
...
hard_reset:
    ...
    //printk("MAX RETRY = %d\n", CONNECTION_RETRY);
    printk("MAX RETRY = Not allowed\n");

#if 0
#ifdef CONFIG_MSTAR_K6Lite // sw patch for gen3
    if (online) {
        ...
        if (rc == 0 && ((sstatus & 0xf) == 0x3)) { // 0x3 => device presence detected and Phy communication established
            if ((((sstatus >> 4) & 0xf) == 0x01) && retry++ < CONNECTION_RETRY) { // gen1
                ...
                u16Temp = readw(MSTAR_RIU_BASE + (0x10390e << 1));
                u16Temp &= ~0x200;
                writew(u16Temp, MSTAR_RIU_BASE + (0x10390e << 1));
...
static int mstar_ahci_kick_engine(struct ata_port *ap)
{
    ...
    rc = 0;
    tmp = ata_wait_register(ap, port_base + MS_PORT_CMD,
...
#error "SATA_CMD_TYPE Unknown"
    ...
    // test: enable PHY
    writew(0x969f, MSTAR_RIU_BASE + (0x103902 << 1)); // enable
    writew(0x969e, MSTAR_RIU_BASE + (0x103902 << 1)); // enable
    writew(0x0200, MSTAR_RIU_BASE + (0x10390e << 1)); // enable
...
irqreturn_t sata_mstar_interrupt(int irq, void *dev_instance)
{
    ...
    u32 serror = 0;
    u32 host_pi = mstar_sata_reg_read(HOST_PORTS_IMPL);
    ...
    host_status = mstar_sata_reg_read(MS_HOST_IRQ_STAT + hba_base);
    host_pi = mstar_sata_reg_read(MS_HOST_PORTS_IMPL + hba_base);

    if (host_status & host_pi) // we currently implement only one port
    ...
    port_status = mstar_sata_reg_read(MS_PORT_IRQ_STAT + port_base);

    mstar_sata_reg_write(port_status, MS_PORT_IRQ_STAT + port_base);
    if (port_status)
    ...
    if (port_status & (1 << 6)) { // PORT_IRQ_CONNECT
        ...
        mstar_sata_reg_write((1 << 26), MS_PORT_SCR_ERR + port_base);
    }
    if (port_status & (1 << 22)) { // PORT_IRQ_PHYRDY
        printk(KERN_WARNING "HDD removed (PhyRdy change, clear diag.N)\n");
        mstar_sata_reg_write((1 << 16), MS_PORT_SCR_ERR + port_base);
    ...
    if (unlikely(port_status & PORT_IRQ_ERROR)) {
        ...
        serror = mstar_sata_reg_read(MS_PORT_SCR_ERR + port_base);
        mstar_sata_reg_write(serror, MS_PORT_SCR_ERR + port_base);
    ...
    port_err = mstar_sata_reg_read(MS_PORT_SCR_ERR + port_base);
    if (port_err)
    ...
    if (clr_port_err)
        mstar_sata_reg_write(clr_port_err, MS_PORT_SCR_ERR + port_base);
...
irq_out:
    qc_active = mstar_sata_reg_read(MS_PORT_SCR_ACT + port_base);
    qc_active |= mstar_sata_reg_read(MS_PORT_CMD_ISSUE + port_base);

    ata_qc_complete_multiple(ap, qc_active);
    mstar_sata_reg_write(host_status, MS_HOST_IRQ_STAT + hba_base);
...
static int mstar_sata_hardware_init(void)
{
    u32 i;
    u32 u32Temp = 0;
    //phys_addr_t misc_base = hpriv->misc_base;
    phys_addr_t misc_base = SATA_MISC_ADDRESS_START;
...
// FIXME: need a way to set DMA mask in DTB, otherwise we will get dma allocation fail.
    ...
    hpriv->mmio = SATA_GHC_0_ADDRESS_START;
...
static int __init mstar_sata_drv_init(void)
{
    ...
    sata_info("Mstar sata host register platform driver\n");
    ...
    sata_info("Mstar sata host register platform device\n");
