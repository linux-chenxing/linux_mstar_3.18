...
//for sharememory
#define SHAREDMEMORY_DEFAULT_LEN       2*1024*1024
...
static int alloc_sharepage(SemutexModHandle *dev,int page_num)
{
    ...
    for(i = 0; i < page_num; i++)
    {
        ...
        if (page_node == NULL)
        {
            printk(KERN_ERR "allocated page_node fail!!! \n");
            goto allocated_fail;
        }
...
struct page * append_onePage(SemutexModHandle *dev)
{
    ...
    if (page_node == NULL)
    {
        printk("allocate page_node fail!!! \n");
        return NULL;
    }
    pg = alloc_page(GFP_HIGHUSER | __GFP_ZERO);
    if (pg == NULL)
        ...
        case MDRV_SEMUTEX_CREATE_SEMAPHORE:
            {
                ...
                {
                    //find by name
                    if(strncmp(semaarg.semaname, dev->sema_info[i].sema_name, SEMA_NAME_LEN) == 0)
                    {
                        ret_index = i;
                ...
                strncpy(dev->sema_info[first_unused].sema_name, semaarg.semaname, SEMA_NAME_LEN);
                dev->sema_info[first_unused].sema_name[SEMA_NAME_LEN-1] = '\0';
        ...
        case MDRV_SEMUTEX_CREATE_SHAREMEMORY:
            {
                ...
                // add tgid condition to prevent warning message caused by zygote repeat launch apk
                if(filp == pprocinfo->createsharememory_filp  && ( pprocinfo->tsk->tgid != current->real_parent-> tgid
 ))
                {
