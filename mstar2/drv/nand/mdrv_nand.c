...
extern int nand_block_checkbad(struct mtd_info *mtd, loff_t ofs, int getchip, int allowbbt);
...
static int _unfd_nand_read_oob_raw(struct mtd_info *mtd, struct nand_chip *chip,
			     int page)
{
    memcpy(chip->oob_poi, u8SpareBuf, mtd->oobsize);

    return 0;
}

static int _unfd_nand_read_page_raw(struct mtd_info *mtd, struct nand_chip *chip,
			      uint8_t *buf, int oob_required, int page)
{
    ...
    u16_Tmp = pNandDrv->u16_Reg50_EccCtrl;
...
#elif defined(CONFIG_ARM64)
    if(!virt_addr_valid((uintptr_t)buf) || !virt_addr_valid((uintptr_t)buf + (U32)mtd->writesize - 1) || ((uintptr_t)buf) % CACHE_LINE )
#endif
    {
        NC_ReadPages(u32CurRow, pu8_MainBuf, u8SpareBuf, 1);
        memcpy(buf, pu8_MainBuf, mtd->writesize);
    }
    else
    {
        NC_ReadPages(u32CurRow, buf, u8SpareBuf, 1);
    }
    ...
    pNandDrv->u16_Reg50_EccCtrl = u16_Tmp;
    NC_Config();
...
static int _unfd_nand_write_oob_raw(struct mtd_info *mtd, struct nand_chip *chip,
			      int page)
{
    memcpy(u8SpareBuf, chip->oob_poi, mtd->oobsize);

    return 0;
}

static int _unfd_nand_write_page_raw(struct mtd_info *mtd, struct nand_chip *chip,
				const uint8_t *buf, int oob_required)
{
...
#elif defined(CONFIG_ARM)
    if(!virt_addr_valid((U32)buf) || !virt_addr_valid((U32)buf + (U32)mtd->writesize - 1) || ((U32)buf) % CACHE_LINE  ) // For High MEM (ARM)
#elif defined(CONFIG_ARM64)
    if(!virt_addr_valid((uintptr_t)buf) || !virt_addr_valid((uintptr_t)buf + (U32)mtd->writesize - 1) || ((uintptr_t)buf) % CACHE_LINE  ) // For High MEM (ARM)
#endif
    {
        memcpy(pu8_MainBuf, buf, mtd->writesize);
    }
    else
    {
        pu8_MainBuf = buf;
    ...
    NC_WritePages(u32CurRow, pu8_MainBuf, u8SpareBuf, 1);
...
static ssize_t fcie_nand_bad_block_cnt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "Bad block count:%u\n", gu32_nand_bad_block_count);
}
...
#endif
{
    ...
    nand->ecc.read_oob_raw              = _unfd_nand_read_oob_raw;
    nand->ecc.read_page_raw             = _unfd_nand_read_page_raw;
    nand->ecc.write_oob_raw             = _unfd_nand_write_oob_raw;
    nand->ecc.write_page_raw            = _unfd_nand_write_page_raw;
