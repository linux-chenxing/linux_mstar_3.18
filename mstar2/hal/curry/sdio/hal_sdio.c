...
E_IO_STS HalSdio_SendCommandEx(CMD_RSP_INFO * pCmdRspInfo)
{
	...
	if(pCmdRspInfo->DirectTuningClock)
	{
		HalSdio_SetClock(pCmdRspInfo->DirectTuningClock);
	}
...
E_IO_STS HalSdio_SendCommand(CMD_RSP_INFO * pCmdRspInfo)
{
	...
	//static unsigned int read_test = 0;
	//static unsigned int write_test = 0;
	...
	if(pCmdRspInfo->CardClock==200000) // SDR104 mode
	{
		...
		if(gu8DowngradeClock) // slow down the clock
		{
			...
			if(gu8DowngradeClock==1)
			{
				...
				{
					pCmdRspInfo->DirectTuningClock = 100000;
			...
			else if(gu8DowngradeClock==2)
			{
				...
				{
					pCmdRspInfo->DirectTuningClock = 50000;
	...
	if( (pCmdRspInfo->Command.Cmd.Index == 53) && (clcok_change_to__50!=1) )
	{
		read_test++;
		if(read_test%10000==9999)
		{
			printk(LIGHT_PURPLE"do crack here\n"NONE);
