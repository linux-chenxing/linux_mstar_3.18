...
void MS_Cfd_PrintDataFormat_Range(MS_U8 input)
{
  switch (input) {

   case 0:
   printf("\n  infoFrame for HDMI TX:  Data_Format=RGB(default)    \n");
   break;

   case 1:
   printf("\n  infoFrame for HDMI TX:  Data_Format=YCbCr 422    \n");
   break;

   case 2:
   printf("\n  infoFrame for HDMI TX:  Data_Format=YCbCr 444    \n");
   break;

   case 3:
   printf("\n  infoFrame for HDMI TX:  Data_Format=YCbCr 420    \n");
   break;

   case 7:
   printf("\n  infoFrame for HDMI TX:  Data_Format=IDO-Defined    \n");
   break;

   default:
   printf("\n  infoFrame for HDMI TX:  Data_Format=Reserved    \n");
   break;
...
void MS_Cfd_PrintYUV_Range(MS_U8 input)
{

    switch (input) {

    case 0:
    printf("\n  infoFrame for HDMI TX:  YCC range=limited \n");
    break;

    case 1:
    printf("\n  infoFrame for HDMI TX:  YCC range=full \n");
    break;

    default:
    printf("\n  infoFrame for HDMI TX:  YCC range=Reserved \n");
    break;
...
void MS_Cfd_PrintRGB_Range(MS_U8 input)
{
    switch (input) {

    case 0:
    printf("\n  infoFrame for HDMI TX:  RGB range=Default \n");
    break;

    case 1:
    printf("\n  infoFrame for HDMI TX:  RGB range=limited \n");
    break;

    case 2:
    printf("\n  infoFrame for HDMI TX:  RGB range=full \n");
    break;

    default:
    printf("\n  infoFrame for HDMI TX:  RGB range=Reserved \n");
    break;
...
else
{
    ...
    MS_Cfd_PrintDataFormat_Range((pstu_api_HDMI_InfoFrame_Param->u8HDMISource_Support_Format&0x07));
    MS_Cfd_PrintYUV_Range(((pstu_api_HDMI_InfoFrame_Param->u8HDMISource_Support_Format>>3)&0x03));
    MS_Cfd_PrintRGB_Range(((pstu_api_HDMI_InfoFrame_Param->u8HDMISource_Support_Format>>5)&0x03));
...
//Maserati M+D IP patch 0
    ...
    if(TRUE == bCFDrun )
    {
        ...
        if(1 == pstu_OSD_Param->u8OSD_UI_Mode)
        {
            MS_Cfd_OSD_Control(pstu_OSD_Param, u8MainSubMode, pstu_ControlParam);
            return 0;
        }
    }
#endif
        ...
        (E_CFD_INPUT_SOURCE_JPEG == pstApiControlParamInit->u8Input_Source))
    {
        ...
        //printf("\033[1;35m###[Brian][%s][%d]### u16SourceMax 0 = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16SourceMax);
        //printf("\033[1;35m###[Brian][%s][%d]### u16SourceMed 0 = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16SourceMed);
        //printf("\033[1;35m###[Brian][%s][%d]### u16SourceMin 0 = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16SourceMin);
        ...
        if (E_CFD_VALID == pstHDMI_EDIDParamInit->u8HDMISink_EDID_Valid )
        {
...

            {
                ...
                else
                {
                    ...
                    //printf("\033[1;35m###[Brian][%s][%d]### f0 u16TgtMed = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16TgtMed);
...
//before Write Register function
#if (CFD_STB == CFD_PRODUCT)
    //only mark when test
    if(TRUE == bCFDrun )
    {
        printf("\033[1;35m###[Brian][%s][%d]### bCFDrun = %d \033[0m\n",__FUNCTION__,__LINE__,bCFDrun);
        printf("\033[1;35m###[Brian][%s][%d]### MS_Cfd_OSD_Control(User only update UI) !!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__);
        //pstu_OSD_Param->u16Hue = MApi_GFLIP_XC_R2BYTE(REG_SC_BK30_01_L);
        //pstu_OSD_Param->u16Saturation = MApi_GFLIP_XC_R2BYTE(REG_SC_BK30_02_L);
        //pstu_OSD_Param->u16Contrast = MApi_GFLIP_XC_R2BYTE(REG_SC_BK30_03_L);
        if(1 == pstu_OSD_Param->u8OSD_UI_Mode)
...
#else
void MS_Cfd_OSD_Control(STU_CFDAPI_OSD_CONTROL *pstu_OSD_Param, MS_U8 u8MainSubMode, STU_CFD_MS_ALG_COLOR_FORMAT *pstu_ControlParam)
{

    printf("\033[1;35m###[Brian][%s][%d]### u8OSD_UI_En = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_OSD_Param->u8OSD_UI_En);
    printf("\033[1;35m###[Brian][%s][%d]### u8OSD_UI_Mode = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_OSD_Param->u8OSD_UI_Mode);
    ...
    if(1 == pstu_OSD_Param->u8OSD_UI_En)
    {
        ...
        if(E_CFD_OSD_CONTROL_ONLY == pstu_OSD_Param->u8OSD_UI_Mode)//E_CFD_OSD_MODE
        {
            AdjustHueSatContrast(u8MainSubMode,pstu_OSD_Param, pstu_ControlParam);
            return;
        }
        else if(E_CFD_OSD_SOURCE_CHANGE == pstu_OSD_Param->u8OSD_UI_Mode)
        {
    ...
    {
        //for STB , don't use bypass case (not real bypass)
#if (CFD_TV == CFD_PRODUCT)
        pstu_OSD_Param->u16Contrast = 1024;
        pstu_OSD_Param->u16Hue = 50;
        pstu_OSD_Param->u16Saturation = 128;
        pstu_OSD_Param->u8ColorCorrection_En = 0;
        AdjustHueSatContrast(u8MainSubMode,pstu_OSD_Param, pstu_ControlParam);//Bypass setting
#endif
    }
}
