...
extern void piu_timer_mdelay(unsigned int mdelay);
...
unsigned int query_frequency(void)
{
    ...
    //test bus setting
    *(volatile unsigned short *)(mstar_pm_base + (0x101ee0 << 1)) &= (~0x8000);
    *(volatile unsigned short *)(mstar_pm_base + (0x101ee0 << 1)) |=  0x8000;
    *(volatile unsigned short *)(mstar_pm_base + (0x101ee0 << 1)) &= (~0x8000);

    msleep(5);

    reg_temp=*(volatile unsigned short *)(mstar_pm_base + (0x101ee2 << 1));
    return reg_temp * 12 * 4 / 1000;
#endif
}
EXPORT_SYMBOL(query_frequency);

unsigned int early_query_frequency(void)
{
#ifdef CONFIG_MSTAR_ARM_BD_FPGA
	return 12;
#else
...
EXPORT_SYMBOL(early_query_frequency);
