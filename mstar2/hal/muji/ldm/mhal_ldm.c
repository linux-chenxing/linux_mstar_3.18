...
{
    printk("!!error, not support in the chip \n");//not support
}
...
// h4c[9:5] reg_bl_height
void MHal_LD_SetLSFBacklightHeight(U8 u8Height) 
{
    MHal_LD_WriteByteMask(REG_LD_LSFBLW, ((u8Height-1) << 5), REG_LD_LSFBLH_BITS_L);
    MHal_LD_WriteByteMask(REG_LD_LSFBLH, ((u8Height-1) >> 3), REG_LD_LSFBLH_BITS_H);
}

U8 MHal_LD_GetLSFBacklightHeight(void) 
{
    return (((MHal_LD_Read2Byte(REG_LD_LSFBLW) & REG_LD_LSFBLH_BITS) >> 5)+1);
}
