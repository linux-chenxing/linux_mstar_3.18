...
void MHal_SATA_HW_Inital(phys_addr_t misc_base, phys_addr_t port_base, phys_addr_t hba_base)
{
...
#if 1
    u16Temp = readw(MSTAR_RIU_BASE + (0x102c2c << 1));
    u16Temp = u16Temp & (~E_PORT_SPEED_MASK);
    u16Temp = u16Temp | MHal_SATA_get_max_speed();
    writew(u16Temp, MSTAR_RIU_BASE + (0x102c2c << 1));
    u16Temp = readw(MSTAR_RIU_BASE + (0x102c2c << 1));
    printk("MAC sata speed= 0x%x\n", u16Temp);
#endif

    // SATA PHY OOB setting
    writew(0x96de, MSTAR_RIU_BASE + (0x103902 << 1));
    ...
    writew(0x2102, MSTAR_RIU_BASE + (0x10390c << 1));
    writew(0x818e, MSTAR_RIU_BASE + (0x103968 << 1)); // bit15 must be 1

    writew(0x0002, MSTAR_RIU_BASE + (0x103974 << 1));
    writew(0xc000, MSTAR_RIU_BASE + (0x103982 << 1));

    //writew(0x0200, MSTAR_RIU_BASE + (0x10390e << 1)); // enable
    writew(0x0000, MSTAR_RIU_BASE + (0x103910 << 1));

    writew(0x085e, MSTAR_RIU_BASE + (0x10390a << 1));
    ...
    writew(0x0001, hba_base + MS_HOST_CTL); // reset HBA

    //writew(0x969f, MSTAR_RIU_BASE + (0x103902 << 1));
    //writew(0x969e, MSTAR_RIU_BASE + (0x103902 << 1)); // [0]: PHY software reset

    // test move to drv layer
    //writew(0x0200, MSTAR_RIU_BASE + (0x10390e << 1)); // enable
}
...
void MHal_SATA_Setup_Port_Implement(phys_addr_t misc_base, phys_addr_t port_base, phys_addr_t hba_base)
{
    ...
    writew(0x0000, hba_base + (MS_HOST_CAP));
    writew(0x0000, hba_base + (MS_HOST_CAP + 0x4));

    // Port Implement
    writew(0x0001, hba_base + (MS_HOST_PORTS_IMPL));
    writew(0x0000, port_base + (MS_PORT_CMD));
    writew(0x0000, port_base + (MS_PORT_CMD + 0x4));
}
