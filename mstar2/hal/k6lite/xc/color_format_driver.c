...
MS_U8 MS_Cfd_SetFormat_OnlyY2R(MS_U8 u8input_format)
{
    MS_U8 u8temp = 0;
...
void MS_Cfd_top_control_Debug(STU_CFDAPI_MAIN_CONTROL *pstApiControlParamInit)
{
       ...
       //printf("\033[1;35m###[Brian][%s][%d]### Into CFD top debug function!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__);
...
MS_U8 MS_Cfd_CheckDoGM(MS_U8 input_format, MS_U8 input_dataformat, MS_U8 output_format, MS_U8 output_dataformat)
{
    ...
    if ((0 == transfer) || (2 == transfer))
    {
          ...
          ((input_format>=E_CFD_CFIO_RGB_BT601_625)&&(input_format<=E_CFD_CFIO_ADOBE_RGB)&&(output_format == E_CFD_CFIO_RGB_NOTSPECIFIED)) ||
             ((input_format>=E_CFD_CFIO_YUV_BT601_625)&&(input_format<=E_CFD_CFIO_ADOBE_YCC601)&&(output_format == E_CFD_CFIO_YUV_NOTSPECIFIED)))
        {
            u8CheckDoGMFlag = 0;
        }
        else
    ...
    else
    {
        ...
        {
            //call y2r() to compare
            u8TempFormtat = MS_Cfd_SetFormat_OnlyY2R(output_format);
        ...
        {
            //call y2r() to compare
            u8TempFormtat = MS_Cfd_SetFormat_OnlyY2R(input_format);
...
MS_U16 MS_Cfd_Kano_CheckSDRIPProcessIsOk(STU_CFDAPI_MAIN_CONTROL *pstApiControlParamInit, STU_CFD_MS_ALG_COLOR_FORMAT *pstu_ControlParam, MS_U8 u8_IP_capability)
{
    MS_U16 u16Check_status = E_CFD_MC_ERR_NOERR;
...
MS_U16 MS_Cfd_Curry_CheckSDRIPProcessIsOk(STU_CFDAPI_MAIN_CONTROL *pstApiControlParamInit, STU_CFD_MS_ALG_COLOR_FORMAT *pstu_ControlParam, MS_U8 u8_IP_capability)
{
    MS_U16 u16Check_status = E_CFD_MC_ERR_NOERR;
...
MS_U8 MS_Cfd_OutputHDMI_SetColorMetry_lite_out2(
    STU_CFD_MS_ALG_COLOR_FORMAT *pstu_ControlParam,
    STU_CFDAPI_MAIN_CONTROL *pstu_Main_Control
    )
{
...
//this version if for real 709 out
#if 0
    ...
    if (E_CFD_MC_FORMAT_RGB == pstControlParamInit->u8Input_DataFormat)
    {
        ...
        else
        {
            ...
            else if (E_CFIO_MODE_HDR2 == pstControlParamInit->u8Input_HDRMode)
            {
                ...
                HDR_DBG_HAL_CFD(printf("\nForce:Current HW can not support HDR RGB in\n"));
                pstControlParamInit->u8Input_HDRIPMode = 1;
                pstControlParamInit->u8Input_SDRIPMode = 1;
                pstControlParamInit->stu_Middle_Format->u8Mid_HDRMode = E_CFIO_MODE_HDR2;
                pstControlParamInit->u8Output_HDRMode = E_CFIO_MODE_HDR2;
...
MS_U16 Mapi_Cfd_inter_Main_Control_PreConstraints_Kano(STU_CFDAPI_MAIN_CONTROL *pstControlParamInit, STU_CFD_MS_ALG_COLOR_FORMAT *pstu_ControlParam)
{
    ...
    if (E_CFD_MC_FORMAT_RGB == pstControlParamInit->u8Input_DataFormat)
    {
        if (1 == pstControlParamInit->u8Input_IsRGBBypass)
        {
            ...
            else if (MS_Cfd_CheckOpenHDR(pstControlParamInit->u8Input_HDRMode))
            {
                ...
                if (E_CFIO_MODE_SDR == pstControlParamInit->u8Output_HDRMode)
                {
                    //force output eqaul to input , means bypass
                    //pstControlParamInit->u8Output_HDRMode = E_CFIO_MODE_HDR2;
                    //pstControlParamInit->stu_Middle_Format->u8Mid_HDRMode = E_CFIO_MODE_HDR2;
    ...
    else // YUV
    {
        ...
        else
        {
                ...
                pstControlParamInit->u8Output_Format = E_CFD_CFIO_RGB_BT2020;
                pstControlParamInit->u8Output_DataFormat =  E_CFD_MC_FORMAT_RGB;
                pstControlParamInit->u8Output_HDRMode = E_CFIO_MODE_SDR;
                pstControlParamInit->u8Output_IsFullRange = E_CFD_CFIO_RANGE_FULL;
        }
...
MS_U16 Mapi_Cfd_inter_Main_Control_PreConstraints_Curry(STU_CFDAPI_MAIN_CONTROL *pstControlParamInit, STU_CFD_MS_ALG_COLOR_FORMAT *pstu_ControlParam, STU_CFDAPI_HDMI_EDID_PARSER *pstHDMI_EDIDParamInit, MS_U8 u8TempMode, STU_CFDAPI_OSD_CONTROL  *pstu_OSD_Param)
{
    ...
    {
        pstControlParamInit->u8Input_IsRGBBypass = 0;
        HDR_DBG_HAL_CFD(printk("\nForce:Current HW can not set u8Input_IsRGBBypass = 1, force u8Input_IsRGBBypass to 0\n"));
        u16_check_status_tmp = E_CFD_MC_ERR_INPUT_ISRGBBYPASS_FORCE0;
    }
...
MS_U16 Mapi_Cfd_inter_Main_Control_PostConstraints_Kano(STU_CFD_MS_ALG_COLOR_FORMAT *pstu_ControlParam )
{
...
//printf("\033[1;35m###[Brian][%s][%d]### u8Video_HDRMode2 = %d !!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_OSD_Process_Configs->u8Video_HDRMode);
    ...
    pstu_api_HDMI_InfoFrame_Param->u8HDMISource_Support_Format = 0x28;
    ...
    if ((E_CFD_CFIO_ADOBE_RGB == pstu_ControlParam->u8Output_Format) && (0x10==(pstHDMI_EDIDParamInit->u8HDMISink_Extended_Colorspace&0x10)))
    {
        pstu_api_HDMI_InfoFrame_Param->u8HDMISource_Colorspace = E_CFD_CFIO_ADOBE_RGB;
    }
    else if ((E_CFD_CFIO_RGB_BT2020 == pstu_ControlParam->u8Output_Format) && (0x80==(pstHDMI_EDIDParamInit->u8HDMISink_Extended_Colorspace&0x80)))
    {
    ...
    //pstu_OSD_Process_Configs->u8Video_IsFullRange = pstu_ControlParam->u8Output_IsFullRange;
...
*/
    ...
    if (( E_CFD_CFIO_DOLBY_HDR_TEMP != pstApiControlParamInit->u8Input_Format) || ( E_CFIO_MODE_HDR1 != pstApiControlParamInit->u8Input_HDRMode))
    {
        u16_check_status_tmp = Mapi_Cfd_inter_Main_Control_PreConstraints_Kano(pstApiControlParamInit,pstu_ControlParam);
        stu_ControlParam.u8DoPreConstraints_Flag = 1;
    }
#endif
    ...
    if (( E_CFD_CFIO_DOLBY_HDR_TEMP != pstApiControlParamInit->u8Input_Format) || ( E_CFIO_MODE_HDR1 != pstApiControlParamInit->u8Input_HDRMode))
    {
        ...
        u16_check_status_tmp = Mapi_Cfd_inter_Main_Control_PreConstraints_Curry(pstApiControlParamInit,pstu_ControlParam,pstHDMI_EDIDParamInit, 0, pstu_OSD_Param);
        stu_ControlParam.u8DoPreConstraints_Flag = 1;
    }
#endif
#endif
    ...
    if ((E_CFD_INPUT_SOURCE_STORAGE == pstApiControlParamInit->u8Input_Source)||
        ...
        (E_CFD_INPUT_SOURCE_JPEG == pstApiControlParamInit->u8Input_Source))
    {
                  //printf("\033[31m [%s] CFD  MM/DTV source\033[m\n", __func__);
                     //printf("\033[1;35m###[Brian][%s][%d]### Into CFD MM/DTV source!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__);
               //check for MM input
    ...
    {
            //printf("\033[31m [%s] HDMI source\033[m\n", __func__);
    ...
    //this is special case for Masearti
        ...
        if (E_CFD_OUTPUT_SOURCE_HDMI == pstApiControlParamInit->u8Output_Source)
        {
            ...
            else if (2 == pstApiControlParamInit->u8HDMIOutput_GammutMapping_Mode)
            {
                pstApiControlParamInit->stu_Middle_Format[0].u8Mid_Format = E_CFD_CFIO_YUV_BT709;
                u8tempMode = 1;
            }
...
//#if (HW_PIPE_CONSTRAINTS == 1)
    if (E_CFD_OUTPUT_SOURCE_HDMI == pstApiControlParamInit->u8Output_Source) //STB
    {
        ...
        //printf("\033[1;35m###[Brian][%s][%d]### Into CFD HDMI output source!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__);
        ...
        if (1 == pstHDMI_EDIDParamInit->u8HDMISink_EDID_Valid )
        {
            ...
            //if (1 == stu_TMO_Input.stu_CFD_TMO_Param.u8TMO_TargetMode)
            {
                            ...
                            //printf("\033[1;35m###[Brian][%s][%d]### f1 pstApiControlParamInit->u8TMO_TargetRefer_Mode = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstApiControlParamInit->u8TMO_TargetRefer_Mode);
                ...
                if ((1 == pstApiControlParamInit->u8TMO_TargetRefer_Mode) && (E_CFD_VALID == pstHDMI_EDIDParamInit->u8HDMISink_HDRData_Block_Valid))
                {
                    ...
                    pstu_ControlParam->stu_CFD_TMO_Param.u16TgtMinFlag = *pu8_unit_flag;
                                   ...
                                   //printf("\033[1;35m###[Brian][%s][%d]### f1 u16TgtMax = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16TgtMax);
                                   //printf("\033[1;35m###[Brian][%s][%d]### f1 u16TgtMed = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16TgtMed);
                                   //printf("\033[1;35m###[Brian][%s][%d]### f1 u16TgtMin = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16TgtMin);
                                   //printf("\033[1;35m###[Brian][%s][%d]### f1 u16TgtMinFlag = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16TgtMinFlag);
                ...
                else
                {
                    ...
                    //printf("\033[1;35m###[Brian][%s][%d]### f0 u16TgtMax = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16TgtMax);
                                        //printf("\033[1;35m###[Brian][%s][%d]### f0 u16TgtMin = %d!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__,pstu_ControlParam->stu_CFD_TMO_Param.u16TgtMin);
                }
            }
...
//#endif
    ...
    if (( E_CFD_CFIO_DOLBY_HDR_TEMP != pstApiControlParamInit->u8Input_Format) || ( E_CFIO_MODE_HDR1 != pstApiControlParamInit->u8Input_HDRMode))
    {
        Mapi_Cfd_inter_Main_Control_PostConstraints_Kano(pstu_ControlParam);
    }
#endif
#elif ((NowHW == Curry) || (NowHW == Kastor))
#if defined HE_PIPE_POST_CONSTRAINTS
    if (( E_CFD_CFIO_DOLBY_HDR_TEMP != pstApiControlParamInit->u8Input_Format) || ( E_CFIO_MODE_HDR1 != pstApiControlParamInit->u8Input_HDRMode))
    {
        Mapi_Cfd_inter_Main_Control_PostConstraints_Curry(pstu_ControlParam);
    }
#endif
#endif
    ...
    u16_check_status_tmp = MS_Cfd_Kano_CheckSDRIPProcessIsOk(pstApiControlParamInit,&stu_ControlParam, CFD_SDR_IP_CAPABILITY);
    ...
    u16_check_status_tmp = MS_Cfd_Curry_CheckSDRIPProcessIsOk(pstApiControlParamInit,&stu_ControlParam, CFD_SDR_IP_CAPABILITY);
    ...
    MS_Cfd_Kano_SDRIP_Debug(&(pstu_Cfd_api_top->pstu_HW_IP_Param->pstu_DLC_Input->stu_Kano_DLC_Param), &(pstu_Cfd_api_top->pstu_HW_IP_Param->pstu_SDRIP_Input->stu_Kano_SDRIP_Param));
       ...
       //printf("\033[1;35m###[Brian][%s][%d]### Into CFD HW debug function!!!!!!!!!!!!!!!!!\033[0m\n",__FUNCTION__,__LINE__);
...
void MS_Cfd_OSD_Control(STU_CFDAPI_OSD_CONTROL *pstu_OSD_Param,MS_U8 u8MainSubMode)
{
    ...
    {
        AdjustHueSatContrast(u8MainSubMode,50,128,1024);//Bypass setting
    }
}
