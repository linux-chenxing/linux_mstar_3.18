...
U32 nand_pads_switch(U32 u32EnableFCIE)
{
    ...
    nand_debug(UNFD_DEBUG_LEVEL_HIGH, 0,"reg_nf_en(%08X)=%04X\n", reg_nf_en, (uintptr_t)REG(reg_nf_en));
    nand_debug(UNFD_DEBUG_LEVEL_HIGH, 0,"reg_allpad_in(%08X)=%04X\n", reg_allpad_in, (uintptr_t)REG(reg_allpad_in));
...
void nand_DumpPadClk(void)
{
	...
	nand_debug(0, 1, "reg_ckg_ecc(0x%lX):0x%lx\n", reg_ckg_ecc, (uintptr_t)REG(reg_ckg_ecc));
	nand_debug(0, 1, "fciepad:0x%lx\n", (uintptr_t)fciepad);
}
