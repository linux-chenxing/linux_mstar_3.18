    ...
    {
        {
            ...
            .DvfsNormalModeInfo =
            {
                ...
                {
                    //Normal Mode: 1301MHz - 1500MHz
                    .DvfsCpuInfo =
                    {
                        .dwLowerCpuClock    = 1301,
...
//  U32     dwRegisterValue = 0;
    ...
    if(DvfsRegInfo->reg_vid_dvfs_id == CONFIG_DVFS_ENABLE_PATTERN)
    {
        if(hMstarDvfsInfo.bDvfsInitOk == 0)
        {
            ...
            {
                DVFS_HAL_INFO("\033[31m[DVFS] Function: %s, Line: %d \033[0m\n", __FUNCTION__, __LINE__);
        ...
        else if(DvfsRegInfo->reg_cur_dvfs_state == CONFIG_DVFS_OVER_TEMPERATURE_MODE)
        {
            ...
            if(hMstarDvfsInfo.dwCpuTemperature > DvfsRegInfo->reg_max_level_temp)
            {
                ...
                {
                    DVFS_HAL_DEBUG("\033[31m[DVFS] Over Temperature Protection: Count = %d / Temperature = %d\033[0m\n", (unsigned int) hMstarDvfsInfo.dwResetCounter, (unsigned int) hMstarDvfsInfo.dwCpuTemperature);
                    hMstarDvfsInfo.dwResetCounter ++;
                }
                else
                {
                    DVFS_HAL_INFO("\033[31m[DVFS] Current Temperature: %d\033[0m\n", (unsigned int) hMstarDvfsInfo.dwCpuTemperature);
                    DVFS_HAL_INFO("\033[31m[DVFS] Over Temperature Mode: SYSTEM RESET\033[0m\n");
...
void MHalDvfsCpuTemperature(void)
{
    ...
    //Read CH7 of PM_SAR to Get CPU Temperature
    *(volatile U16 *)(CONFIG_REGISTER_BASE_ADDRESS + (0x001400 << 1)) |= (0x01 << 14);
    //For CH7 Setting. Write reg_pm_dmy[0] = 1b'1.
    *(volatile U16 *)(CONFIG_REGISTER_BASE_ADDRESS + (0x001420 << 1)) |= 0x01;
    dwTempData = *(volatile U16 *)(CONFIG_REGISTER_BASE_ADDRESS + (0x00148c << 1));
    if(dwTempData == 0)
    {
