...
U32 nand_pads_switch(U32 u32EnableFCIE)
{
    ...
    nand_debug(UNFD_DEBUG_LEVEL_LOW,1,"reg_allpad_in(%08lX)=%04X\n", reg_allpad_in, REG(reg_allpad_in));
...
U32 nand_clock_setting(U32 u32ClkParam)
{
    ...
    nand_debug(UNFD_DEBUG_LEVEL_HIGH, 0,"reg_ckg_fcie(%08lX)=%08X\n",
...
void nand_DumpPadClk(void)
{
    ...
    nand_debug(0, 1, "\npad setting: \n");
    //fcie pad register
    nand_debug(0, 1, "NC_DDR_CTRL(0x%lX):0x%x\n", NC_DDR_CTRL, REG(NC_DDR_CTRL));
    //chiptop pad register
    nand_debug(0, 1, "reg_all_pad_in(0x%lX):0x%x\n", reg_allpad_in, REG(reg_allpad_in));

    nand_debug(0, 1, " reg_sync_pe(0x%08lX): 0x%04X\n", reg_sync_pe,  REG(reg_sync_pe));
    nand_debug(0, 1, " reg_s_gpio_pe(0x%08lX): 0x%04X\n", reg_s_gpio_pe,  REG(reg_s_gpio_pe));   
}
...
{	
	...
	else if( dma_DMAAddr >= 0xA0000000)
	{
		_chip_flush_miu_pipe();
		REG_CLR_BITS_UINT16( NC_MIU_DMA_SEL, BIT_MIU0_SELECTn);
		dma_DMAAddr -= 0xA0000000;
	}
	else
	{
		_dma_cache_wback_inv(dma_DMAAddr, u32_ByteCnt);
