...
MS_BOOL bJump=FALSE;
static ST_KDRV_XC_HDR_GD_FORMAT _astHDRGDFormat[HDR_MEM_ENTRY_NUM];
...
void initDolbyHDRMem(ST_KDRV_XC_HDR_DOLBY_MEMORY_FORMAT_EX* pstHDRShareMem)
{
    ...
    memset(&_astHDRGDFormat, 0, sizeof(ST_KDRV_XC_HDR_GD_FORMAT) * HDR_MEM_ENTRY_NUM);
}
...
void setDolbyHDRGDInfo(MS_U32 u32Idx, ST_KDRV_XC_HDR_GD_FORMAT *pstHDRGDInfo)
{
    ...
    memcpy(&_astHDRGDFormat[u32Idx], pstHDRGDInfo, sizeof(ST_KDRV_XC_HDR_GD_FORMAT));
}
...
MS_BOOL getDolbyHDRGDInfo(MS_U32 u32Idx, ST_KDRV_XC_HDR_GD_FORMAT *pstHDRGDInfo)
{
    ...
    memcpy(pstHDRGDInfo, &_astHDRGDFormat[u32Idx], sizeof(ST_KDRV_XC_HDR_GD_FORMAT));
}
...
MS_U32 getDolbyHDRLutAddr(MS_U32 u32ShareMemBaseAddr, MS_U32 u32Idx)
{
    ...
    u32RetAddr = u32ShareMemBaseAddr + HDR_MEM_COMMON_ENTRY_SIZE + HDR_TOTAL_INPUT_MD_SIZE + HDR_TOTAL_REGSET_SIZE + u32Idx * HDR_DV_3DLUT_NUM * BYTE_PER_WORD;
