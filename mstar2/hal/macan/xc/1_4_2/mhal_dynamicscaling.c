...
typedef struct
{
    ...
    MS_U16 su16OldValue_ForceMadi;
    MS_BOOL bOldValue_ForceP;
} KHAL_DS_PRIVATE;
...
void KHal_SC_ResetSWDSCommand(EN_KDRV_SC_DEVICE u32DeviceID,EN_KDRV_WINDOW eWindow, MS_U8 u8DSIndex)
{
    ...
    MS_U16 u16SWRstCmd_Pointer = (client_ds[u32DeviceID].u8DynamicScalingIndexDepth - 1) * (BYTE_PER_WORD / DS_CMD_LEN_64BITS);
...
MS_BOOL KHal_XC_InitDynamicScalingIndex(EN_KDRV_SC_DEVICE u32DeviceID, EN_KDRV_WINDOW eWindow)
{
    ...
    client_ds[u32DeviceID].au8DSCurrentIndex[eWindow] = 3;
    client_ds[u32DeviceID].au8DSFireIndex[eWindow] = 3;
...
MS_BOOL KHAL_SC_Set_DynamicScaling(EN_KDRV_SC_DEVICE u32DeviceID,MS_PHY u32MemBaseAddr, MS_U8 u8MIU_Select, MS_U8 u8IdxDepth, MS_BOOL bOP_On, MS_BOOL bIPS_On, MS_BOOL bIPM_On, MS_U32 u32DSBufferSize, MS_BOOL bEnable_ForceP, EN_KDRV_WINDOW eWindow)
{
    ...
    static MS_VIRT tmp_phy = 0;
    MS_U16 u16PanelHTotal = 0;
    MS_U16 u16PanelVTotal = 0;

    u16PanelHTotal = SC_R2BYTEMSK(u32DeviceID,XC_ADDR_L(0x10,0x0C), 0x3FFF);
    u16PanelVTotal = SC_R2BYTEMSK(u32DeviceID,XC_ADDR_L(0x10,0x0D), 0x1FFF);
    DS_DBG(printk("[%s,%5d] [DS] panel info [htotal %u, vtotal %u]\n", __FUNCTION__,__LINE__, u16PanelHTotal, u16PanelVTotal));
...
//*****************init value*******************
    if (ds_private[u32DeviceID].sbOldValueReaded == FALSE)
    {
        ...
        ds_private[u32DeviceID].su16OldValue_ForceMadi = 0;
        ds_private[u32DeviceID].bOldValue_ForceP = FALSE;
    }
//**********************************************
...
//===============================================================//
...
#if 0//#if defined(CONFIG_MSTAR_XC_HDR_SUPPORT)
    ...
    client_ds[u32DeviceID].u8DynamicScalingIndexDepth = 16;

    client_ds[u32DeviceID].u32DynamicScalingBufSize = 0x1800; //For pureSN case, DS DRAM size is 0x1800
#endif

    client_ds[u32DeviceID].u8DynamicScalingIndexDepth = u8IdxDepth;

    if(client_ds[u32DeviceID].u8DynamicScalingIndexDepth == 255)//For avp, Dolby
    {
        client_ds[u32DeviceID].u32DynamicScalingBufSize = 0x30000;
    }
    else //for vdp
    {
        if (u32DSBufferSize != 0xFFF00)
        {
            client_ds[u32DeviceID].u32DynamicScalingBufSize = 0x30000;
        }
        else
        {
            client_ds[u32DeviceID].u32DynamicScalingBufSize = 0xFFF00;//externel DS buffer size = 1M-0x100
        }
    }
    ...
    if(u32MemBaseAddr)
    {
        ...
        tmp_phy = client_ds[u32DeviceID].va_phy;
        //*******************************************************
    }
    else
    {
...
//        {
    ...
    if(u16OnOff & 0x0E00)
    {
        ...
        if(ds_private[u32DeviceID].sbOldValueReaded == FALSE)
        {
            ...
            if(bEnable_ForceP)
            {
                ds_private[u32DeviceID].bOldValue_ForceP = bEnable_ForceP;
                ds_private[u32DeviceID].su16OldValue_ForceMadi = SC_R2BYTE(u32DeviceID, XC_ADDR_L(0x12,0x02));
            }
    ...
    else
    {
        ...
        if(ds_private[u32DeviceID].sbOldValueReaded)
        {
            ...
            if(ds_private[u32DeviceID].bOldValue_ForceP)
            {
                ds_private[u32DeviceID].bOldValue_ForceP = bEnable_ForceP;
                SC_W2BYTE(u32DeviceID, XC_ADDR_L(0x12,0x02), ds_private[u32DeviceID].su16OldValue_ForceMadi);
            }
    ...
    if(u16OnOff & 0x0E00)
    {
        if (FALSE == ds_private[u32DeviceID].bTrig_pStored)
        {
            ...
            SC_W2BYTEMSK(u32DeviceID, XC_ADDR_L(0x1F,0x10), 0xE8, 0x00FF);
            ...
            if((u16PanelHTotal > 3000) && (u16PanelVTotal > 2000))
            {
    ...
    {
        // restore
        if (ds_private[u32DeviceID].bTrig_pStored)
        {
            ds_private[u32DeviceID].bTrig_pStored = FALSE;
            if((u16PanelHTotal > 3000) && (u16PanelVTotal > 2000))
            {
            ...
            {
                ds_private[u32DeviceID].bOldValue_ForceP = bEnable_ForceP;
                SC_W2BYTEMSK(u32DeviceID, XC_ADDR_L(0x12,0x01), 0, BIT(9));
            }
        }
    }
...
{
    SC_W2BYTEMSK(u32DeviceID, XC_ADDR_L(0x1F,0x13), client_ds[u32DeviceID].u8DynamicScalingIndexDepth , 0x00FF);
}
...
MS_BOOL KHal_DS_set_client(EN_KDRV_SC_DEVICE u32DeviceID,E_DS_CLIENT client,MS_U32 max_depth_num)
{
    ...
    {
        printk("\033[1;34m[%s:%d]Error: client[%d]: u8DynamicScalingIndexDepth=%d,offset=%d\033[m\n",__FUNCTION__,__LINE__,client, client_ds[u32DeviceID].u8DynamicScalingIndexDepth,offset);
        return FALSE;
    }
    //******************************//
    ds_client_data data;
...
MS_BOOL KHal_SC_Set_DS_StoredAddr(KHAL_DS_STORED_ADDR_INFO *pstDSStoredAddr)
{
    ...
    _stDSStoredAddrInfo.stIPMAddr.virtDSAddr = pstDSStoredAddr->stIPMAddr.virtDSAddr;
    ...
    _stDSStoredAddrInfo.stOPMAddr.virtDSAddr = pstDSStoredAddr->stOPMAddr.virtDSAddr;
    ...
    _stDSStoredAddrInfo.stIPSAddr.virtDSAddr = pstDSStoredAddr->stIPSAddr.virtDSAddr;
    ...
    _stDSStoredAddrInfo.stOPSAddr.virtDSAddr = pstDSStoredAddr->stOPSAddr.virtDSAddr;
                                    ...
                                    MS_U32 u32CmdCnt, k_ds_reg_ip_op_sel IPOP_Sel, k_ds_reg_source_sel Source_Select, K_XC_DS_CMDCNT *pstXC_DS_CmdCnt, MS_U8 u8DSIndex)
{
    ...
    //align memory
    if (MS_MLOAD_BUS_WIDTH < BYTE_PER_WORD)
    {
        MS_U32 u32CmdCntPerMIUBus = (MS_MLOAD_BUS_WIDTH / MS_MLOAD_CMD_LEN_64BITS);
        MS_U16 u16CMDCNT = 0;
        if(Source_Select == K_DS_XC)
        {
            if(IPOP_Sel == K_DS_IP)
            {
                if(eWindow == E_KDRV_MAIN_WINDOW)
                {
                    u16CMDCNT = pstXC_DS_CmdCnt->u16CMDCNT_IPM;
                }
                else//subwindow
                {
                    u16CMDCNT = pstXC_DS_CmdCnt->u16CMDCNT_IPS;
                }
            }
            else//DS_OP
            {
                if(eWindow == E_KDRV_MAIN_WINDOW)
                {
                    u16CMDCNT = pstXC_DS_CmdCnt->u16CMDCNT_OPM;
                }
                else
                {
                    u16CMDCNT = pstXC_DS_CmdCnt->u16CMDCNT_OPS;
                }
            }
        }
        else if(Source_Select == K_DS_MVOP)// only in IP cmd
        {
            if(IPOP_Sel == K_DS_IP)
            {
                ...
                {
                    u16CMDCNT = pstXC_DS_CmdCnt->u16CMDCNT_IPS;//IPS
                }
            }
            else
            {
        ...
        int i = u16CMDCNT % u32CmdCntPerMIUBus;
        if (i != 0)
        {
            for (; i < u32CmdCntPerMIUBus; i++)
            {
                //Dummy cmd BKFF_7F=0x0000
                KHal_SC_WriteSWDSCommand(u32DeviceID, eWindow, client,(((MS_U16)(0xFF) << 8) | (MS_U16)((0x7F)*2)), 0x0000, IPOP_Sel, K_DS_XC, pstXC_DS_CmdCnt, u8DSIndex);
            }
    ...
    if(Source_Select == K_DS_XC)
    {
        if(IPOP_Sel == K_DS_IP)
        {
            if(eWindow == E_KDRV_MAIN_WINDOW)
            {
                ...
                pu64Data = (MS_U64*)_DSGET_PA2VA(u32DeviceID, u32Addr);//IPM
#endif
                //printk("WriteStoredSWDSCommand: 0x%x, %d, %d\n", u32Addr, *pu8CmdData, u32BufferSize);

                if(pstXC_DS_CmdCnt->u16CMDCNT_IPM < u16MaxCmdCnt)
                {
#ifdef XC_REPLACE_MEMCPY_BY_BDMA
                    MHal_XC_MemCpy_by_BDMA((MS_U32)pu8CmdData, pu64Data, u32BufferSize);
#else
...
#ifdef XC_REPLACE_MEMCPY_BY_BDMA
        MS_U32 *pu32Data = u32OPAddr;
        MHal_XC_MemCpy_by_BDMA((MS_U32)pu8CmdData, pu32Data, u32BufferSize);
#else
