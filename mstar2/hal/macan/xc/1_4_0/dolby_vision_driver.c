...
static MS_BOOL _bNeedDownloadGamma = TRUE;
static MS_BOOL _bNeedDownloadDegamma = TRUE;
static MS_BOOL _bNeedDownloadTMO = TRUE;
...
{
    MsHdr_Comp_Regtable_t* pRegTable = NULL;
    pRegTable = (MsHdr_Comp_Regtable_t*)MsHdr_Malloc(sizeof(MsHdr_Comp_Regtable_t));
    if (pRegTable == NULL)
    {
        printk("[%s %d]memory allocate error!",__FUNCTION__,__LINE__);
        return NULL;
    }
...
{
    DoVi_Comp_ExtConfig_t* pConfig = NULL;
...
//PQ settings
void DoVi_DumpPQSettings(const DoVi_TargetDisplayConfig_t   *pTargetDisplayConfig)
{
    int i = 0;
    int j = 0;

    DoVi_Printf("//%x //size of  struct DoVi_TargetDisplayConfig_t \n", sizeof(DoVi_TargetDisplayConfig_t));
    DoVi_Printf("//%x //Gain\n", pTargetDisplayConfig->Gain);
    DoVi_Printf("//%x //Offset\n", pTargetDisplayConfig->Offset);
    DoVi_Printf("//%x //Gamma\n", pTargetDisplayConfig->Gamma);
    DoVi_Printf("//%x //Eotf\n", pTargetDisplayConfig->Eotf);
    DoVi_Printf("//%x //BitDepth\n", pTargetDisplayConfig->BitDepth);
    DoVi_Printf("//%x //RangeSpec\n", pTargetDisplayConfig->RangeSpec);
    DoVi_Printf("//%x //DiagSize\n", pTargetDisplayConfig->DiagSize);
    DoVi_Printf("//%x //MaxPq\n", pTargetDisplayConfig->MaxPq);
    DoVi_Printf("//%x //MinPq\n", pTargetDisplayConfig->MinPq);
    DoVi_Printf("//%x //MSWeight\n", pTargetDisplayConfig->MSWeight);
    DoVi_Printf("//%x //MSEdgeWeight\n", pTargetDisplayConfig->MSEdgeWeight);
    DoVi_Printf("//%x //MinPQBias\n", pTargetDisplayConfig->MinPQBias);
    DoVi_Printf("//%x //MidPQBias\n", pTargetDisplayConfig->MidPQBias);
    DoVi_Printf("//%x //MaxPQBias\n", pTargetDisplayConfig->MaxPQBias);
    DoVi_Printf("//%x //trimSlopeBias\n", pTargetDisplayConfig->trimSlopeBias);
    DoVi_Printf("//%x //trimOffsetBias\n", pTargetDisplayConfig->trimOffsetBias);
    DoVi_Printf("//%x //trimPowerBias\n", pTargetDisplayConfig->trimPowerBias);
    DoVi_Printf("//%x //msWeightBias\n", pTargetDisplayConfig->msWeightBias);
    DoVi_Printf("//%x //Brightness\n", pTargetDisplayConfig->Brightness);
    DoVi_Printf("//%x //Contrast\n", pTargetDisplayConfig->Contrast);
    DoVi_Printf("//%x //ChromaWeightBias\n", pTargetDisplayConfig->ChromaWeightBias);
    DoVi_Printf("//%x //SaturationGainBias\n", pTargetDisplayConfig->SaturationGainBias);
    DoVi_Printf("//%x //ChromaWeight\n", pTargetDisplayConfig->ChromaWeight);
    DoVi_Printf("//%x //SaturationGain\n", pTargetDisplayConfig->SaturationGain);
    DoVi_Printf("//%x //CrossTalk\n", pTargetDisplayConfig->CrossTalk);
    DoVi_Printf("//%x //TuningMode\n", pTargetDisplayConfig->TuningMode);
    // int16_t  reserved[10];
    DoVi_Printf("//%x //BackLightUI\n", pTargetDisplayConfig->BackLightUI);
    DoVi_Printf("//%x //BrightnessUI\n", pTargetDisplayConfig->BrightnessUI);
    DoVi_Printf("//%x //ContrastUI\n", pTargetDisplayConfig->ContrastUI);

    for (i = 0; i < 14; i++)
    {
        DoVi_Printf("//%x //MidPQBiasLut[%x]\n", pTargetDisplayConfig->MidPQBiasLut[i], i);
    }

    for (i = 0; i < 14; i++)
    {
        DoVi_Printf("//%x //SaturationGainBiasLut[%x]\n", pTargetDisplayConfig->SaturationGainBiasLut[i], i);
    }

    for (i = 0; i < 14; i++)
    {
        DoVi_Printf("//%x //ChromaWeightBiasLut[%x]\n", pTargetDisplayConfig->ChromaWeightBiasLut[i], i);
    }

/*
    //int16_t  reserved0Lut[14];
    //int16_t  reserved1Lut[14];
    //int16_t  reserved2Lut[14];
    TgtGDCfg_t gdConfig;
*/

    //TgtGDCfg_t gdConfig;
    DoVi_Printf("//%x //gdEnable\n", pTargetDisplayConfig->gdConfig.gdEnable);
    DoVi_Printf("//%x //gdWMin\n", pTargetDisplayConfig->gdConfig.gdWMin);
    DoVi_Printf("//%x //gdWMax\n", pTargetDisplayConfig->gdConfig.gdWMax);
    DoVi_Printf("//%x //gdWMm\n", pTargetDisplayConfig->gdConfig.gdWMm);
    DoVi_Printf("//%x //gdWDynRngSqrt\n", pTargetDisplayConfig->gdConfig.gdWDynRngSqrt);
    DoVi_Printf("//%x //gdWeightMean\n", pTargetDisplayConfig->gdConfig.gdWeightMean);
    DoVi_Printf("//%x //gdWeightStd\n", pTargetDisplayConfig->gdConfig.gdWeightStd);
    DoVi_Printf("//%x //gdDelayMilliSec\n", pTargetDisplayConfig->gdConfig.gdDelayMilliSec);
    DoVi_Printf("//%x //gdRgb2YuvExt\n", pTargetDisplayConfig->gdConfig.gdRgb2YuvExt);

    for (j = 0; j <= 3; j++)
    {
        for (i = 0; i < 3; i++)
        {
	      DoVi_Printf("//%x //gdM33Rgb2Yuv[%x][%x]\n", pTargetDisplayConfig->gdConfig.gdM33Rgb2Yuv[j][i], j,i);
        }
    }

    DoVi_Printf("//%x //gdM33Rgb2YuvScale2P\n", pTargetDisplayConfig->gdConfig.gdM33Rgb2YuvScale2P);
    DoVi_Printf("//%x //gdRgb2YuvOffExt\n", pTargetDisplayConfig->gdConfig.gdRgb2YuvOffExt);

    for (i = 0; i < 3; i++)
    {
        DoVi_Printf("//%x //gdV3Rgb2YuvOff[%x]\n", pTargetDisplayConfig->gdConfig.gdV3Rgb2YuvOff[i], i);
    }

}

//configures
void DoVi_DumpDmConfigs(const DoVi_Config_t* pConfig)
{
    int i = 0;
    int j = 0;

    DoVi_Printf("//%x //u8InputBits\n", pConfig->u8InputBits);
    DoVi_Printf("//%x //u8OutputBits\n", pConfig->u8OutputBits);
    DoVi_Printf("//\n");
    //DoVi_Printf("%x //m33Yuv2RgbScale2P\n", pMdsExt->m33Yuv2RgbScale2P);
    DoVi_Printf("//%x //u16SminPQ\n", pConfig->u16SminPQ);
    DoVi_Printf("//%x //u16SmaxPQ\n", pConfig->u16SmaxPQ);
    DoVi_Printf("//%x //u16Sgamma\n", pConfig->u16Sgamma);
    DoVi_Printf("//%x //u8SMode\n", pConfig->u8SMode);
    DoVi_Printf("//%x //u8SEOTF\n", pConfig->u8SEOTF);
    DoVi_Printf("//%x //u8SSignalRange\n", pConfig->u8SSignalRange);
    DoVi_Printf("//%x //u16Sdiagonalinches\n", pConfig->u16Sdiagonalinches);
    DoVi_Printf("//\n");
    DoVi_Printf("//%x //s32CrossTalk\n", pConfig->s32CrossTalk);
    DoVi_Printf("//%x //u16TminPQ\n", pConfig->u16TminPQ);
    DoVi_Printf("//%x //u16TmaxPQ\n", pConfig->u16TmaxPQ);
    DoVi_Printf("//%x //u16Tgamma\n", pConfig->u16Tgamma);
    DoVi_Printf("//%x //u8TMode\n", pConfig->u8TMode);
    DoVi_Printf("//%x //u8TEOTF\n", pConfig->u8TEOTF);
    DoVi_Printf("//%x //u8TSignalRange\n", pConfig->u8TSignalRange);
    DoVi_Printf("//%x //u16Tdiagonalinches\n", pConfig->u16Tdiagonalinches);
    DoVi_Printf("//\n");
    DoVi_Printf("//%x //u8RunMode\n", pConfig->u8RunMode);
    DoVi_Printf("//%x //s16TMinPQBias\n", pConfig->s16TMinPQBias);
    DoVi_Printf("//%x //s16TMidPQBias\n", pConfig->s16TMidPQBias);
    DoVi_Printf("//%x //s16TMaxPQBias\n", pConfig->s16TMaxPQBias);
    DoVi_Printf("//%x //s16TrimSlopeBias\n", pConfig->s16TrimSlopeBias);
    DoVi_Printf("//%x //s16TrimOffsetBias\n", pConfig->s16TrimOffsetBias);
    DoVi_Printf("//%x //s16TrimPowerBias\n", pConfig->s16TrimPowerBias);
    DoVi_Printf("//%x //s16TContrast\n", pConfig->s16TContrast);
    DoVi_Printf("//%x //s16TBrightness\n", pConfig->s16TBrightness);
    DoVi_Printf("//%x //s32Rolloff\n", pConfig->s32Rolloff);
    DoVi_Printf("//\n");
    DoVi_Printf("//%x //u8MsMethod\n", pConfig->u8MsMethod);
    DoVi_Printf("//%x //s16MsWeight\n", pConfig->s16MsWeight);
    DoVi_Printf("//%x //s16MsWeightBias\n", pConfig->s16MsWeightBias);
    DoVi_Printf("//%x //u16MsWeightEdge\n", pConfig->u16MsWeightEdge);
    DoVi_Printf("//\n");
    DoVi_Printf("//%x //u16CrossTalk\n", pConfig->u16CrossTalk);
    DoVi_Printf("//%x //u16ChromaWeight\n", pConfig->u16ChromaWeight);
    DoVi_Printf("//%x //s16ChromaWeightBias\n", pConfig->s16ChromaWeightBias);
    DoVi_Printf("//%x //u16CcGain\n", pConfig->u16CcGain);
    DoVi_Printf("//%x //u16CcOffset\n", pConfig->u16CcOffset);
    DoVi_Printf("//%x //u16SaturationGain\n", pConfig->u16SaturationGain);
    DoVi_Printf("//%x //u8MsMethod\n", pConfig->u8MsMethod);
    DoVi_Printf("//%x //s16SaturationGainBias\n", pConfig->s16SaturationGainBias);

    for (i = 0; i < 3; i++)
    {
        DoVi_Printf("//%x //s16Lut3DMin[%x]\n", pConfig->s16Lut3DMin[i], i);
    }

    for (i = 0; i < 3; i++)
    {
        DoVi_Printf("//%x //s16Lut3DMax[%x]\n", pConfig->s16Lut3DMax[i], i);
    }
...
{
    MS_U8 u8AllocateCnt = 0;
    MsHdr_RegTable_t* pRegTable = NULL;
    ...
    if (pRegTable->stDegamma.pu32Table == NULL)
    {
        printk("[%s %d]memory allocate error!",__FUNCTION__,__LINE__);
        goto MEMORY_ALLOCATE_FAIL;
    }
    ...
    if (pRegTable->stGamma.pu16Table == NULL)
    {
        printk("[%s %d]memory allocate error!",__FUNCTION__,__LINE__);
        goto MEMORY_ALLOCATE_FAIL;
    }
    ...
    if (pRegTable->stTM.pu16Table == NULL)
    {
        printk("[%s %d]memory allocate error!",__FUNCTION__,__LINE__);
        goto MEMORY_ALLOCATE_FAIL;
    }
    ...
    if (pRegTable->st3D.pu16Table == NULL)
    {
        printk("[%s %d]memory allocate error!",__FUNCTION__,__LINE__);
        goto MEMORY_ALLOCATE_FAIL;
    }
    memset(pRegTable->st3D.pu16Table, 0, sizeof(MS_U16) * DOVI_3DLUT_SIZE*3);
    pRegTable->st3D.pu8BCMSFile = (MS_U8*)MsHdr_Malloc(DOVI_BCMS_DEFAULT_SIZE);
    memset(pRegTable->st3D.pu8BCMSFile, 0, DOVI_BCMS_DEFAULT_SIZE);

    return pRegTable;
MEMORY_ALLOCATE_FAIL:
    switch (u8AllocateCnt)
    {
    case 4:     // st3D.pu16Table fail
        MsHdr_Free(pRegTable->stTM.pu16Table);
    case 3:     // stTM.pu16Table fail
        MsHdr_Free(pRegTable->stGamma.pu16Table);
    case 2:     // stGamma.pu16Table fail
        MsHdr_Free(pRegTable->stDegamma.pu32Table);
    case 1:     // stDegamma.pu32Table fail
        MsHdr_Free(pRegTable);
    default:    // pRegTable fail
    break;
    }
    return NULL;
}
...
void MsHdr_DmFreeRegTable(MsHdr_RegTable_t* pRegTable)
{
    ...
    MsHdr_Free(pRegTable->st3D.pu8BCMSFile);
    MsHdr_Free(pRegTable);
}
...
/// @return
//-------------------------------------------------------------------------------------------------
DoVi_PQ_control_t* DoVi_PQSettingControlAllocConfig(void)
{
    DoVi_PQ_control_t* pConfig;
    pConfig = (DoVi_PQ_control_t*)MsHdr_Malloc(sizeof(DoVi_PQ_control_t));
    memset(pConfig, 0, sizeof(DoVi_PQ_control_t));
    return pConfig;
}
...
/// @return
//-------------------------------------------------------------------------------------------------
void DoVi_PQSettingControlFreeConfig(DoVi_PQ_control_t* pConfig)
{
    MsHdr_Free(pConfig);
}
...
/// @return
//-------------------------------------------------------------------------------------------------
DoVi_TargetDisplayConfig_t* DoVi_PQSettingAllocConfig(void)
{
    DoVi_TargetDisplayConfig_t* pConfig;
    pConfig = (DoVi_TargetDisplayConfig_t*)MsHdr_Malloc(sizeof(DoVi_TargetDisplayConfig_t));
    memset(pConfig, 0, sizeof(DoVi_TargetDisplayConfig_t));
    return pConfig;
}
...
/// @return
//-------------------------------------------------------------------------------------------------
void DoVi_PQSettingFreeConfig(DoVi_TargetDisplayConfig_t* pConfig)
{
    MsHdr_Free(pConfig);
}
...
/// @return
//-------------------------------------------------------------------------------------------------
static int _DoVi_PQSetting_UpdateFromBin(DoVi_TargetDisplayConfig_t* pConfig, const MS_U8* pu8Array, const MS_U32 u32Size)
{
    if (u32Size != DOVI_PQ_CONFIG_BINARY_SIZE)
        return -1;

    memcpy(pConfig, pu8Array, sizeof(DoVi_TargetDisplayConfig_t));
...
int  DoVi_SetDisplayProperty( DoVi_TargetDisplayConfig_t   *pTargetDisplayConfig,  uint16_t   u16BackLightUIVal  )
{

    #define LOG2_UI_STEPSIZE 3

    //int16_t midPQBiasLUT[14] ={ -246,-208,-170,-132,-95,-57,-19, 19,57,95,132,170,208,246}
    int16_t x_x1,x1,y2,y1;
    int32_t temp;

    //memcpy(pTargetDisplayConfig->MidPQBiasLut, midPQBiasLUT, sizeof(int16_t)*14);
    u16BackLightUIVal = DoVi_Clamp(u16BackLightUIVal,0,100);
    ...
    y1 = pTargetDisplayConfig->reserved0Lut[x1];
    y2 = pTargetDisplayConfig->reserved0Lut [x1+1];
    y2 = y2-y1 ; //Y2-Y1
    temp = (y2 * x_x1) >>LOG2_UI_STEPSIZE   ;//(y2-y1)/(x2-x1) * (x-x1)
    pTargetDisplayConfig->MaxPQBias =(int16_t)(temp + y1);
...
int  DoVi_SetPQ2Configs( DoVi_Config_t* pConfig, const DoVi_TargetDisplayConfig_t   *pTargetDisplayConfig)
{
    if (bNewPQBCMS_109727 == TRUE)
    {
        pConfig->u16CcGain = pTargetDisplayConfig->Gain;
        pConfig->u16CcOffset = pTargetDisplayConfig->Offset;

        //not used now
        pConfig->u16Tgamma = pTargetDisplayConfig->Gamma;
        pConfig->u8TEOTF = pTargetDisplayConfig->Eotf;


        //tgtSigEnv.YuvRng = pTargetDisplayConfig->RangeSpec
        pConfig->u8TSignalRange = pTargetDisplayConfig->RangeSpec;
        pConfig->u16Tdiagonalinches = pTargetDisplayConfig->DiagSize;


        pConfig->u16TmaxPQ = pTargetDisplayConfig->MaxPq;
        pConfig->u16TminPQ = pTargetDisplayConfig->MinPq;
        pConfig->s16MsWeight = pTargetDisplayConfig->MSWeight;
        pConfig->u16MsWeightEdge = pTargetDisplayConfig->MSEdgeWeight;

        pConfig->s16TMinPQBias = pTargetDisplayConfig->MinPQBias;
        pConfig->s16TMidPQBias = pTargetDisplayConfig->MidPQBias;
        pConfig->s16TMaxPQBias = pTargetDisplayConfig->MaxPQBias;

        pConfig->s16TrimSlopeBias = pTargetDisplayConfig->trimSlopeBias;
        pConfig->s16TrimOffsetBias = pTargetDisplayConfig->trimOffsetBias;
        pConfig->s16TrimPowerBias = pTargetDisplayConfig->trimPowerBias;

        pConfig->s16MsWeightBias = pTargetDisplayConfig->msWeightBias;

        pConfig->s16TBrightness = pTargetDisplayConfig->Brightness;
        pConfig->s16TContrast = pTargetDisplayConfig->Contrast;

        pConfig->s16ChromaWeightBias = pTargetDisplayConfig->ChromaWeightBias;
        pConfig->s16SaturationGainBias = pTargetDisplayConfig->SaturationGainBias;

        pConfig->u16ChromaWeight = pTargetDisplayConfig->ChromaWeight;
        pConfig->u16SaturationGain = pTargetDisplayConfig->SaturationGain;
        pConfig->u16CrossTalk = pTargetDisplayConfig->CrossTalk;
        pConfig->s32CrossTalk = pTargetDisplayConfig->CrossTalk;
    }
...
int DoVi_Replace_3DLut(MsHdr_3D_RegTable_t* p3DLutReg, const MS_U8* pu8Array, MS_U32 u32Size)
{
    ...
    {
        bNewPQBCMS_109727 = FALSE;
    }
    else if (u32Size == DOVI_PQ_TUNING_FORMAT_SIZE)
    {
        bNewPQBCMS_109727 = TRUE;
    }
    else
    {
        bNewPQBCMS_109727 = FALSE;
    ...
    if (bNewPQBCMS_109727 == TRUE)
    {
        memcpy(p3DLutReg->pu8BCMSFile, pu8Array, DOVI_BCMS_DEFAULT_SIZE);
        _DoVi_PQSetting_UpdateFromBin(  _pstDoVi_TargetDisplayConfig
                                      , (pu8Array + DOVI_PQ_TUNING_FORMAT_SIZE - DOVI_CONFIG_BINARY_SIZE)
                                      , DOVI_CONFIG_BINARY_SIZE);

        DoVi_SetDisplayProperty( _pstDoVi_TargetDisplayConfig,  _pstDoVi_TargetDisplayConfig->BackLightUI );
...
void DoVi_Set_HDRInputType(DoVi_Config_t* pConfig, MS_U8 u8InputType)
{
    pConfig->u8InputType = u8InputType;
}
...
void DoVi_DmSetDefaultConfig(DoVi_Config_t* pConfig)
{
    ...
    pConfig->u8InputType = DOVI_DM_INPUT_TYPE_OTT;
}
}
...
void DoVi_Prepare_B0102(MsHdr_Y2R_RegTable_t* pRegTable, const DoVi_Config_t* pConfig, const DoVi_MdsExt_t* pMdsExt, DoVi_Comp_ExtConfig_t* pComConfig)
{
    ...
    if (DOVI_DM_INPUT_TYPE_OTT == pConfig->u8InputType)
	{
	    pRegTable->u8Shift = 1; // 0:shift 10b, 1:shift 11b, 2:shift 12b, 3:shift 9b
	}
	else if (DOVI_DM_INPUT_TYPE_HDMI == pConfig->u8InputType)
	{
        pRegTable->u8Shift = 3; // 0:shift 10b, 1:shift 11b, 2:shift 12b, 3:shift 9b
	}
	else
	{
        pRegTable->u8Shift = 1; // 0:shift 10b, 1:shift 11b, 2:shift 12b, 3:shift 9b
    ...
    if (DOVI_DM_INPUT_TYPE_OTT == pConfig->u8InputType)
	{
	    s64OffsetY = pMdsExt->YCCtoRGB_offset0 << (pConfig->u8InputBits - pMdsExt->signal_bit_depth);
	    s64OffsetU = pMdsExt->YCCtoRGB_offset1 << (pConfig->u8InputBits - pMdsExt->signal_bit_depth);
	    s64OffsetV = pMdsExt->YCCtoRGB_offset2 << (pConfig->u8InputBits - pMdsExt->signal_bit_depth);
	}
	else if (DOVI_DM_INPUT_TYPE_HDMI == pConfig->u8InputType)
	{
        s64OffsetY = pMdsExt->YCCtoRGB_offset0;
        s64OffsetU = pMdsExt->YCCtoRGB_offset1;
        s64OffsetV = pMdsExt->YCCtoRGB_offset2;
	}
	else
	{
        s64OffsetY = pMdsExt->YCCtoRGB_offset0 << (pConfig->u8InputBits - pMdsExt->signal_bit_depth);
...
void DoVi_Prepare_B04(MsHdr_CC_RegTable_t* pRegTable, const DoVi_Config_t* pConfig, const DoVi_MdsExt_t* pMdsExt)
{
    ...
    pRegTable->u16HuntGain = pConfig->u16CcGain;

    //pRegTable->u16HuntOffset = 0xFFF;
    pRegTable->u16HuntOffset = pConfig->u16CcOffset;
...
int DoVi_3DLUT_parsing_reorder(GmLut_t* pBCMS_Header, MS_U16* pu16Table, const MS_U8* pu8BCMSFile)
{
    ...
    temp = (pu8BCMSFile[0]-'0')*1000+(pu8BCMSFile[1]-'0')*100+(pu8BCMSFile[2]-'0')*10+(pu8BCMSFile[3]-'0');
    pBCMS_Header->Version = temp;

    if (pBCMS_Header->Version < 64)
    {
        printk("invalide lut file: version not supported.\n");
        return -1;
    }

    pBCMS_Header->LittleEndian = (pu8BCMSFile[4] == 'L' || pu8BCMSFile[4] =='l');

    if (pBCMS_Header->LittleEndian == 0)
    {
        printk("invalide lut file: only little endia gmlut is supported\n");
        return -1;
    }
    ...
    if ( (cBuf[0] == 'r' || cBuf[0] == 'R') &&
        ...
        (cBuf[2] == 'b' || cBuf[2] == 'B') )
    {
        pBCMS_Header->Type = GmLutTypeIpt2Rgb;
    }
    else if ( (cBuf[0] == 'y' || cBuf[0] == 'Y') &&
		(cBuf[1] == 'u' || cBuf[1] == 'U') &&
		(cBuf[2] == 'v' || cBuf[2] == 'V') )
    {
        pBCMS_Header->Type = GmLutTypeIpt2Yuv;
    }
    else
    ...
    cBuf = &(pu8BCMSFile[13]);
    pBCMS_Header->DimC1 = (MS_S16)( (pu8BCMSFile[14]<<8) | pu8BCMSFile[13] );
    pBCMS_Header->DimC2 = (MS_S16)( (pu8BCMSFile[16]<<8) | pu8BCMSFile[15] );
    pBCMS_Header->DimC3 = (MS_S16)( (pu8BCMSFile[18]<<8) | pu8BCMSFile[17] );

    pBCMS_Header->IdxMinC1 = (MS_S16)( (pu8BCMSFile[20]<<8) | pu8BCMSFile[19] );
    pBCMS_Header->IdxMaxC1 = (MS_S16)( (pu8BCMSFile[22]<<8) | pu8BCMSFile[21] );
    pBCMS_Header->IdxMinC2 = (MS_S16)( (pu8BCMSFile[24]<<8) | pu8BCMSFile[23] );
    pBCMS_Header->IdxMaxC2 = (MS_S16)( (pu8BCMSFile[26]<<8) | pu8BCMSFile[25] );
    pBCMS_Header->IdxMinC3 = (MS_S16)( (pu8BCMSFile[28]<<8) | pu8BCMSFile[27] );
    pBCMS_Header->IdxMaxC3 = (MS_S16)( (pu8BCMSFile[30]<<8) | pu8BCMSFile[29] );


    DoVi_Printf("pBCMS_Header->Version(%d)\n",__FUNCTION__,__LINE__,pBCMS_Header->Version);
    DoVi_Printf("pBCMS_Header->LittleEndian(%d)\n",__FUNCTION__,__LINE__,pBCMS_Header->LittleEndian);
    DoVi_Printf("pBCMS_Header->Type(%d)\n",__FUNCTION__,__LINE__,pBCMS_Header->Type);
    DoVi_Printf("pBCMS_Header->DimC1(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->DimC1);
    DoVi_Printf("pBCMS_Header->DimC2(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->DimC2);
    DoVi_Printf("pBCMS_Header->DimC3(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->DimC3);
    DoVi_Printf("pBCMS_Header->IdxMinC1(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->IdxMinC1);
    DoVi_Printf("pBCMS_Header->IdxMaxC1(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->IdxMaxC1);
    DoVi_Printf("pBCMS_Header->IdxMinC2(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->IdxMinC2);
    DoVi_Printf("pBCMS_Header->IdxMaxC2(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->IdxMaxC2);
    DoVi_Printf("pBCMS_Header->IdxMinC3(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->IdxMinC3);
    DoVi_Printf("pBCMS_Header->IdxMaxC3(0x%x)\n",__FUNCTION__,__LINE__,pBCMS_Header->IdxMaxC3);

    pu16buffer = &(pu8BCMSFile[31]);

    //process_sram0(&mstar_3dlut[count], &dolby_3dlut[0]); count += MSTAR_SRAM0_SIZE*3;
    process_sram0(&(pu16Table[count]), pu16buffer);  count += DOVI_3DLUT_S0_SIZE*3;
    process_sram1(&(pu16Table[count]), pu16buffer);  count += DOVI_3DLUT_S1_SIZE*3;
    process_sram2(&(pu16Table[count]), pu16buffer);  count += DOVI_3DLUT_S2_SIZE*3;
    process_sram3(&(pu16Table[count]), pu16buffer);  count += DOVI_3DLUT_S3_SIZE*3;
    process_sram4(&(pu16Table[count]), pu16buffer);  count += DOVI_3DLUT_S4_SIZE*3;
    process_sram5(&(pu16Table[count]), pu16buffer);  count += DOVI_3DLUT_S5_SIZE*3;
    process_sram6(&(pu16Table[count]), pu16buffer);  count += DOVI_3DLUT_S6_SIZE*3;
    process_sram7(&(pu16Table[count]), pu16buffer);  count += DOVI_3DLUT_S7_SIZE*3;


#if 0 // this is for debugging purpose.
    //dump interior variables
    //check with golden pattern

    for (temp =0 ; temp <14382 ; temp++)
    {
		if (pu16Table[temp] != test_3d_lut[temp])
		{
			printk("-----3D LUT diffs !!! ----\n");
			printk("pu16Table[temp]  = %x\n",temp,pu16Table[temp]);
			printk("test_3d_lut[temp]  = %x\n",temp,test_3d_lut[temp]);
			flag = 1;
			break;
		}
    }
    ...
    if (0 == flag)
    {
		printk("-----3D LUT test pass !!! ----\n");
    }
    else
    {
		printk("-----3D LUT test fail !!! ----\n");
    }
#endif
...
/// @return
//-------------------------------------------------------------------------------------------------
void DoVi_Prepare_B05(MsHdr_3D_RegTable_t* pRegTable, DoVi_Config_t* pConfig, const DoVi_MdsExt_t* pMdsExt)
{
    ...
    if (bNewPQBCMS_109727 == TRUE)
    {
        GmLut_t stBCMS_Header;
        memset(&stBCMS_Header,0,sizeof(GmLut_t));
        //parsing & update
        //@param : input buffer of dolby
        //@param : output buffer for Mstar 3D lut SRAM
        //@param : structure of BCMS file header
        s32temp = DoVi_3DLUT_parsing_reorder(&stBCMS_Header, pRegTable->pu16Table, pRegTable->pu8BCMSFile);
        pConfig->s16Lut3DMin[0] = stBCMS_Header.IdxMinC1;
        pConfig->s16Lut3DMax[0] = stBCMS_Header.IdxMaxC1;
        pConfig->s16Lut3DMin[1] = stBCMS_Header.IdxMinC2;
        pConfig->s16Lut3DMax[1] = stBCMS_Header.IdxMaxC2;
        pConfig->s16Lut3DMin[2] = stBCMS_Header.IdxMinC3;
        pConfig->s16Lut3DMax[2] = stBCMS_Header.IdxMaxC3;
    }
...
void MsHdr_Update_3D(const MsHdr_3D_RegTable_t* pRegTable)
{
    ...
    {
        #if 1
        MHal_XC_W2BYTE((0x130000 | DoVi_Addr(REG_SC_BK7A_50_L)), ((MS_U16)pRegTable->s16MaxC & 0xFFFF));
        MHal_XC_W2BYTE((0x130000 | DoVi_Addr(REG_SC_BK7A_51_L)), ((MS_U16)pRegTable->s16MinC & 0xFFFF));
        MHal_XC_W2BYTE((0x130000 | DoVi_Addr(REG_SC_BK7A_52_L)), ((pRegTable->u32InvC & 0xFFFF) & 0xFFFF));
        MHal_XC_W2BYTE((0x130000 | DoVi_Addr(REG_SC_BK7A_53_L)), ((pRegTable->u32InvC >>16) & 0xFFFF));
        MHal_XC_W2BYTE((0x130000 | DoVi_Addr(REG_SC_BK7A_54_L)), ((MS_U16)pRegTable->s16MinY & 0xFFFF));
        MHal_XC_W2BYTE((0x130000 | DoVi_Addr(REG_SC_BK7A_55_L)), ((MS_U16)pRegTable->s16MaxY & 0xFFFF));
        MHal_XC_W2BYTE((0x130000 | DoVi_Addr(REG_SC_BK7A_56_L)), ((pRegTable->u32InvY & 0xFFFF) & 0xFFFF));
        MHal_XC_W2BYTE((0x130000 | DoVi_Addr(REG_SC_BK7A_57_L)), ((pRegTable->u32InvY >>16) & 0xFFFF));
        #else
...
#define SKIP_AND_EXIT_ON_FALSE_RETURN(len) \
    ...
    pMdsExt->Trima[TrimTypeMsWeight][0] = pConfig->s16MsWeight;
...
void DoVi_DmBlankingUpdate(const MsHdr_RegTable_t* pRegTable)
{
    ...
    if (   _bNeedDownload3DLut
        || _bNeedDownloadGamma
        || _bNeedDownloadDegamma
        || _bNeedDownloadTMO)
    {
            MsHdr_Update_Autodownload(pRegTable);
            _bNeedDownload3DLut = FALSE;
            _bNeedDownloadGamma = FALSE;
            _bNeedDownloadDegamma = FALSE;
            _bNeedDownloadTMO = FALSE;
    }
    else
    {
        if (!(CFD_IS_MM(_stCfdInit[0].u8InputSource) && (_u8Version == DOLBY_DS_VERSION)))
...
void DoVi_ResetDownloadProcedure(void)
{
    _bNeedDownloadDegamma = TRUE;
    _bNeedDownloadGamma   = TRUE;
    _bNeedDownload3DLut   = TRUE;
    _bNeedDownloadTMO     = TRUE;
}
