...
void nand_Prepare_Power_Saving_Mode_Queue(void)
{
    ...
    /* (1) Clear HW Enable */
	REG_WRITE_UINT16(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE, 0x00), 0x0020);
    ...
    REG_SET_BITS_UINT16(NC_PWRSAVE_CTL, BIT_POWER_SAVE_MODE|BIT_POWER_SAVE_MODE_INT_EN);
}
#endif
...
irqreturn_t NC_FCIE_IRQ(int irq, void *dummy)
{
    ...
    REG_READ_UINT16(NC_MIE_EVENT, u16_Events);

    if(u16_Events & BIT_NC_POWER_SAVE)
    {
        REG_CLR_BITS_UINT16(NC_PWRSAVE_CTL, BIT_POWER_SAVE_MODE_INT_EN);
