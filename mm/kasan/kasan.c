 ...
 */
static inline void __shadow_memset(const void *addr, size_t size, u8 value)
{
	void *shadow_start = kasan_mem_to_shadow(addr);
	void *shadow_end = kasan_mem_to_shadow(addr + size);
...
static void kasan_poison_shadow(const void *address, size_t size, u8 value)
{
	if (IS_ENABLED(CONFIG_ARM) &&
	    unlikely(address < (void *)MODULES_VADDR ||
	             address >= (void *)VMALLOC_START))
		return;

	__shadow_memset(address, size, value);
}
...
void kasan_unpoison_shadow(const void *address, size_t size)
{
	...
	__shadow_memset(address, size, 0);
						...
						size_t size, bool write)
{
	...
	if (IS_ENABLED(CONFIG_ARM) &&
	    unlikely(addr < MODULES_VADDR || addr >= VMALLOC_START))
		return;
