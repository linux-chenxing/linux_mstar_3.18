...
static void Mstar_U3phy_MS28_init(uintptr_t U3PHY_D_base, uintptr_t U3PHY_A_base)
{
	...
	#ifdef XHCI_2PORTS
	if (U3PHY_D_base == _MSTAR_U3PHY_P0_DTOP_BASE)
		writeb(0xD,	(void*)(U3PHY_D_base+0x1E*2));	// P0 and P1 separated
	#endif
#endif
	...
	unsigned int utmi_flag)
{
	...
	//uintptr_t U3BC_base = u3phy_addr->bc_base;

	printk("Mstar_xhc_init version:%s\n", XHCI_MSTAR_VERSION);

	// Init USB2 UTMI
	Mstar_U2utmi_init(UTMI_base, U3TOP_base, utmi_flag);
	...
	#ifndef CONFIG_MSTAR_XUSB_PCIE_PLATFORM
	Mstar_U3phy_MS28_init(u3phy_addr->u3dtop1_base, u3phy_addr->u3atop1_base);
	#endif
		#if defined(XHCI_PHY_EFUSE)
	Mstar_U3phy_MS28_eFuse_2p(u3phy_addr->u3atop_base, u3phy_addr->u3atop1_base);
		#endif
	#else
	Mstar_U3phy_MS28_init(u3phy_addr->u3dtop_base, u3phy_addr->u3atop_base);
	#endif
#else
	Mstar_U3phy_init(u3phy_addr->u3dtop_base, u3phy_addr->u3atop_base);
#endif
...
static int xhci_mstar_plat_probe(struct platform_device *pdev)
{
	...
	printk("xhci_mstar_plat_probe:%s\n", pdev->name);
	if (usb_disabled())
		return -ENODEV;
	...
	if( 0==strncmp(pdev->name, "soc:Mstar-xhci-1", strlen("soc:Mstar-xhci-1")) )
	{
	...
	else if( 0==strncmp(pdev->name, "soc:Mstar-xhci-2", strlen("soc:Mstar-xhci-2")) )
	{
	...
	#define NAME_LEN (16)
	if( 0==strncmp(pdev->name, "soc:Mstar-xhci-1", NAME_LEN) ) {
	...
	else if( 0==strncmp(pdev->name, "soc:Mstar-xhci-2", NAME_LEN) ) {
		#if defined(XHCI_ENABLE_DPDM_SWAP)
		flag |= EHCFLAG_DPDM_SWAP; /* extensible */
		#endif
...
static struct of_device_id mstar_xhci_of_device_ids[] = {
	{.compatible = "soc:Mstar-xhci-1"},
	{},
};
#ifdef ENABLE_SECOND_XHC
static struct of_device_id mstar_xhci_1_of_device_ids[] = {
	{.compatible = "soc:Mstar-xhci-2"},
	{},
};
#endif
#endif
...
int xhci_register_plat(void)
{
	...
	if (retval < 0)
		return retval;
	return platform_driver_register(&second_xhci_mstar_driver);
#else
