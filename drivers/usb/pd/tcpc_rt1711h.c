...
extern int ms_i2c_xfer(struct i2c_adapter *padap, struct i2c_msg *pmsg, int num);

#define I2C_BUS_NR          0
#define RT1716_SLAVE_ADDR   0x9C
...
usb_tcp_plug_notifier_fn_t usb_tcp_plug_notifier = NULL;
usb_tcp_unplug_notifier_fn_t usb_tcp_unplug_notifier = NULL;
usb_tcp_vbus_notifier_fn_t usb_tcp_vbus_notifier = NULL;

EXPORT_SYMBOL(usb_tcp_plug_notifier);
EXPORT_SYMBOL(usb_tcp_unplug_notifier);
EXPORT_SYMBOL(usb_tcp_vbus_notifier);

static int ms_rt1716_i2c_write8(u8 reg, u8 val)
{
    struct i2c_adapter adpa;
    struct i2c_msg msg;
    ...
    msg.addr = RT1716_SLAVE_ADDR >> 1;
    msg.flags = 0;
    msg.buf = data;
    msg.len = 2;

    if(ms_i2c_xfer(&adpa, &msg, 1) < 0)
    {
        printk("i2c-%d timeout\n", I2C_BUS_NR);
        return -1;
    }
...
static int ms_rt1716_i2c_write16(u8 reg, u16 val)
{
    struct i2c_adapter adpa;
    struct i2c_msg msg;
...
static int ms_rt1716_i2c_read8(u8 reg, u8 *val)
{
    struct i2c_adapter adpa;
    struct i2c_msg msg;
    unsigned char data;
...
static int ms_rt1716_i2c_read16(u8 reg, u16 *val)
{
    struct i2c_adapter adpa;
    struct i2c_msg msg;
    unsigned char data;
...
#else
	ret = ms_rt1716_i2c_read8(reg, &val);
#endif /* CONFIG_RT_REGMAP */
	if (ret < 0) {
...
#else
	ret = ms_rt1716_i2c_write8(reg, data);
#endif /* CONFIG_RT_REGMAP */
	if (ret < 0)
					...
					uint8_t reg_addr, uint16_t data)
{
	...
	/* don't need swap */
	ret = ms_rt1716_i2c_write16(reg_addr, data);
					...
					uint8_t reg_addr, uint16_t *data)
{
	...
	/* don't need swap */
	ret = ms_rt1716_i2c_read16(reg_addr, data);
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	pr_info("%s\n", __func__);
	of_property_read_u32(np, "rt1716,irq_pin", &irq_pin);
	chip->irq_gpio = irq_pin;
...
static int tcp_notifer_call(struct notifier_block *nb,
                                unsigned long event, void *data)
{
    ...
    switch (event) {
        case TCP_NOTIFY_TYPEC_STATE:
            if (tcp_noti->typec_state.new_state == TYPEC_ATTACHED_SNK) {
                if (usb_tcp_plug_notifier)
                {
                    usb_tcp_plug_notifier(tcp_noti->typec_state.polarity);
                }
            } else if ((tcp_noti->typec_state.old_state == TYPEC_ATTACHED_SNK) &&
                       tcp_noti->typec_state.new_state == TYPEC_UNATTACHED) {
                if (usb_tcp_unplug_notifier)
                {
                    usb_tcp_unplug_notifier();
                }
            }
            break;
        case TCP_NOTIFY_SINK_VBUS:
            if (usb_tcp_vbus_notifier)
            {
                usb_tcp_vbus_notifier(tcp_noti->vbus_state.mv, tcp_noti->vbus_state.ma);
            }
            break;
        default:
            break;
...
static inline int rt1711h_check_revision(struct i2c_client *client)
{
	...
	ret = ms_rt1716_i2c_read16(TCPC_V10_REG_VID, &vid);
	if (ret < 0) {
	...
	ret = ms_rt1716_i2c_read16(TCPC_V10_REG_PID, &pid);
	if (ret < 0) {
	...
	ret = ms_rt1716_i2c_write8(RT1711H_REG_SWRESET, data);
	if (ret < 0)
		return ret;

	usleep_range(1000, 2000);

	ret = ms_rt1716_i2c_read16(TCPC_V10_REG_DID, &did);
	if (ret < 0) {
				...
				const struct i2c_device_id *id)
{
    ...
    volatile unsigned int *ptr_addr;

	pr_info("%s\n", __func__);

    //Pad switch
    ptr_addr = (volatile unsigned int *)0xFD204C00;
    *ptr_addr = 0x0;
    ptr_addr = (volatile unsigned int *)0xFD204C18;
    *ptr_addr = 0x1;
    ptr_addr = (volatile unsigned int *)0xFD2066C0;
    *ptr_addr = 0x444;
	...
	nb = devm_kzalloc(&client->dev, sizeof(struct notifier_block), GFP_KERNEL);
	if (!nb)
		return -ENOMEM;
	...
	nb->notifier_call = tcp_notifer_call;
	ret = register_tcp_dev_notifier(chip->tcpc, nb);
	if (ret < 0) {
		pr_err("rt1711 init tcp notifier call fail\n");
		goto err_irq_init;
	}
...
static int __init rt1711_init(void)
{
	...
	np = of_find_node_by_name(NULL, "rt1716");
	if (np != NULL)
		pr_info("rt1716 node found...\n");
	else
		pr_info("rt1716 node not found...\n");
