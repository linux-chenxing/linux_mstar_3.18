...
#define INT_MS_OTG (69)//INT_IRQ_OTG
#endif
        ...
        struct msb250x_request *req)
{
    ...
    if (is_last)
    {
        ...
        else
        {
			...
			{
				ep_csr = udc_read8(MSB250X_USBCREG(0x142));
				udc_write8(ep_csr|MSB250X_UDC_TXCSR1_TXPKTRDY,MSB250X_USBCREG(0x142));
			}
        }
#ifndef TX_modify
                 ...
                 struct msb250x_request *req)
{
    ...
    **/
    ...
    if (is_last)
    {
		...
		if(use_dma && ep->dev->DmaRxMode == DMA_RX_MODE1)
...
if((ep->bEndpointAddress)!=0)
{
    ...
    if(!ep->halted)
    {
...
#ifdef CONFIG_USB_MSB250X_DMA
        else if (use_dma && (ep->bEndpointAddress & USB_DIR_IN))
        {
            //putb("do_req_TX",0,0);
        /* DMA TX: */
        ...
        else if (use_dma && !(ep->bEndpointAddress & USB_DIR_IN) && (ep->dev->DmaRxMode != DMA_RX_MODE_NULL))
        {
#ifdef RX_mode1_log
            printk("[USB]DMA_RX\n");
#endif
        ...
        struct msb250x_request *req, int status)
{
    ...
    if (req)
    {
        ...
        if (idx != 0)
        {
            ...
            if (check_dma_busy() != DMA_BUSY)
            {
                if (use_dma && (req->req.length > (ep->ep.maxpacket)))
                {
            ...
            else
            {
                ...
                if (is_in)
                {
                    ...
                    if(ep_num==1)
						...
						status = udc_read8(MSB250X_USBCREG(0x142));
...
static void msb250x_udc_handle_ep(struct msb250x_ep *ep)
{
    ...
    { 
		...
		if(idx==1)
			...
			ep_csr1 = udc_read8(MSB250X_USBCREG(0x142));
        ...
        if (ep_csr1 & MSB250X_UDC_TXCSR1_SENTSTALL)
        {
            ...
            if(idx==1)
				...
				udc_write8(ep_csr1 & ~MSB250X_UDC_TXCSR1_SENTSTALL, MSB250X_USBCREG(0x142));
	...
	//auto nak	, not allow any data out ACK
	   //printk("[USB handle_ep]MSB250X_UDC_DMA_MODE_CTL:%x\n",udc_read16(MSB250X_UDC_DMA_MODE_CTL));   	
        if(((ep->bEndpointAddress&0x0f)==2) && !(ep->bmAttributes & USB_ENDPOINT_XFER_ISOC)) {
                 ...
                 const struct usb_endpoint_descriptor *desc)
{
    ...
    if (desc->bEndpointAddress & USB_DIR_IN)
    {
		...
		{
			udc_write16(_ep->maxpacket , MSB250X_USBCREG(0x140));
			udc_write8(csr1, MSB250X_USBCREG(0x142));
			udc_write8(csr2, (MSB250X_USBCREG(0x142))+1);
        }
        ...
        if ((ep->bEndpointAddress & USB_DIR_IN) != 0)
        {
            if (value)
            {
                ...
                if(idx==1)
					...
					udc_write8(ep_csr | MSB250X_UDC_TXCSR1_SENDSTALL,MSB250X_USBCREG(0x142));
            }
            else
            {
                ...
                {
                    udc_write8(MSB250X_UDC_TXCSR1_CLRDATAOTG, MSB250X_USBCREG(0x142));
                    udc_write8(0,MSB250X_USBCREG(0x142));
                }
    ...
    //USBInit(E_Class_Serial, HIGH_SPEED);    
{
    ...
    //switch pad mux to otg20   
    writeb(readb((void*)(0xfd000000+(0x101e00+0xD2)*2)) | 0x01, (void*)(0xfd000000+(0x101e00+0xD2)*2));
