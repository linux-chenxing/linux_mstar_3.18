...
s8 USB_Set_DMA(struct usb_ep *_ep, struct msb250x_request *req, u32 count, u32 mode)
{
   ...
   //else
	...
	address = BUS2PA(virt_to_phys((void *)address));
    ...
    switch(mode & DMA_MODE_MASK)
    {
        ...
        case DMA_TX | DMA_MODE_ONE:
            ...
            {
                csr2 = udc_read8((MSB250X_USBCREG(0x142))+1);
                udc_write8((csr2 | TXCSR2_MODE1), (MSB250X_USBCREG(0x142))+1);
            }
#if 0
if(count>=200)
{
...
void USB_DMA_IRQ_Handler(u8 ch, struct msb250x_udc *dev)
{
    ...
    if (mode & DMA_TX)
    {
		...
		if((ep->bEndpointAddress & 0x7F)==1)
			...
			csr2_tmp=udc_read8(MSB250X_USBCREG(0x142));
		else
			printk("ERROR\n");
