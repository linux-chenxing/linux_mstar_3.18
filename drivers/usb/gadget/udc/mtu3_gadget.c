...
__acquires(mep->mtu->lock)
{
	...
	mreq->request.dma = PA2BUS(mreq->request.dma); // MMM
		...
		struct usb_request *req, gfp_t gfp_flags)
{
	...
	mreq->request.dma = BUS2PA(mreq->request.dma); // MMM
		...
		u32 epnum, u32 is_in)
{
	...
	if (!epnum) {
		...
		//mep->ep.caps.type_control = true; // MMM
		mep->ep.ops = &mtu3_ep0_ops;
		mtu->g.ep0 = &mep->ep;
	} else {
		usb_ep_set_maxpacket_limit(&mep->ep, 1024);
		//mep->ep.caps.type_iso = true; // MMM
		//mep->ep.caps.type_bulk = true; // MMM
		//mep->ep.caps.type_int = true; // MMM
