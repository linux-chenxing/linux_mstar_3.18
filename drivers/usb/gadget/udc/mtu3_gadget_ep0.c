...
{
#ifdef MSTAR_ENABLE_U3DEV_U1_U2 // MMM
	void __iomem *mbase = mtu->mac_base;
	u32 lpc;
#endif
	int handled = -EINVAL;
...
//#include <linux/usb/composite.h> // MMM
#define USB_GADGET_DELAYED_STATUS 0x7fff // MMM
...
finish:
	if (mtu->test_mode) {
		...
		/* MMM, gadget delayed status support */
		#ifdef MMM_DELAY_STATUS_SUPPORT
		if (handled == USB_GADGET_DELAYED_STATUS)
			return 0;
		#endif
...
static int ep0_queue(struct mtu3_ep *mep, struct mtu3_request *mreq)
{
	...
	/* MMM, gadget status delayed support */
	#ifdef MMM_DELAY_STATUS_SUPPORT
	if (mreq->request.length== 0) {
		void __iomem *mbase = mtu->mac_base;

		mtu3_writel(mbase, U3D_EP0CSR,
			(mtu3_readl(mbase, U3D_EP0CSR) & EP0_W1C_BITS)
			| EP0_SETUPPKTRDY | EP0_DATAEND);
		printk("[USB] %s, delay status...\n", __func__);
	}
	#endif
