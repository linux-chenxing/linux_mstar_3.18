...
static void mtu3_ss_func_set(struct mtu3 *mtu, bool enable)
{
	...
	if (enable)
	{
		...
		/* MMM, auto switch U3 to U2 */
		mtu3_setbits(mtu->mac_base, U3D_USB2_TEST_MODE, BIT(10));
	}
	else
	{
		mtu3_clrbits(mtu->mac_base, U3D_USB3_CONFIG, USB3_EN);
		/* MMM, auto switch U3 to U2 */
		mtu3_clrbits(mtu->mac_base, U3D_USB2_TEST_MODE, BIT(10));
	}
...
static int mtu3_device_enable(struct mtu3 *mtu)
{
	...
	cap_dev = mtu3_readl(mtu->ippc_base, U3D_SSUSB_IP_DEV_CAP); // MMM
	mtu->is_u3_ip = !!SSUSB_IP_DEV_U3_PORT_NUM(cap_dev); // MMM
	if (mtu->is_u3_ip) {
		...
		/* MMM, correct timer counter base */
		regt_cnt = mtu3_readl(mbase, U3D_LINK_TIMER_COUNT);
		regt_cnt &= ~0xff;
		regt_cnt |= 0xf1; // from 0x7d to 0xf1
		mtu3_writel(mbase, U3D_LINK_TIMER_COUNT, regt_cnt);
	}
	mtu3_clrbits(ibase, SSUSB_U2_CTRL(0),
		(SSUSB_U2_PORT_DIS | SSUSB_U2_PORT_PDN |
		SSUSB_U2_PORT_HOST_SEL));

	/* MStar IP without OTG design */
...
static void mtu3_intr_enable(struct mtu3 *mtu)
{
	...
	if (mtu->is_u3_ip) {
		...
		value = HOT_RST_INTR | WARM_RST_INTR | VBUS_RISE_INTR |
		    ...
		    VBUS_FALL_INTR | ENTER_U3_INTR | EXIT_U3_INTR | U3_RESUME_INTR; // MMM
...
static void mtu3_regs_init(struct mtu3 *mtu)
{
	...
	/* vbus detected by HW */
	//mtu3_clrbits(mbase, U3D_MISC_CTRL, VBUS_FRC_EN | VBUS_ON); // MMM, Mstar design
}
...
static irqreturn_t mtu3_u3_ltssm_isr(struct mtu3 *mtu)
{
	...
	/* MMM, MStar resume ACPU setting */
	if (ltssm & U3_RESUME_INTR) {
		uintptr_t U3PHY_A_base = _MSTAR_U3PHY_ATOP0_BASE; // MMM

		dev_dbg(mtu->dev, "MStar resume ACPU setting\n");
		//mtu3_clrbits(ssusb->ippc_base, U3D_SSUSB_IP_PW_CTRL0, BIT(24)); // 51
		mtu3_clrbits(mtu->ippc_base, U3D_SSUSB_IP_PW_CTRL2, SSUSB_IP_DEV_PDN);
		mtu3_setbits(mtu->mac_base, U3D_LINK_POWER_CONTROL, UX_EXIT);
		writeb(readb((void*)(U3PHY_A_base+0x4A*2)) | BIT1, (void*)(U3PHY_A_base+0x4A*2));
	}
	...
	if (ltssm & ENTER_U3_INTR) {
		...
		/* MMM, MStar suspend ACPU setting */
		dev_dbg(mtu->dev, "MStar suspend ACPU setting\n");
		//mtu3_setbits(ssusb->ippc_base, U3D_SSUSB_IP_PW_CTRL0, BIT(24)); // 51
		mtu3_setbits(mtu->ippc_base, U3D_SSUSB_IP_PW_CTRL2, SSUSB_IP_DEV_PDN);
		writeb(readb((void*)(U3PHY_A_base+0x4A*2)) & ~BIT1, (void*)(U3PHY_A_base+0x4A*2));
	}
