...
static int msb250x_udc_read_fifo_ctl_req(struct usb_ctrlrequest *crq)
{
	...
	if (bytes_read) {
		u8 *buffer = outbuf;
		do {
			u8 x = __raw_readb((void *)fifo);
			//printk("x:%p\n",x);
					*buffer++ = x;
		} while (--bytes_read);
	}
	//insb((void *)/*IO_ADDRESS*/(fifo), outbuf, bytes_read);
#else
...
{
    size_t len = min(req->req.length - req->req.actual, max), org_len;
	...
	if (len) {
		...
		do {
			__raw_writeb(*buffer++, (void *)/*IO_ADDRESS*/(fifo));
		} while (--len);
	}
		//outsb((void *)/*IO_ADDRESS*/(fifo), buf, len);
        ...
        struct msb250x_request *req, unsigned avail)
{
	...
	do {
			u8 x = __raw_readb((void *)/*IO_ADDRESS*/(fifo));
					*buffer++ = x;
		   } while (--len);
	}
	//insb((void *)/*IO_ADDRESS*/(fifo), buf, len);
#else
...
void otg_usb_bc_enable(uintptr_t utmi_base, uintptr_t bc_base, u8 enable)
{
	if (enable) {
		//printf("BC enable \n");
...
PUSB_INFO_st USBInit(u8 u8DeviceClass, u8 u8DeviceCap)
{
	...
	// let OTG driver to handle the switch control
#else
	...
	// sync code from eCos
	{
	size_t UTMI_base=UTMI_BASE_ADDR;		
	u16 reg_t;
	// sync code from eCos		
...
#ifdef ENABLE_SQUELCH_LEVEL
	/* Init UTMI squelch level setting befor CA */
	if(UTMI_squelch_LEVEL_2A & (BIT3|BIT2|BIT1|BIT0))
	{
		size_t UTMI_base=UTMI_BASE_ADDR;
		writeb((UTMI_squelch_LEVEL_2A & (BIT3|BIT2|BIT1|BIT0)), (void*)(UTMI_base+0x2a*2));
...
static int msb250x_udc_probe(struct platform_device *pdev)
{
    ...
    retval = request_irq(irq, msb250x_udc_irq,
