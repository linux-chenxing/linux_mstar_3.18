...
uvc_function_ep0_complete(struct usb_ep *ep, struct usb_request *req)
{
    ...
    if (uvc->event_setup_out) {
        video->imagesize = ctrl->dwMaxVideoFrameSize;
        video->max_payload_size = ctrl->dwMaxPayloadTransferSize;
...
uvc_copy_descriptors(struct uvc_device *uvc, enum usb_device_speed speed)
{
    ...
    struct f_uvc_opts *opts;
    opts = to_f_uvc_opts(uvc->func.fi);
...
uvc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    if (!opts->bulk_streaming_ep) {
        ...
        uvc_hs_bulk_streaming_ep.wMaxPacketSize =
               min(opts->streaming_maxpacket, 1024U);
