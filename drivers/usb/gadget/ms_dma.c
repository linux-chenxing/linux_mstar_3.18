...
//u8 DmaRxMode1=0;
//u8 DmaTxMode1=0;
static void Set32BitsReg(u32 volatile* Reg,uintptr_t val)
{
    udc_write16((uintptr_t)(val & 0xffff), Reg);
    udc_write16((uintptr_t)(val>>16), (Reg+1));
...
{
     return (udc_read16(Reg)&0xffff)|(udc_read16((Reg+1))<<16);
}
...
s8 USB_Set_DMA(struct usb_ep *_ep, struct msb250x_request *req, u32 count, u32 mode)
{
   ...
   //else
	...
	size_ttt=count;
//printk("address:%p\n",address);	
//printk("address_virt2phys:%p\n",(virt_to_phys((void *)address)));
//printk("address_pa:%p\n",__pa(address));	
//printk("address_BUS2PA:%p\n",(BUS2PA((virt_to_phys((void *)address)))));
//printk("count:%x\n",count);
...
//printk("new_addr:%p\n",new_addr);
//printk("ch:%p\n",ch);
	Set32BitsReg((uintptr_t volatile*)DMA_ADDR_REGISTER(ch), new_addr);
    Set32BitsReg((uintptr_t volatile*)DMA_COUNT_REGISTER(ch), count);
...
void USB_DMA_IRQ_Handler(u8 ch, struct msb250x_udc *dev)
{
    ...
    addr = Get32BitsReg((uintptr_t volatile*)DMA_ADDR_REGISTER(ch));
    bytesleft =Get32BitsReg((uintptr_t volatile*)DMA_COUNT_REGISTER(ch));
