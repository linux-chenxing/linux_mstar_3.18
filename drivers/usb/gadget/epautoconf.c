...
)
{
    ...
    case USB_ENDPOINT_XFER_ISOC:
        ...
        break;
    /* BULK:  limit 64 bytes full speed, 512 high speed 1024 super speed */
    case USB_ENDPOINT_XFER_BULK:
        ep->maxpacket_limit=1024;
        if(ep->maxpacket_limit < max)
            return 0;
    ...
    if (!gadget_is_dualspeed(gadget) && (USB_ENDPOINT_XFER_BULK == type) && !ep_comp) {
        ...
        if ((USB_ENDPOINT_XFER_BULK == type) ||
            ...
            if (USB_DIR_IN & desc->bEndpointAddress)
            {
                if( USB_ENDPOINT_XFER_BULK == type)
                    ...
                    ep = find_ep (gadget, "ep4in-isoc");
                if (ep && ep_matches (gadget, ep, desc,ep_comp))
			    {
                    printk("[USB]usb_ep_autoconfig ep-in = %s\n",ep->name);
                    return ep;
                }
                else{
                    printk("EP IN ERROR\n");
