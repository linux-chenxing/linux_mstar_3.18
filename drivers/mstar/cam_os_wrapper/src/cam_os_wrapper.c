...
u64 CamOsTimeDiff(CamOsTimespec_t *ptStart, CamOsTimespec_t *ptEnd, CamOsTimeDiffUnit_e eUnit)
{
    if (ptStart && ptEnd)
    {
        ...
        case CAM_OS_TIME_DIFF_SEC:
            return (u64)ptEnd->nSec - ptStart->nSec;
        case CAM_OS_TIME_DIFF_MS:
            return (u64)(ptEnd->nSec - ptStart->nSec)*1000 + (u64)(ptEnd->nNanoSec - ptStart->nNanoSec)/1000000;
        case CAM_OS_TIME_DIFF_US:
            return (u64)(ptEnd->nSec - ptStart->nSec)*1000000 + (u64)(ptEnd->nNanoSec - ptStart->nNanoSec)/1000;
        case CAM_OS_TIME_DIFF_NS:
            return (u64)(ptEnd->nSec - ptStart->nSec)*1000000000 + ptEnd->nNanoSec - ptStart->nNanoSec;
        default:
            return 0;
        }
    }
...
CamOsRet_e CamOsThreadStop(CamOsThread tThread)
{
    ...
    if(ptTaskHandle)
    {
        ...
        CAM_OS_CLEAR_BIT(ptTaskHandle->eHandleObj, aThreadStopBitmap);
    }
#elif defined(CAM_OS_LINUX_USER)
    CamOsPrintf("%s: This function is not supported in this platform\n\r", __FUNCTION__);
    eRet = CAM_OS_FAIL;
...
void CamOsTcondWaitInterruptible(CamOsTcond_t *ptTcond)
{
    ...
    if(ptHandle)
    {
        ...
        {
            reinit_completion(&ptHandle->tCompletion);
            wait_for_completion_interruptible(&ptHandle->tCompletion);
        }
    }
#endif
}
...
CamOsRet_e CamOsTcondTimedWaitInterruptible(CamOsTcond_t *ptTcond, u32 nMsec)
{
    ...
    if(ptHandle)
    {
        ...
        {
            reinit_completion(&ptHandle->tCompletion);
            if(!wait_for_completion_interruptible_timeout(&ptHandle->tCompletion, msecs_to_jiffies(nMsec)))
            {
                eRet = CAM_OS_TIMEOUT;
            }
        }
