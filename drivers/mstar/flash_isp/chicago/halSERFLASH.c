...
MS_BOOL HAL_SERFLASH_SetCKG(SPI_DrvCKG eCkgSpi)
{
	...
	switch (eCkgSpi)
    {
        case E_SPI_108M://110M
            WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x29), BIT(9), BIT(9));
            WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x28),BITS(15:14, 0),BMASK(15:14)); // set ckg_spi
            break;

        case E_SPI_86M: //88.4M
            WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x29), BIT(9), BIT(9));
            WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x28),BITS(15:14, 1),BMASK(15:14)); // set ckg_spi
            break;

        case E_SPI_54M: //55.3M
            WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x29), BIT(9), BIT(9));
            WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x28),BITS(15:14, 2),BMASK(15:14)); // set ckg_spi
            break;

        case E_SPI_43M: //48M
            WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x29), BIT(9), BIT(9));
            WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x28),BITS(15:14, 3),BMASK(15:14)); // set ckg_spi
            break;

        case E_SPI_XTALI:
        default:
			WRITE_WORD_MASK(GET_REG_ADDR(BASE_REG_CHIPTOP_ADDR, 0x29), 0x0, BIT(9));
			break;
    }
...
MS_BOOL HAL_MAP_Read(MS_U32 u32FlashAddr, MS_U32 u32FlashSize, MS_U8 *user_buffer)
{
    ...
    if(!spimem)
    {
        spimem = ioremap(MS_SPI_ADDR, MS_SPI_IO_SIZE);
        if(!spimem)
        {
                printk(KERN_INFO "Failed to ioremap SPI memory "
				"space at 0x%x\n",MS_SPI_ADDR);
    ...
    memcpy(user_buffer, (const void *)(spimem+u32FlashAddr), u32FlashSize);
    return TRUE;
}
