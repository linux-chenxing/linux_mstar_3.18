...
static void dump_config(MHAL_WARP_CONFIG* config)
{
    ...
    DEV_MSG(DEV_MSG_DBG, ".disp_table = {%d, %d, %d, 0x%p},\n", config->disp_table.resolution, config->disp_table.format,config->disp_table.size, config->disp_table.absolute_table);
    DEV_MSG(DEV_MSG_DBG, ".bb_table = {%d, 0x%p},\n", config->bb_table.size, config->bb_table.table);
    ...
    DEV_MSG(DEV_MSG_DBG, ".input_data = {%d, 0x%p, 0x%p},\n", config->input_data.num_planes, config->input_data.data[0], config->input_data.data[1]);
    DEV_MSG(DEV_MSG_DBG, ".output_image = {%d, %d, %d},\n", config->output_image.format, config->output_image.width, config->output_image.height);
    DEV_MSG(DEV_MSG_DBG, ".output_data = {%d, 0x%p, 0x%p},\n", config->output_data.num_planes, config->output_data.data[0], config->output_data.data[1]);
    DEV_MSG(DEV_MSG_DBG, ".afbc_en = %d,\n", config->afbc_en);
    DEV_MSG(DEV_MSG_DBG, ".fill_value = {%d, %d},\n", config->fill_value[0], config->fill_value[1]);
    DEV_MSG(DEV_MSG_DBG, ".axi_max_read_burst_size = %d,\n", config->axi_max_read_burst_size);
    DEV_MSG(DEV_MSG_DBG, ".axi_max_write_burst_size = %d,\n", config->axi_max_write_burst_size);
    DEV_MSG(DEV_MSG_DBG, ".axi_max_read_outstanding = %d,\n", config->axi_max_read_outstanding);
    DEV_MSG(DEV_MSG_DBG, ".axi_max_write_outstanding = %d,\n", config->axi_max_write_outstanding);

    DEV_MSG(DEV_MSG_DBG, ".debug_bypass_displacement_en = %d,\n", config->debug_bypass_displacement_en);
    DEV_MSG(DEV_MSG_DBG, ".debug_bypass_interp_en = %d,\n", config->debug_bypass_interp_en);
    DEV_MSG(DEV_MSG_DBG, ".output_tiles_width = %d,\n", config->output_tiles_width);
    DEV_MSG(DEV_MSG_DBG, ".output_tiles_height = %d,\n", config->output_tiles_height);
    DEV_MSG(DEV_MSG_DBG, ".num_output_tiles = %d,\n", config->num_output_tiles);
}
...
MS_S32 MHAL_WARP_Trigger(MHAL_WARP_INST_HANDLE instance, MHAL_WARP_CONFIG* config, MHAL_WARP_CALLBACK callback, void *user_data)
{
    ...
    {
        inst_handle->state = MHAL_WARP_INSTANCE_STATE_PROC_ERROR;
