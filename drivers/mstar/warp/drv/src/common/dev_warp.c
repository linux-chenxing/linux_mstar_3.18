...
static WARP_DEV_PROC_STATE_E warp_dev_check_config(MHAL_WARP_CONFIG *config)
{
    ...
    //check B.B bumbers
    width = (output_image->width + config->output_tiles_width - 1) / config->output_tiles_width;
    height = (output_image->height + config->output_tiles_height - 1) / config->output_tiles_height;
    size = width*height;

    if( config->num_output_tiles != size )
    {
    ...
    switch(op)
    {
        case MHAL_WARP_OP_MODE_MAP:
            ...
            if (CHECK_ALIGNEMT(disp_table->overlay))
            {
                return WARP_DEV_PROC_STATE_ADDRESS_NOT_ALIGNED_ERROR;
            }
...
static void warp_dev_start(warp_dev_handle *handle, MHAL_WARP_CONFIG *config)
{
    ...
    warp_hal_set_output_tile(hal_handle, (HAL_WARP_CONFIG *)config);
    ...
    //point to B.B table
    warp_hal_set_bb(hal_handle, (HAL_WARP_CONFIG *)config);
...
void warp_dev_post_proc(struct work_struct *wq)
{
    ...
    else
    {
         //run warp
         handle->state = WARP_DEV_STATE_PROCESSING;
         DEV_MSG(DEV_MSG_DBG, "process: %p\n", next_request->config->input_data.data[0]);
...
WARP_DEV_STATE warp_dev_isr_proc(MHAL_WARP_DEV_HANDLE device)
{
    ...
    if( (irq_ceva == handle->ceva_irq) && irq_status)
    {
