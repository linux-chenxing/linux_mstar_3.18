...
typedef struct
{
    ...
    union
    {
        const HAL_WARP_DISPPLAY_ABSOLUTE_ENTRY_T* absolute_table;
        const HAL_WARP_DISPPLAY_RELATIVE_ENTRY_T* relative_table;
        u32* overlay;
    };
...
typedef struct
{
    ...
    const HAL_WARP_BOUND_BOX_ENTRY_T *table;           // list of all bound boxes
...
typedef struct
{
    ...
    u8* data[2];                                     // pointers to the image planes' data
...
typedef struct
{
    ...
    u32 afbc_en;                                     // Enable AFBC mode
    u8 fill_value[2];                                // Fill value for out of range pixels

    u32 axi_max_read_burst_size;                     // Maximum read burst size of the AXI master port
    u32 axi_max_write_burst_size;                    // Maximum write burst size of the AXI master port
    u32 axi_max_read_outstanding;                    // Maximum outstanding read accesses of the AXI master port
    u32 axi_max_write_outstanding;                   // Maximum outstanding write accesses of the AXI master port

    // Debug features
    u32 debug_bypass_displacement_en;
    u32 debug_bypass_interp_en;

    u32 output_tiles_width;                          // calculated internally based on the bounding boxes
    u32 output_tiles_height;                         // calculated internally based on the bounding boxes
    u32 num_output_tiles;                            // calculated internally based on the bounding boxes
} HAL_WARP_CONFIG;
