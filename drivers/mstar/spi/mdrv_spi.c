...
void HAL_MSPI_SetSpiEnable(struct mstar_spi_dev *mspi_dev)
{
	U16 u16regval = 0;

	mutex_lock(&hal_mspi_lock);
	u16regval = READ_WORD(mspi_dev->spibase, REG_MSPI_CTRL);
    u16regval |= MSPI_RESET_BIT | MSPI_ENABLE_BIT;
    WRITE_WORD(mspi_dev->spibase, REG_MSPI_CTRL, u16regval);
	mutex_unlock(&hal_mspi_lock);
}

void HAL_MSPI_SetIntEnable(struct mstar_spi_dev *mspi_dev, BOOL enale )
{
	U16 u16regval = 0;
...
U32 MDrv_MSPI_SetSpeed(struct mstar_spi_dev *mspi_dev, U32 speed)
{
    ...
    mspi_dbg("%s:%d clk=%d,div=%d,speed=%d\n",__func__, __LINE__,
		clk_cfg[i].spi_clk, clk_cfg[i].spi_div, clk_cfg[i].spi_speed);
...
void MDrv_MSPI_Init(struct mstar_spi_dev *mspi_dev)
{
    ...
    HAL_MSPI_SetSpiEnable(mspi_dev);
	HAL_MSPI_SetIntEnable(mspi_dev, TRUE);
...
static int mstar_spi_probe(struct platform_device *pdev)
{
	...
	mspi_dev->spibase = (char *)(IO_ADDRESS(res->start));

    res = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	mspi_dev->chipbase = (char *)(IO_ADDRESS(res->start));

	res = platform_get_resource(pdev, IORESOURCE_MEM, 2);
	mspi_dev->clkbase= (char *)(IO_ADDRESS(res->start));
