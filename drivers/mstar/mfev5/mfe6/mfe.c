...
static int _enc_done(mhve_ops* mops)
{
    ...
    hndl->u_oused+= (rqct->i_bitcnt + regs->bits_delta)/8;
    /* update ref-list */
    dpb = hndl->p_recn;
    if (sh->i_ref_idc)
...
static int _enc_conf(mhve_ops* mops)
{
    ...
    regs->outbs_addr = hndl->u_obase + hndl->u_oused;
    regs->outbs_size = hndl->u_osize - hndl->u_oused;
    return 0;
}
