...
//  Global Functions
//-------------------------------------------------------------------------------------------------
XC_ISR_INFO stXC1_IsrInfo[MAX_SC_INT][MAX_ISR_NUM_OF_EACH_INT];
// for getting the DLC Ymax/min from VBlanking
MS_U8 g_u8DLC_Ymax_From_IP1F2_sc1 = 0x00;
MS_U8 g_u8DLC_Ymin_From_IP1F2_sc1 = 0xFF;
MS_U8 g_u8DLC_Cbmax_From_IP1F2_sc1 = 0x00;
MS_U8 g_u8DLC_Cbmin_From_IP1F2_sc1 = 0xFF;
MS_U8 g_u8DLC_Crmax_From_IP1F2_sc1 = 0x00;
MS_U8 g_u8DLC_Crmin_From_IP1F2_sc1 = 0xFF;
...
//-------------------------------------------------------------------------------------------------
static MS_U8 _MApi_XC1_GetIntSrc(SC_INT_SRC eIntNum)
{
    MS_U8 u8Int_bit;
...
static void _MDrv_SC1_clear_interrupt(SC_INT_SRC u8IntSrc)
{
    //MS_U8 u8Bank;
    //MS_U8 u8ByteIndex;
    //MS_U8 u8ByteMask;
    ...
    MS_U8 u8IntRemap = _MApi_XC1_GetIntSrc(u8IntSrc);
...
void MDrv_SC1_set_interrupt(SC_INT_SRC u8IntSrc, MS_BOOL bEnable)
{
    MS_U8 u8Bank;
    //MS_U8 u8ByteIndex;
    //MS_U8 u8ByteMask;
...
void MDrv_SC1_get_rgb_maxmin_from_ip1f2(void)
{
    //MS_U8 u8Bank;
    ...
    case 0:
        ...
        g_u8DLC_Cbmin_From_IP1F2_sc1 = MDrv_ReadByte(L_BK_IP1F2(0x11));
        MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x03, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Crmax_From_IP1F2_sc1 = MDrv_ReadByte(L_BK_IP1F2(0x11));*/
        SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x00, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Cbmin_From_IP1F2_sc1 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
        SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x03, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Crmax_From_IP1F2_sc1 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
        break;

    case 1:
        /*MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x01, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Crmin_From_IP1F2_sc1 = MDrv_ReadByte(L_BK_IP1F2(0x11));
        MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x04,( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Ymax_From_IP1F2_sc1 = MDrv_ReadByte(L_BK_IP1F2(0x11));*/
    ...
    case 2:
        ...
        g_u8DLC_Ymin_From_IP1F2_sc1 = MDrv_ReadByte(L_BK_IP1F2(0x11));
        MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x05, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Cbmax_From_IP1F2_sc1 = MDrv_ReadByte(L_BK_IP1F2(0x11));*/
...
void MDrv_SC1_get_y_maxmin_from_ip1f2(void)
{
    //MS_U8 u8Bank;
    ...
    {
        //g_u8DLC_Ymin_From_IP1F2_sc1 = MDrv_ReadByte(L_BK_IP1F2(0x11));
        g_u8DLC_Ymin_From_IP1F2_sc1 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
    }
    else
    {
        ...
        {
            g_u8DLC_Ymin_From_IP1F2_sc1 = 16;           // YUV min value is 16
        }
    }
    ...
    {
        g_u8DLC_Ymax_From_IP1F2_sc1 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
    }
    else
    {
        if(0)   //(mvideo_sc_is_pre_rgb2yuv(MAIN_WINDOW)), Daten FixMe
        ...
        {
            g_u8DLC_Ymax_From_IP1F2_sc1 = 235;           // YUV max value is 235
        }
    }
...
#endif
{
    ...
    {
        MDrv_SC1_set_interrupt(SC_INT_VSINT, DISABLE);
        MDrv_SC1_vsync_isr();
        MDrv_SC1_set_interrupt(SC_INT_VSINT, ENABLE);
    }
#endif
    ...
    for(eSCIntNum=SC_INT_START; eSCIntNum<MAX_SC_INT; eSCIntNum++)
    {
        if(u32IntSource & (1 << eSCIntNum))
        {
            ...
            _MDrv_SC1_clear_interrupt(eSCIntNum);

            // call ISR
            for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
            {
                if(stXC1_IsrInfo[eSCIntNum][i].aryXC_ISR != NULL)
                {
                    // execute ISR
                    (void)(stXC1_IsrInfo[eSCIntNum][i].aryXC_ISR) ((SC_INT_SRC)eSCIntNum, stXC1_IsrInfo[eSCIntNum][i].aryXC_ISR_Param);
                }
            }
        }
    }
...
//-------------------------------------------------------------------------------------------------
MS_U8   MDrv_XC1_InterruptGetMaxIntNum(void)
{
    return MAX_SC_INT;
}

//-------------------------------------------------------------------------------------------------
/// After get maximum number of interrupts by @ref MDrv_XC1_InterruptGetMaxIntNum, you can
...
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_XC1_InterruptAvaliable(SC_INT_SRC eIntNum)
{
    MS_U8 i;
    MS_BOOL bReturn = FALSE;

    // check if out of range or not supported
    if((eIntNum >= MDrv_XC1_InterruptGetMaxIntNum()) || (((1UL << eIntNum) & SUPPORTED_XC_INT) == 0))
    {
        goto Exit;
    }
    ...
    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
        if(stXC1_IsrInfo[eIntNum][i].aryXC_ISR == NULL)
        {
            bReturn = TRUE;
            break;
        }
...
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_XC1_InterruptIsAttached(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_U8 i;

    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
        if((stXC1_IsrInfo[eIntNum][i].aryXC_ISR == pIntCb) && (stXC1_IsrInfo[eIntNum][i].aryXC_ISR_Param == pParam))
        {
            return TRUE;
        }
    }
...
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_XC1_InterruptAttach(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_BOOL bReturn = FALSE;
    _XC_ENTRY_MUTEX(_XC_Mutex);
    bReturn = MDrv_XC1_InterruptAttachWithoutMutex(eIntNum, pIntCb, pParam);
...
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_XC1_InterruptDeAttach(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_BOOL bReturn = FALSE;

    _XC_ENTRY_MUTEX(_XC_Mutex);
    bReturn = MDrv_XC1_InterruptDeAttachWithoutMutex(eIntNum, pIntCb, pParam);

    _XC_RETURN_MUTEX(_XC_Mutex);
    return bReturn;
}

MS_BOOL MDrv_XC1_InterruptAttachWithoutMutex(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_U8 i;
    MS_BOOL bReturn = FALSE;
    ...
    if((eIntNum >= MDrv_XC1_InterruptGetMaxIntNum()) || (((1UL << eIntNum) & SUPPORTED_XC_INT) == 0))
    {
        SC_ISR_DBG(printf("ISRAttach failed %d (max %d, supported %lx)\n", eIntNum, MDrv_XC1_InterruptGetMaxIntNum(), (MS_U32)SUPPORTED_XC_INT));
        goto Exit;
    }
    ...
    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
        if(stXC1_IsrInfo[eIntNum][i].aryXC_ISR == NULL)
        {
            ...
            MDrv_SC1_set_interrupt(eIntNum, DISABLE);

            stXC1_IsrInfo[eIntNum][i].aryXC_ISR_Param = pParam;
            stXC1_IsrInfo[eIntNum][i].aryXC_ISR = pIntCb;

            // enable interrupt
            MDrv_SC1_set_interrupt(eIntNum, ENABLE);
...
MS_BOOL MDrv_XC1_InterruptDeAttachWithoutMutex(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_U8 i;
    MS_BOOL bReturn = FALSE;
    ...
    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
        if((stXC1_IsrInfo[eIntNum][i].aryXC_ISR == pIntCb) &&
            (stXC1_IsrInfo[eIntNum][i].aryXC_ISR_Param == pParam))
        {
            // disable interrupt first to avoid racing condition
            MDrv_SC1_set_interrupt(eIntNum, DISABLE);

            stXC1_IsrInfo[eIntNum][i].aryXC_ISR = NULL;
            stXC1_IsrInfo[eIntNum][i].aryXC_ISR_Param = NULL;
            bReturn = TRUE;
