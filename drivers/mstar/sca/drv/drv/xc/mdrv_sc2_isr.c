...
//  Global Functions
//-------------------------------------------------------------------------------------------------
XC_ISR_INFO stXC2_IsrInfo[MAX_SC_INT][MAX_ISR_NUM_OF_EACH_INT];
// for getting the DLC Ymax/min from VBlanking
MS_U8 g_u8DLC_Ymax_From_IP1F2_sc2 = 0x00;
MS_U8 g_u8DLC_Ymin_From_IP1F2_sc2 = 0xFF;
MS_U8 g_u8DLC_Cbmax_From_IP1F2_sc2 = 0x00;
MS_U8 g_u8DLC_Cbmin_From_IP1F2_sc2 = 0xFF;
MS_U8 g_u8DLC_Crmax_From_IP1F2_sc2 = 0x00;
MS_U8 g_u8DLC_Crmin_From_IP1F2_sc2 = 0xFF;
...
//-------------------------------------------------------------------------------------------------
static MS_U8 _MApi_XC2_GetIntSrc(SC_INT_SRC eIntNum)
{
    MS_U8 u8Int_bit;
...
static void _MDrv_SC2_clear_interrupt(SC_INT_SRC u8IntSrc)
{
    //MS_U8 u8Bank;
    //MS_U8 u8ByteIndex;
    //MS_U8 u8ByteMask;
    ...
    //SC_W2BYTEMSK((REG_SC1_BK00_12_L + u8ByteIndex), 1 , u8ByteMask);
    MS_U8 u8IntRemap = _MApi_XC2_GetIntSrc(u8IntSrc);

    if(u8IntRemap < 16)
    {
        SC_W2BYTEMSK(REG_SC1_BK00_12_L , BIT(u8IntRemap),BIT(u8IntRemap));
    }
    else
    {
        SC_W2BYTEMSK(REG_SC1_BK00_13_L , BIT(u8IntRemap-16) ,BIT(u8IntRemap-16));
    }
    //MDrv_WriteByte(BK_SELECT_00, u8Bank);
}

void MDrv_SC2_set_interrupt(SC_INT_SRC u8IntSrc, MS_BOOL bEnable)
{
    MS_U8 u8Bank;
    //MS_U8 u8ByteIndex;
    //MS_U8 u8ByteMask;
    ...
    if(bEnable)
    {
        ...
        {
            SC_W2BYTEMSK(REG_SC1_BK00_12_L ,BIT(u8IntRemap),BIT(u8IntRemap));
            SC_W2BYTEMSK(REG_SC1_BK00_14_L , 0 ,BIT(u8IntRemap));
        }
        else
        {
            SC_W2BYTEMSK(REG_SC1_BK00_13_L ,BIT(u8IntRemap-16),BIT(u8IntRemap-16));
            SC_W2BYTEMSK(REG_SC1_BK00_15_L , 0 ,BIT(u8IntRemap-16));
        }
    }
    else
    {
        ...
        if(u8IntRemap < 16)
        {
            SC_W2BYTEMSK(REG_SC1_BK00_14_L , BIT(u8IntRemap),BIT(u8IntRemap));
            SC_W2BYTEMSK(REG_SC1_BK00_12_L , 0,BIT(u8IntRemap));
        }
        else
        {
            SC_W2BYTEMSK(REG_SC1_BK00_15_L ,BIT(u8IntRemap-16),BIT(u8IntRemap-16));
            SC_W2BYTEMSK(REG_SC1_BK00_13_L , 0 ,BIT(u8IntRemap-16));
        }
...
void MDrv_SC2_get_rgb_maxmin_from_ip1f2(void)
{
    //MS_U8 u8Bank;
    ...
    SC_W2BYTEMSK(REG_SC1_BK01_0E_L, 0x11,0x00FF); // enable ATG
    ...
    case 0:
        ...
        g_u8DLC_Cbmin_From_IP1F2_sc2 = MDrv_ReadByte(L_BK_IP1F2(0x11));
        MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x03, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Crmax_From_IP1F2_sc2 = MDrv_ReadByte(L_BK_IP1F2(0x11));*/
        SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x00, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Cbmin_From_IP1F2_sc2 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
        SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x03, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Crmax_From_IP1F2_sc2 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
        break;

    case 1:
        /*MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x01, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Crmin_From_IP1F2_sc2 = MDrv_ReadByte(L_BK_IP1F2(0x11));
        MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x04,( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Ymax_From_IP1F2_sc2 = MDrv_ReadByte(L_BK_IP1F2(0x11));*/
        SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x01, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Cbmin_From_IP1F2_sc2 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
        SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x04, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Crmax_From_IP1F2_sc2 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
        break;

    case 2:
        /*MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x02, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Ymin_From_IP1F2_sc2 = MDrv_ReadByte(L_BK_IP1F2(0x11));
        MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x05, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Cbmax_From_IP1F2_sc2 = MDrv_ReadByte(L_BK_IP1F2(0x11));*/
        SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x02, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Cbmin_From_IP1F2_sc2 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
        SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x05, ( BIT(2) | BIT(1) | BIT(0) ));
        g_u8DLC_Crmax_From_IP1F2_sc2 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
        break;
    }
    u8Cnt++;
...
void MDrv_SC2_get_y_maxmin_from_ip1f2(void)
{
    //MS_U8 u8Bank;
    ...
    if((SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF) >= 16)
    {
        //g_u8DLC_Ymin_From_IP1F2_sc2 = MDrv_ReadByte(L_BK_IP1F2(0x11));
        g_u8DLC_Ymin_From_IP1F2_sc2 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
    }
    else
    {
        ...
        {
            g_u8DLC_Ymin_From_IP1F2_sc2 = 16;           // YUV min value is 16
        }
    }

    // read YMax
    //MDrv_WriteByteMask(L_BK_IP1F2(0x0F), 0x04, ( BIT(2) | BIT(1) | BIT(0) ));
    SC_W2BYTEMSK(REG_SC1_BK01_0F_L, 0x0004, ( BIT(2) | BIT(1) | BIT(0) ));
    // prevent YUV input over shoot, to speed up, we try to do some adjust if Ymax > 235
    //if(MDrv_ReadByte(L_BK_IP1F2(0x11)) <= 235)
    if((SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF) <= 235)
    {
        g_u8DLC_Ymax_From_IP1F2_sc2 = SC_R2BYTE(REG_SC1_BK01_11_L) & 0x00FF;
    }
    else
    {
        if(0)   //(mvideo_sc_is_pre_rgb2yuv(MAIN_WINDOW)), Daten FixMe
        ...
        {
            g_u8DLC_Ymax_From_IP1F2_sc2 = 235;           // YUV max value is 235
        }
    }
...
void MDrv_SC2_vsync_isr(void)
{
    ...
    if(SC_R2BYTE(REG_SC1_BK01_02_L) & BIT(7))
    {
        u16FrameCnt = 0;
        return;
    }
    ...
    // fill memory
    pDS_Info->u16FrameCnt = u16FrameCnt++;//SC_R2BYTE(REG_SC1_BK12_1D_L);
//    pDS_Info->u8DisplayIdx = 0x00;//SC_R2BYTE(REG_SC1_BK12_1E_L) & 0xFF;
//    pDS_Info->u8ScalerIdx = 0x00;//(SC_R2BYTE(REG_SC1_BK12_1E_L) >> 8) & 0xFF;
    pDS_Info->u32DS_H_Ratio = SWAP(SC_R4BYTE(REG_SC1_BK23_1C_L));
    pDS_Info->u32DS_V_Ratio = SWAP(SC_R4BYTE(REG_SC1_BK23_1E_L));
    pDS_Info->u16H_Cap = SC_R2BYTE(REG_SC1_BK01_07_L);
    ...
    // fill memory
    pDS_Brief_Info->u32DS_H_Ratio = SWAP(SC_R4BYTE(REG_SC1_BK23_1C_L));
    pDS_Brief_Info->u32DS_V_Ratio = SWAP(SC_R4BYTE(REG_SC1_BK23_1E_L));
...
#else
void MDrv_SC2_isr( InterruptNum eIntNum )
#endif
{
    MS_U32 u32IntSource;
    ...
    u32IntSource = SC_R4BYTE(REG_SC1_BK00_10_L);
    ...
    {
        MDrv_SC2_set_interrupt(SC_INT_VSINT, DISABLE);
        MDrv_SC2_vsync_isr();
        MDrv_SC2_set_interrupt(SC_INT_VSINT, ENABLE);
    }
#endif
    ...
    for(eSCIntNum=SC_INT_START; eSCIntNum<MAX_SC_INT; eSCIntNum++)
    {
        if(u32IntSource & (1 << eSCIntNum))
        {
            ...
            _MDrv_SC2_clear_interrupt(eSCIntNum);

            // call ISR
            for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
            {
                if(stXC2_IsrInfo[eSCIntNum][i].aryXC_ISR != NULL)
                {
                    // execute ISR
                    (void)(stXC2_IsrInfo[eSCIntNum][i].aryXC_ISR) ((SC_INT_SRC)eSCIntNum, stXC2_IsrInfo[eSCIntNum][i].aryXC_ISR_Param);
                }
            }
        }
    }
...
//-------------------------------------------------------------------------------------------------
MS_U8   MDrv_XC2_InterruptGetMaxIntNum(void)
{
    return MAX_SC_INT;
}

//-------------------------------------------------------------------------------------------------
/// After get maximum number of interrupts by @ref MDrv_XC2_InterruptGetMaxIntNum, you can
...
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_XC2_InterruptAvaliable(SC_INT_SRC eIntNum)
{
    MS_U8 i;
    MS_BOOL bReturn = FALSE;

    // check if out of range or not supported
    if((eIntNum >= MDrv_XC2_InterruptGetMaxIntNum()) || (((1UL << eIntNum) & SUPPORTED_XC_INT) == 0))
    {
        goto Exit;
    }
    ...
    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
        if(stXC2_IsrInfo[eIntNum][i].aryXC_ISR == NULL)
        {
            bReturn = TRUE;
            break;
        }
...
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_XC2_InterruptIsAttached(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_U8 i;

    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
        if((stXC2_IsrInfo[eIntNum][i].aryXC_ISR == pIntCb) && (stXC2_IsrInfo[eIntNum][i].aryXC_ISR_Param == pParam))
        {
            return TRUE;
        }
    }
...
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_XC2_InterruptAttach(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_BOOL bReturn = FALSE;
    _XC_ENTRY_MUTEX(_XC_Mutex);
    bReturn = MDrv_XC2_InterruptAttachWithoutMutex(eIntNum, pIntCb, pParam);
...
//-------------------------------------------------------------------------------------------------
MS_BOOL MDrv_XC2_InterruptDeAttach(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_BOOL bReturn = FALSE;

    _XC_ENTRY_MUTEX(_XC_Mutex);
    bReturn = MDrv_XC2_InterruptDeAttachWithoutMutex(eIntNum, pIntCb, pParam);

    _XC_RETURN_MUTEX(_XC_Mutex);
    return bReturn;
}

MS_BOOL MDrv_XC2_InterruptAttachWithoutMutex(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_U8 i;
    MS_BOOL bReturn = FALSE;
    ...
    if((eIntNum >= MDrv_XC2_InterruptGetMaxIntNum()) || (((1UL << eIntNum) & SUPPORTED_XC_INT) == 0))
    {
        SC_ISR_DBG(printf("ISRAttach failed %d (max %d, supported %lx)\n", eIntNum, MDrv_XC2_InterruptGetMaxIntNum(), (MS_U32)SUPPORTED_XC_INT));
        goto Exit;
    }
    ...
    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
        if(stXC2_IsrInfo[eIntNum][i].aryXC_ISR == NULL)
        {
            ...
            MDrv_SC2_set_interrupt(eIntNum, DISABLE);

            stXC2_IsrInfo[eIntNum][i].aryXC_ISR_Param = pParam;
            stXC2_IsrInfo[eIntNum][i].aryXC_ISR = pIntCb;

            // enable interrupt
            MDrv_SC2_set_interrupt(eIntNum, ENABLE);
...
MS_BOOL MDrv_XC2_InterruptDeAttachWithoutMutex(SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    MS_U8 i;
    MS_BOOL bReturn = FALSE;
    ...
    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
        if((stXC2_IsrInfo[eIntNum][i].aryXC_ISR == pIntCb) &&
            (stXC2_IsrInfo[eIntNum][i].aryXC_ISR_Param == pParam))
        {
            // disable interrupt first to avoid racing condition
            MDrv_SC2_set_interrupt(eIntNum, DISABLE);

            stXC2_IsrInfo[eIntNum][i].aryXC_ISR = NULL;
            stXC2_IsrInfo[eIntNum][i].aryXC_ISR_Param = NULL;
            bReturn = TRUE;
