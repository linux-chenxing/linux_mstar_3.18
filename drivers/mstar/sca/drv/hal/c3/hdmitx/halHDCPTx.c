...
{
    {HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_MODE_61, 0x8000, 0x8000}, // Enable HDCP encryption
    {HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, 0x000C, 0x0000},
};

MSTHDMITX_REG_TYPE TxHdcpEncryptOnTbl[] =
{
    {HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, 0x0008, 0x0008},
};

MSTHDMITX_REG_TYPE TxHdcpEncryptOffTbl[] =
{
    {HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, 0x0008, 0x0000},
};
...
MS_BOOL MHal_HDMITx_HdcpCheckBksvLn(void)
{
    ...
    for (i = 0; i < 7; i++)
    {
        ...
        if(i%2)
            MHal_HDMITx_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_LN_64+(i/2), (Lm[i]<<8) | Lm[i-1]);
        if(bDebugHDCPFlag)
            printf("0x%x, ", Lm[i]);
    }
    if(bDebugHDCPFlag)
        printf("\n");
    MHal_HDMITx_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_LN_SEED_67, (temp<<8) | Lm[6]);
    if(bDebugHDCPFlag)
        printf("Seed=0x%x\r\n", temp);
...
MS_BOOL MHal_HDMITx_HdcpWriteAn(MS_U8 internal)
{
    ...
    if (internal == 1)
    {
        MHal_HDMITx_Mask_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_MODE_61, 0x0100, 0x0100);
        MHal_HDMITx_Mask_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, BIT1, BIT1);
        MHal_HDMITx_Mask_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, BIT1, 0);

        if(bDebugHDCPFlag)
            printf("Internal An=");
        for (i = 0; i < 4; i++)
        {
            temp = MHal_HDMITx_Read(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_AN_68+i);
    ...
    {
        MHal_HDMITx_Mask_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_MODE_61, 0x0100, 0x0000);
        for(i = 0; i < 4; i++)
            MHal_HDMITx_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_AN_68+i, (TxHdcpAnTbl[2*i+1] <<8) | (TxHdcpAnTbl[2*i]));
...
void MHal_HDMITx_HdcpStartCipher(void)
{
    ...
    MHal_HDMITx_Mask_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, 0x000F, 0);
    MHal_HDMITx_Mask_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, 0x000F, BIT0);
    MHal_HDMITx_Mask_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, 0x000F, 0);

    while (count-- && !(MHal_HDMITx_Read(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62) & BIT8));
    if (count == 0)
    {
        if(bDebugHDCPFlag)
...
{
    MHal_HDMITx_Mask_Write(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_COMMAND_62, 0x000F, 0x000C);
}
...
{
    *ptx_ri = MHal_HDMITx_Read(HDMITX_MISC_HDCP_REG_BASE, REG_HDCP_TX_RI_60);
