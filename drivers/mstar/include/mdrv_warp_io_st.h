...
typedef struct
{
    __s32 size;                                         // number of bound boxes
...
typedef struct
{
    ...
    __u8* data[2];                                      // pointers to the image planes' data
...
typedef struct
{
    ...
    __u32 afbc_en;                                      // Enable AFBC mode
    __u8 fill_value[2];                                 // Fill value for out of range pixels

    __u32 axi_max_read_burst_size;                      // Maximum read burst size of the AXI master port
    __u32 axi_max_write_burst_size;                     // Maximum write burst size of the AXI master port
    __u32 axi_max_read_outstanding;                     // Maximum outstanding read accesses of the AXI master port
    __u32 axi_max_write_outstanding;                    // Maximum outstanding write accesses of the AXI master port

    // Debug features
    __u32 debug_bypass_displacement_en;
    __u32 debug_bypass_interp_en;

    __u32 output_tiles_width;                           // calculated internally based on the bounding boxes
    __u32 output_tiles_height;                          // calculated internally based on the bounding boxes
    __u32 num_output_tiles;                             // calculated internally based on the bounding boxes
} MHAL_WARP_CONFIG;
