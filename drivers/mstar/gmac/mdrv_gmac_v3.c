...
//        Chip_Inv_Cache_Range_VA_PA((u32)(&(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index].addr)),(&(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index].addr)) - LocPtr->RAM_VA_PA_OFFSET, sizeof(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index].addr));
    ...
    #ifdef GMAC_RX_ADDRESS_34BIT
            LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].high_tag |= GMAC_MIU1_ADDR;
    #endif
        }
        else
        {
...
#elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
        if((LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr = (u32)__virt_to_phys(LocPtr->rx_desc_sk_buff_list[LocPtr->rx_desc_free_index]->data) + GMAC_DESC_WRAP) > MSTAR_MIU1_BUS_BASE)
        {
            LocPtr->rx_desc_list[LocPtr->rx_desc_free_index].addr -= MSTAR_MIU1_BUS_BASE;
...
static int MDev_GMAC_rx (struct net_device *dev)
{
    ...
    do
    {
        ...
        #elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
        TO_DO;//Chip_Inv_Cache_Range_VA_PA((size_t)p_recv,(size_t)p_recv - LocPtr->RAM_VA_PA_OFFSET ,pktlen);
        #else
            #ERROR
        #endif
    #endif
...
static int MDev_GMAC_init(void)
{
    ...
    if (request_irq(gmac_dev->irq, MDev_GMAC_interrupt, 0/*SA_INTERRUPT*/, gmac_dev->name, gmac_dev))
        goto end;
