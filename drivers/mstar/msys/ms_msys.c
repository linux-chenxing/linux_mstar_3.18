    ...
    if ( CHK_VERCHK_HEADER(&(((MSYS_ADDR_TRANSLATION_INFO __user *)arg)->VerChk_Version)) )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                udid_info.udid = (u64)INREG16(BASE_REG_PMTOP_PA + REG_ID_00) ;
            #else
...
static int msys_request_freq(unsigned long arg)
{
    ...
    if((freq_info.padnum>GPIO_NR) ||(freq_info.padnum<PAD_PM_IRIN))
    {
    ...
    {
        if((freq_info.freq>FREQ_75MHZ) ||(freq_info.freq<FREQ_6MHZ))
        {
        ...
        {
            case PAD_SNR0_GPIO5:
                // reg_ccir0_ctrl_mode[8]=1
                OUTREGMSK16(BASE_REG_PADTOP1_PA + REG_ID_02, (BIT8), BIT8|BIT9);//set pad
                OUTREGMSK16(BASE_REG_CLKGEN2_PA + REG_ID_55, (freq_info.freq << 2), BIT2|BIT3|BIT4); //set freq

                if(freq_info.bInvert)
                    CLRREG16(BASE_REG_CLKGEN2_PA + REG_ID_55, BIT1); //invert clk
                CLRREG16(BASE_REG_CLKGEN2_PA + REG_ID_55, BIT0); //enable clk
                break;
            case PAD_SNR1_GPIO5:
                // reg_ccir0_ctrl_mode[9]=1
                OUTREGMSK16(BASE_REG_PADTOP1_PA + REG_ID_02, (BIT9), BIT8|BIT9);//set pad
                OUTREGMSK16(BASE_REG_CLKGEN2_PA + REG_ID_55, (freq_info.freq << 2), BIT10|BIT11|BIT12); //set freq

                if(freq_info.bInvert)
                    CLRREG16(BASE_REG_CLKGEN2_PA + REG_ID_55, BIT9); //invert clk
                CLRREG16(BASE_REG_CLKGEN2_PA + REG_ID_55, BIT8); //enable clk
                break;
            case PAD_SNR2_GPIO5:
                // reg_ccir2_ctrl_mode[3]=1
                OUTREGMSK16(BASE_REG_PADTOP1_PA + REG_ID_03, (BIT3), BIT3|BIT4);//set pad
                OUTREGMSK16(BASE_REG_CLKGEN2_PA + REG_ID_56, (freq_info.freq << 2), BIT2|BIT3|BIT4); //set freq

                if(freq_info.bInvert)
                    CLRREG16(BASE_REG_CLKGEN2_PA + REG_ID_56, BIT1); //invert clk
                CLRREG16(BASE_REG_CLKGEN2_PA + REG_ID_56, BIT0); //enable clk
                break;
            default:
                MSYS_ERROR("Not implement PAD: %d for this platform\n", freq_info.padnum);
                break;
    ...
    else //disable clk
    {
        switch(freq_info.padnum)
        {
            case PAD_SNR0_GPIO5:
                // reg_ccir0_ctrl_mode[8]=1
                SETREG16(BASE_REG_CLKGEN2_PA + REG_ID_55, BIT0); //disable clk
                break;
            case PAD_SNR1_GPIO5:
                // reg_ccir0_ctrl_mode[9]=1
                SETREG16(BASE_REG_CLKGEN2_PA + REG_ID_55, BIT0); //disable clk
            ...
            case PAD_SNR2_GPIO5:
                // reg_ccir2_ctrl_mode[3]=1
                SETREG16(BASE_REG_CLKGEN2_PA + REG_ID_56, BIT0); //disable clk
                break;
            default:
                MSYS_ERROR("Not implement PAD: %d for this platform\n", freq_info.padnum);
                break;
