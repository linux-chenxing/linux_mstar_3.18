...
#include "infinity3_reg_isp0.h"
#include "infinity3_reg_isp1.h"
#include "infinity3_reg_isp2.h"
#include "infinity3_reg_isp3.h"
#include "infinity3_reg_isp4.h"
#include "infinity3_reg_isp5.h"
#include "infinity3_reg_isp6.h"
#include "infinity3_reg_isp7.h"
#include "infinity3_reg_isp8.h"
#include "infinity3_reg_isp9.h"
...
#endif
static volatile bank_0 *pISP_INT;
...
static volatile bank_3 *isp_miu_bases;
...
#endif
static volatile bank_4 *isp_miu;
...
static volatile u32 u4DefDisable[3] =
{
    ...
    //ISP_SHIFTBITS(INT_DB_UPDATE_DONE)|ISP_SHIFTBITS(INT_SR_VREF_RISING)|ISP_SHIFTBITS(INT_SR_VREF_FALLING)|ISP_SHIFTBITS(INT_ISP_BUSY)|ISP_SHIFTBITS(INT_ISP_IDLE)|ISP_SHIFTBITS(INT_ISP_FIFO_FULL),
    ...
    |ISP_SHIFTBITS(INT3_HDR_HISTO_DONE)|ISP_SHIFTBITS(INT3_RGBIR_HISTO_DONE)|ISP_SHIFTBITS(INT3_AWB_ROW_DONE)|ISP_SHIFTBITS(INT3_HISTO_ROW_DONE)
...
inline void HalInitRegs(void **pRegs)
{
    ...
    *(volatile u32*)(RIUBASE_ISP[0] + 0x7a*4) = 0;
	...
	pISP_INT = (volatile bank_0*)(RIUBASE_ISP[0]);

    // polarity & interface
    //isp_vsync_pol = pISP_INT->reg_sensor_vsync_polarity;
    //isp_vsync_pol = (volatile u32*)(RIUBASE_ISP[0] + 0x10*4);
    //isp_sif = pISP_INT->reg_isp_if_src_sel;
    //isp_sif = (volatile u32*)(RIUBASE_ISP[0] + 0x05*4);

  // statistics output address
  isp_miu_bases = (volatile bank_3*)(RIUBASE_ISP[3]);
...
inline void HalISPMaskClear1(u32 clear)
{
    ...
    pISP_INT->reg_c_irq_clr = 0;
    //pISP_INT->reg_c_irq_mask &= ~(clear & ~u4DefDisable[0]);
    pISP_INT->reg_c_irq_mask = u4DefDisable[0];
}
...
inline void HalISPMaskClear2(u32 clear)
{
    ...
    pISP_INT->reg_c_irq_clr2 = 0;
    //pISP_INT->reg_c_irq_mask2 &= ~(clear & ~u4DefDisable[1]);
    pISP_INT->reg_c_irq_mask2 = u4DefDisable[1];
}
...
inline void HalISPMaskClear3(u32 clear)
{
    ...
    pISP_INT->reg_c_irq_clr3 = 0;
    //pISP_INT->reg_c_irq_mask &= ~(clear & ~u4DefDisable[2]);
    pISP_INT->reg_c_irq_mask3 = u4DefDisable[2];
}
...
inline void HalISPSetAeBaseAddr(volatile unsigned long Addr)
{
    ...
    isp_miu_bases->reg_ae_statis_base_0 = Addr>>4;
    isp_miu_bases->reg_ae_statis_base_1 = Addr>>20;

    pr_debug("[ISP] AeBaseAddr 0x%08x, H:0x%04x, L:0x%04x\n", (unsigned int)Addr, (unsigned int)isp_miu_bases->reg_ae_statis_base_1,
        (unsigned int)isp_miu_bases->reg_ae_statis_base_0);
}

inline void HalISPSetAwbBaseAddr(volatile unsigned long Addr)
{
    Addr = Chip_Phys_to_MIU(Addr);
    isp_miu_bases->reg_awb_statis_base_0 = Addr>>4;
    isp_miu_bases->reg_awb_statis_base_1 = Addr>>20;

    pr_debug("[ISP] AwbBaseAddr 0x%08x, H:0x%04x, L:0x%04x\n", (unsigned int)Addr, (unsigned int)isp_miu_bases->reg_awb_statis_base_1,
        (unsigned int)isp_miu_bases->reg_awb_statis_base_0);
}
...
inline void HalISPSetMotBaseAddr(volatile unsigned long Addr)
{
    ...
    Addr = Chip_Phys_to_MIU(Addr);
    isp_miu_bases->reg_mot_fb_base_0 = Addr>>4;
    isp_miu_bases->reg_mot_fb_base_1 = Addr>>20;
...
inline void HalISPSetDnrFbAddr(unsigned long Addr, int id)
{
    ...
    Addr = Chip_Phys_to_MIU(Addr);
    if(id==0) {
        isp_miu_bases->reg_dnr_fb_base_0_0 = (Addr>>4) & 0xFFFF;
        isp_miu_bases->reg_dnr_fb_base_0_1 = (Addr>>20) & 0xFFF;
	} else if(id==1) {
		isp_miu_bases->reg_dnr_fb_base_1_0 = (Addr>>4) & 0xFFFF;
...
inline void HalISPMLoadWriteData(volatile unsigned int Sram_Id, volatile unsigned long Addr)
{
    ...
    isp_miu->reg_isp_load_st_addr_0 = (Addr>>4) & 0xFFFF;
...
inline void HalISPMLoadWriteAllTable(volatile unsigned long Addr, volatile unsigned long *offset)
{
    ...
    for(i = 0; i < IQ_MEM_NUM; i++) {
        ...
        //Set MIU 16-byte start address
        isp_miu->reg_isp_load_st_addr_0 = (tbl_addr>>4) & 0xFFFF;
