...
static int isp_probe(struct platform_device* pdev)
{
    ...
    of_property_read_u32_array(pdev->dev.of_node, "banks", (unsigned int*)u4Banks, 10);
    for (i = 0; i < 10; i++) {
        isp_mem.pISPRegs[i] = (void*)ioremap(BANK_TO_ADDR32(u4Banks[i])+u4IO_PHY_BASE, BANK_SIZE);
    }
    of_property_read_u32(pdev->dev.of_node, "isp-flag", &g_isp_flag);
    of_property_read_u32(pdev->dev.of_node, "isp-res", &g_isp_max_res);
