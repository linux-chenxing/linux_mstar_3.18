...
void ceva_hal_enable_warp(ceva_hal_handle *handle)
{
    ...
    // handle->reg_sys.reg_rstz_warp = CEVA_HAL_RESET_WARP_ALL;
    handle->reg_sys.reg42 = 0xFFFF;
    REGW(handle->base_sys, 0x42, handle->reg_sys.reg42);

#if (CHECK_REGISTER)
    CWA_MSG(CWA_MSG_DBG, "REGW: addr 0x%02X,  value 0x%08X\n", 0x40, handle->reg_sys.reg40);
    CWA_MSG(CWA_MSG_DBG, "REGW: addr 0x%02X,  value 0x%08X\n", 0x42, handle->reg_sys.reg42);
...
#if (CHECK_REGISTER)
    CWA_MSG(CWA_MSG_DBG, "base_axi2miu2: addr 0x%02X,  value 0x%08X\n", 0x02, axi2miu2_data);
    CWA_MSG(CWA_MSG_DBG, "base_axi2miu3: addr 0x%02X,  value 0x%08X\n", 0x02, axi2miu3_data);
