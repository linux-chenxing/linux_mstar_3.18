...
static ssize_t check_fbmg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t n)
{
    ...
    if(NULL!=buf)
    {
        ...
        {
            SCL_ERR( "[FB]Set %d\n",(int)*str);
            stFbMgCfg.enSet = EN_IOCTL_HVSP_FBMG_SET_PRVCROP_ON;
            MDrv_HVSP_SetFbManageConfig(stFbMgCfg.enSet);
        }
        else if((int)*str == 66)  //input B
        {
            SCL_ERR( "[FB]Set %d\n",(int)*str);
            stFbMgCfg.enSet = EN_IOCTL_HVSP_FBMG_SET_PRVCROP_OFF;
            MDrv_HVSP_SetFbManageConfig(stFbMgCfg.enSet);
        }
        else if((int)*str == 67)  //input C
        {
            SCL_ERR( "[FB]Set %d\n",(int)*str);
            gbCIIR = 1;
            stFbMgCfg.enSet = EN_IOCTL_HVSP_FBMG_SET_CIIR_ON;
            MDrv_HVSP_SetFbManageConfig(stFbMgCfg.enSet);
        }
        else if((int)*str == 68)  //input D
        {
            SCL_ERR( "[FB]Set %d\n",(int)*str);
            gbCIIR = 0;
            stFbMgCfg.enSet = EN_IOCTL_HVSP_FBMG_SET_CIIR_OFF;
            MDrv_HVSP_SetFbManageConfig(stFbMgCfg.enSet);
        }
...
int _mdrv_ms_hvsp1_io_set_primask_config(struct file *filp, unsigned long arg)
{
    ...
    ST_MDRV_HVSP_VERSIONCHK_CONFIG stVersion;
    EN_MDRV_MULTI_INST_STATUS_TYPE enMultiInstRet;
    stVersion = _mdrv_ms_hvsp1_io_fill_versionchkstruct(sizeof(ST_IOCTL_HVSP_PRIMASK_CONFIG),
        (((ST_IOCTL_HVSP_PRIMASK_CONFIG __user *)arg)->VerChk_Size),
        &(((ST_IOCTL_HVSP_PRIMASK_CONFIG __user *)arg)->VerChk_Version));
    if(_mdrv_ms_hvsp1_io_version_check(stVersion))
    {
        SCL_ERR( "[HVSP1]   %s  \n", __FUNCTION__);
        return -EINVAL;
    ...
    {
        if(copy_from_user(&stIOCfg, (ST_IOCTL_HVSP_PRIMASK_CONFIG __user *)arg, sizeof(ST_IOCTL_HVSP_PRIMASK_CONFIG)))
        {
            return -EFAULT;
        }
        else
...
int _mdrv_ms_hvsp1_io_primask_trigger_config(struct file *filp, unsigned long arg)
{
    ...
    stVersion = _mdrv_ms_hvsp1_io_fill_versionchkstruct(sizeof(ST_IOCTL_HVSP_PRIMASK_TRIGGER_CONFIG),
        (((ST_IOCTL_HVSP_PRIMASK_TRIGGER_CONFIG __user *)arg)->VerChk_Size),
        &(((ST_IOCTL_HVSP_PRIMASK_TRIGGER_CONFIG __user *)arg)->VerChk_Version));
    if(_mdrv_ms_hvsp1_io_version_check(stVersion))
    {
        SCL_ERR( "[HVSP1]   %s  \n", __FUNCTION__);
        return -EINVAL;
    ...
    {
        if(copy_from_user(&stIOCfg, (ST_IOCTL_HVSP_PRIMASK_TRIGGER_CONFIG __user *)arg, sizeof(ST_IOCTL_HVSP_PRIMASK_TRIGGER_CONFIG)))
        {
            return -EFAULT;
        }
        else
...
static unsigned int mdrv_ms_hvsp1_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    if(enMultiInstRet == E_MDRV_MULTI_INST_STATUS_SUCCESS)
    {
        ...
        if(MDrv_HVSP_GetCMDQDoneStatus(E_MDRV_HVSP_ID_1))
        {
            ret = POLLIN;
        }
        else
