...
int _mdrv_ms_scldma4_io_set_lock_config(struct file *filp, unsigned long arg)
{
    ...
    ST_MDRV_SCLDMA_VERSIONCHK_CONFIG stVersion;
    static signed long *ps32IdBuf[4];
    static int idx = 0;
    int i= 0;
    int diff= 0;
    ...
    {
        ps32IdBuf[idx] = stCfg.ps32IdBuf;
        for(i = 0;i<idx;i++)
        {
            if(ps32IdBuf[i]!=stCfg.ps32IdBuf)
            {
                diff = 1;
            }
            else
        ...
        if(diff || (idx==0))
        {
            SCL_ERR( "[SCLDMA4]multiinst unlock buffer:%lx\n",(unsigned long)stCfg.ps32IdBuf);
            idx ++;
            diff = 0;
        }
    }
...
int _mdrv_ms_scldma4_io_set_unlock_config(struct file *filp, unsigned long arg)
{
    ...
    else
    {
        ...
        if(diff || (idx==0))
        {
            SCL_ERR( "[SCLDMA1]multiinst unlock buffer:%lx\n",(unsigned long)stCfg.ps32IdBuf);
            idx ++;
            diff = 0;
        }
    }
