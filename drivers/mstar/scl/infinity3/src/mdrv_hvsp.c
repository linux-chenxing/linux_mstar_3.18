...
unsigned char MDrv_HVSP_SetInitIPMConfig(EN_MDRV_HVSP_ID_TYPE enHVSP_ID, ST_MDRV_HVSP_IPM_CONFIG *pCfg)
{
    ...
    stLDCCfg = _MDrv_HVSP_FillLDCStruct(pCfg, (stIPMCfg.bYCMRead ? 0 :1));
...
ST_HVSP_PRIMASK_CONFIG _MDrv_HVSP_FillPriMaskConfig(ST_MDRV_HVSP_PRIMASK_CONFIG stCfg)
{
...
ST_MDRV_HVSP_PRIMASK_CONFIG _MDrv_HVSP_TransPercentToPriCfg
    (ST_MDRV_HVSP_PRIMASK_CONFIG stPerCfg ,ST_MDRV_HVSP_SCALING_CONFIG sthvspCfg)
{
...
ST_MDRV_HVSP_PRIMASK_CONFIG _MDrv_HVSP_TransPriCfgToPercent
    (ST_MDRV_HVSP_PRIMASK_CONFIG stCfg ,ST_MDRV_HVSP_SCALING_CONFIG sthvspCfg)
{
    ST_MDRV_HVSP_PRIMASK_CONFIG stPerCfg;
...
ST_HVSP_PRIMASK_CONFIG _MDrv_HVSP_GetPriMaskConfig
    (ST_MDRV_HVSP_PRIMASK_CONFIG stCfg, ST_MDRV_HVSP_SCALING_CONFIG sthvspCfg)
{
    ST_HVSP_PRIMASK_CONFIG stPriMaskCfg;
    //unsigned long u32WidthRatio;
    //unsigned long u32HeightRatio;
    ...
    stPriMaskCfg = _MDrv_HVSP_FillPriMaskConfig(stCfg);
    return stPriMaskCfg;
}
...
void _MDrv_HVSP_SetPriMaskStorageDynamic(ST_MDRV_HVSP_PRIMASK_CONFIG stPriMaskCfg)
{
    unsigned char u8Num;
    unsigned char bStage = 1;
    for(u8Num=0;u8Num<PriMaskNum;u8Num++)
    {
    ...
    if(bStage)
    {
        if(gu8PriMaskNum[gu8PriMaskInst] <PriMaskNum)
        {
            MDRV_HVSP_MUTEX_LOCK();
            MsOS_Memcpy(&gstHvspPriMaskCfg[gu8PriMaskInst][gu8PriMaskNum[gu8PriMaskInst]],&stPriMaskCfg,sizeof(ST_HVSP_PRIMASK_CONFIG));
            gu8PriMaskNum[gu8PriMaskInst] = _MDrv_HVSP_FindUnusePriMask();
            MDRV_HVSP_MUTEX_UNLOCK();
        }
        else
        {
...
unsigned char MDrv_HVSP_SetPostCropConfig(EN_MDRV_HVSP_ID_TYPE enHVSP_ID, ST_MDRV_HVSP_POSTCROP_CONFIG *pCfg)
{
    ...
    stRet = Drv_HVSP_SetScaling(enID, stScalingCfg,(ST_HVSP_CLK_CONFIG *)pCfg->stclk);
    ret = stRet.bRet;
    return  ret;
...
ST_MDRV_HVSP_SCALING_CONFIG _MDrv_HVSP_ChangebyZoomSizeLimitation
    (ST_MDRV_HVSP_SCALING_CONFIG stGCfg,ST_HVSP_SCALING_CONFIG stCfg )
{
    ST_MDRV_HVSP_SCALING_CONFIG stRet;
...
unsigned char MDrv_HVSP_SetScalingConfig(EN_MDRV_HVSP_ID_TYPE enHVSP_ID, ST_MDRV_HVSP_SCALING_CONFIG *pCfg )
{
   ...
   if(enID == E_HVSP_ID_1)
   {
    ...
    if(enID == E_HVSP_ID_1)
    {
        if(!ret)
        {
            stChange = _MDrv_HVSP_ChangebyZoomSizeLimitation(gstHvspScalingCfg,stRet);
        }
        else
        {
        ...
        if(gu8PriMaskNum[gu8PriMaskInst] && bChangePri)
        {
            ...
            for(idx=0;idx<gu8PriMaskNum[gu8PriMaskInst];idx++)
            {
                ...
                {
                    stPriMaskStore =
                        _MDrv_HVSP_TransPercentToPriCfg(gstHvspPriMaskCfg[gu8PriMaskInst][idx],stChange);
                    stPriMaskCfg = _MDrv_HVSP_GetPriMaskConfig(stPriMaskStore,stChange);
...
unsigned char MDrv_HVSP_SetMiscConfig(ST_MDRV_HVSP_MISC_CONFIG *pCfg)
{
    ...
    if(MsOS_copy_from_user(pBuf, (ST_MDRV_HVSP_MISC_CONFIG  __user *)pCfg->u32Addr, pCfg->u32Size))
    {
...
unsigned char MDrv_HVSP_SetPriMaskConfig(ST_MDRV_HVSP_PRIMASK_CONFIG stCfg)
{
        ...
        gstHvspPriMaskCfg[gu8PriMaskInst][stCfg.u8idx].bMask)
    {
        ...
        for(u8idx = 0 ; u8idx<PriMaskNum; u8idx++)
        {
            ...
            {
                stPriMaskStore =
                    _MDrv_HVSP_TransPercentToPriCfg(gstHvspPriMaskCfg[gu8PriMaskInst][u8idx],gstHvspScalingCfg);
                stPriMaskCfg = _MDrv_HVSP_GetPriMaskConfig(stPriMaskStore,gstHvspScalingCfg);
                Drv_HVSP_SetPriMaskConfig(stPriMaskCfg);
            }
        }
    }
    stPriMaskStore = _MDrv_HVSP_TransPriCfgToPercent(stCfg,gstHvspScalingCfg);
    _MDrv_HVSP_SetPriMaskStorage(stPriMaskStore);
    stPriMaskCfg = _MDrv_HVSP_GetPriMaskConfig(stCfg,gstHvspScalingCfg);
...
ssize_t MDrv_HVSP_SCIQShow(char *buf, EN_MDRV_HVSP_ID_TYPE enHVSP_ID)
{
    ...
    Drv_HVSP_GetHVSPAttribute(enHVSP_ID,&sthvspformCfg);
