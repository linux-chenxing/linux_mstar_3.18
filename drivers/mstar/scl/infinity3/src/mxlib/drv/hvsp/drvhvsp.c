...
#define SRAMFORSCALDOWN 0
#define SRAMFORSCALUP 1
#define SRAMFORSC3HDOWN 0x10
#define SRAMFORSC3HUP 0x11
#define SRAMFORSC3VDOWN 0x0
#define SRAMFORSC3VUP 0x01
...
#define HVSP_RATIO(input, output)           ((MS_U32)((MS_U64)((MS_U32)(input) * 1048576) / (MS_U32)(output)))
...
void _Drv_HVSP_InitSWVarialbe(EN_HVSP_ID_TYPE HVSP_IP)
{
    ...
    if(HVSP_IP == E_HVSP_ID_1)
    {
        ...
        gbLDCEn = 0;
        gbPriMaskPending = 0;
        gbVScalingup[HVSP_IP] = 1;
        gbHScalingup[HVSP_IP] = 1;
...
MS_BOOL Drv_HVSP_GetCMDQDoneStatus(EN_HVSP_POLL_ID_TYPE enPollId)
{
    ...
    {
        MsOS_WaitEvent(Drv_SCLIRQ_Get_IRQ_SYNCEventID(),E_SCLIRQ_EVENT_CMDQDONE, &u32Events, E_OR, 100);
    }
    if(sbDone&&(MsOS_GetEvent(Drv_SCLIRQ_Get_IRQ_SYNCEventID())&(E_SCLIRQ_EVENT_CMDQFIRE|E_SCLIRQ_EVENT_CMDQ)))
    {
        DRV_HVSP_MUTEX_LOCK();
        sbDone = 0;
...
void _Drv_HVSP_SetHWinit(void)
{
    ...
    Hal_HVSP_SRAM_Dump(EN_HVSP_SRAM_DUMP_HVSP_V_1,SRAMFORSCALDOWN);
    Hal_HVSP_SRAM_Dump(EN_HVSP_SRAM_DUMP_HVSP_V_2,SRAMFORSC3VDOWN);
    Hal_HVSP_SRAM_Dump(EN_HVSP_SRAM_DUMP_HVSP_H,SRAMFORSCALUP);
    Hal_HVSP_SRAM_Dump(EN_HVSP_SRAM_DUMP_HVSP_H_1,SRAMFORSCALDOWN);
...
MS_U8 _Drv_HVSP_GetScalingHRatioConfig(EN_HVSP_ID_TYPE enHVSP_ID,MS_U8 bUp)
{
    ...
    if(bUp)
    {
        if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_H >= (MS_U32)HVSP_RATIO(3,4))
        {
            bret = 0;
        }
        else if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_H >= (MS_U32)HVSP_RATIO(2,4))
        {
            bret = 1;
        }
        else if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_H >= (MS_U32)HVSP_RATIO(1,4))
        {
            bret = 2;
        }
        else
    ...
    {
        if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_H <= (MS_U32)HVSP_RATIO(4,3))
        {
            bret = 0;
        }
        else if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_H <= (MS_U32)HVSP_RATIO(4,2))
        {
            bret = 1;
        }
        else if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_H <= (MS_U32)HVSP_RATIO(4,1))
        {
            bret = 2;
        }
        else
...
MS_U8 _Drv_HVSP_GetScalingVRatioConfig(EN_HVSP_ID_TYPE enHVSP_ID,MS_U8 bUp)
{
    ...
    if(bUp)
    {
        if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_V > (MS_U32)HVSP_RATIO(3,4))
        {
            bret = 0;
        }
        else if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_V > (MS_U32)HVSP_RATIO(2,4))
        {
            bret = 1;
        }
        else if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_V > (MS_U32)HVSP_RATIO(1,4))
        {
            bret = 2;
        }
        else
    ...
    {
        if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_V <= (MS_U32)HVSP_RATIO(4,3))
        {
            bret = 0;
        }
        else if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_V <= (MS_U32)HVSP_RATIO(4,2))
        {
            bret = 1;
        }
        else if(gstScalinInfo[enHVSP_ID].u32ScalingRatio_V <= (MS_U32)HVSP_RATIO(4,1))
        {
            bret = 2;
        }
        else
...
void _Drv_HVSP_SetHorizotnalSRAMTbl(EN_HVSP_ID_TYPE enHVSP_ID)
{
    ...
    if((_Change_SRAM_H_Qmap(enHVSP_ID,(gbHScalingup[enHVSP_ID]&0x1))))
    {
        ...
        if(Drv_SCLIRQ_GetIsBlankingRegion()|| enHVSP_ID ==E_HVSP_ID_3)
        {
            ...
            Hal_HVSP_SRAM_Dump(enHVSP_ID+HVSP_ID_SRAM_H_OFFSET,gbHScalingup[enHVSP_ID]); //level 1 :up 0:down
...
void _Drv_HVSP_SetVerticalSRAMTbl(EN_HVSP_ID_TYPE enHVSP_ID)
{
    ...
    if(_Change_SRAM_V_Qmap(enHVSP_ID,(gbVScalingup[enHVSP_ID]&0x1)))
    {
        ...
        if(Drv_SCLIRQ_GetIsBlankingRegion()|| enHVSP_ID ==E_HVSP_ID_3)
        {
            ...
            Hal_HVSP_SRAM_Dump(enHVSP_ID+HVSP_ID_SRAM_V_OFFSET,gbVScalingup[enHVSP_ID]); //level 1 :up 0:down
            SCL_DBG(SCL_DBG_LV_DRVHVSP()&(Get_DBGMG_HVSP(enHVSP_ID)),
...
ST_HVSP_CROP_INFO _Drv_HVSP_SetCrop(MS_BOOL u8CropID,ST_HVSP_SCALING_CONFIG stCfg)
{
    ST_HVSP_CROP_INFO stCropInfo;
    MS_U16 u16In_hsize;
    MS_U16 u16In_vsize;
...
ST_HVSP_SCALING_CONFIG _Drv_HVSP_SetScalingRetVal(ST_HVSP_CROP_INFO stCropInfo_1, ST_HVSP_CROP_INFO stCropInfo_2)
{
    ST_HVSP_SCALING_CONFIG stRet;
...
ST_HVSP_SCALING_CONFIG Drv_HVSP_SetScaling(EN_HVSP_ID_TYPE enHVSP_ID, ST_HVSP_SCALING_CONFIG stCfg, ST_HVSP_CLK_CONFIG* stclk)
{
    ...
    MS_U16 u16DSPWidth = 0;
    MS_U32 u32bLDCEvent = 0;
    u16DSPWidth = gstScalinInfo[enHVSP_ID].stSizeAfterScaling.u16Width;
    stCropInfo_1.bEn = 0;
    stCropInfo_2.bEn = 0;
    if(enHVSP_ID == E_HVSP_ID_1)
    {
        ...
        stCropInfo_1 = _Drv_HVSP_SetCrop(DRV_HVSP_CROP_1,stCfg);
        stCropInfo_2 = _Drv_HVSP_SetCrop(DRV_HVSP_CROP_2,stCfg);
            ...
            (enISPClkType, stCropInfo_1.u16Hsize ,stCfg.u16Dsp_Height)))
        {
            if(stCfg.bCropEn[DRV_HVSP_CROP_1])
            {
                ...
                if(_Is_Src5MSize())
                {
                    stCfg.u16Crop_Width[DRV_HVSP_CROP_1] =
                        Hal_HVSP_GetLimitationByISPClk(enISPClkType, stCropInfo_1.u16Hsize ,stCfg.u16Dsp_Width) ;
                    stCropInfo_1.u16Hsize= stCfg.u16Crop_Width[DRV_HVSP_CROP_1];
                    stCropInfo_1.u16Hst= ((stCropInfo_1.u16Hst+stCropInfo_1.u16Hsize)>stCropInfo_1.u16In_hsize) ?
                        (stCropInfo_1.u16In_hsize - stCropInfo_1.u16Hsize) : stCropInfo_1.u16Hst;
                    gstIPMCfg.u16Fetch= stCropInfo_1.u16Hsize;
                    stCropInfo_2.u16In_hsize= gstIPMCfg.u16Fetch;
                    stCropInfo_2.u16Hsize= gstIPMCfg.u16Fetch;//stCfg.u16Src_Height;
                }
            ...
            if(stCfg.bCropEn[DRV_HVSP_CROP_2])
            {
                ...
                if(_Is_Src5MSize())
                {
                    stCfg.u16Crop_Width[DRV_HVSP_CROP_2] =
                        Hal_HVSP_GetLimitationByISPClk(enISPClkType, stCropInfo_1.u16Hsize ,stCfg.u16Dsp_Width) ;
                    stCropInfo_1.u16Hsize= stCfg.u16Crop_Width[DRV_HVSP_CROP_2];
                    stCropInfo_1.u16Hst= ((stCropInfo_1.u16Hst+stCropInfo_1.u16Hsize)>stCropInfo_1.u16In_hsize) ?
                        (stCropInfo_1.u16In_hsize - stCropInfo_1.u16Hsize) : stCropInfo_1.u16Hst;
                }
                SCL_ERR("[DRVHVSP]%s %d:: Buffer num : %hhd ,Zoom size bigger than Limitation and force to change(%hd,%hd,%hd,%hd)\n"
            ...
            stRet = _Drv_HVSP_SetScalingRetVal(stCropInfo_1,stCropInfo_2);
            stRet.bRet = 0;
        }
        else if((_Is_SingleBufferAndZoomSizeBiggerthanLimitation
            (u8FBbuffer,stCropInfo_2.u16Hsize, Hal_HVSP_GetLimitationByISPClk
            (enISPClkType, stCropInfo_1.u16Hsize ,stCfg.u16Dsp_Width)))&&(_Is_Src5MSize()))
        {
            if(stCfg.bCropEn[DRV_HVSP_CROP_1])
            {
                stCfg.u16Crop_Width[DRV_HVSP_CROP_1] =
    ...
    else
    {
        if(_IsChangeFBBufferResolution(u16FBWidth,u16FBHeight) || bChangePreCropPosition)
        {
            if(u32bLDCEvent)
            {
                ...
                _Drv_HVSP_SetCMDQTrigFire(stCfg.stCmdTrigCfg,enHVSP_ID);
                MsOS_WaitEvent(Drv_SCLIRQ_Get_IRQ_SYNCEventID(), E_SCLIRQ_EVENT_CMDQDONE, &u32bLDCEvent, E_OR, 100);
                Hal_HVSP_SetLDCPathSel(1);
            }
            else
            {
...
MS_U32 Drv_HVSP_GetInputSrcMux(void)
{
...
ST_HVSP_SCINTS_TYPE Drv_HVSP_GetSCLInts(void)
{
    ST_SCLIRQ_SCINTS_TYPE stints;
    ST_HVSP_SCINTS_TYPE sthvspints;
...
void Drv_HVSP_SCLIQ(EN_HVSP_ID_TYPE enID,EN_DRV_HVSP_IQ_TYPE enIQ)
{
    ...
    switch(enIQ)
    {
        case EN_DRV_HVSP_IQ_H_Tbl0:
            ...
            Drv_HVSP_SetCMDQTrigType(stCmdTrigCfg);
            DRV_HVSP_MUTEX_UNLOCK();
            Hal_HVSP_SRAM_Dump(enID+HVSP_ID_SRAM_H_OFFSET,gbHScalingup[enID]&0x1); //level 1 :up 0:down
        ...
        case EN_DRV_HVSP_IQ_H_Tbl1:
            ...
            Drv_HVSP_SetCMDQTrigType(stCmdTrigCfg);
            DRV_HVSP_MUTEX_UNLOCK();
            Hal_HVSP_SRAM_Dump(enID+HVSP_ID_SRAM_H_OFFSET,(0x10 |(gbHScalingup[enID]&0x1))); //level 1 :up 0:down
        ...
        case EN_DRV_HVSP_IQ_H_Tbl2:
            ...
            Drv_HVSP_SetCMDQTrigType(stCmdTrigCfg);
            DRV_HVSP_MUTEX_UNLOCK();
            Hal_HVSP_SRAM_Dump(enID+HVSP_ID_SRAM_H_OFFSET,(0x20 |(gbHScalingup[enID]&0x1))); //level 1 :up 0:down
        ...
        case EN_DRV_HVSP_IQ_H_Tbl3:
            ...
            Drv_HVSP_SetCMDQTrigType(stCmdTrigCfg);
            DRV_HVSP_MUTEX_UNLOCK();
            Hal_HVSP_SRAM_Dump(enID+HVSP_ID_SRAM_H_OFFSET,(0x30 |(gbHScalingup[enID]&0x1))); //level 1 :up 0:down
        ...
        case EN_DRV_HVSP_IQ_V_Tbl0:
            ...
            Drv_HVSP_SetCMDQTrigType(stCmdTrigCfg);
            DRV_HVSP_MUTEX_UNLOCK();
            Hal_HVSP_SRAM_Dump(enID+HVSP_ID_SRAM_V_OFFSET,gbVScalingup[enID]&0x1); //level 1 :up 0:down
        ...
        case EN_DRV_HVSP_IQ_V_Tbl1:
            ...
            Drv_HVSP_SetCMDQTrigType(stCmdTrigCfg);
            DRV_HVSP_MUTEX_UNLOCK();
            Hal_HVSP_SRAM_Dump(enID+HVSP_ID_SRAM_V_OFFSET,(0x10 |(gbVScalingup[enID]&0x1))); //level 1 :up 0:down
        ...
        case EN_DRV_HVSP_IQ_V_Tbl2:
            ...
            Drv_HVSP_SetCMDQTrigType(stCmdTrigCfg);
            DRV_HVSP_MUTEX_UNLOCK();
            Hal_HVSP_SRAM_Dump(enID+HVSP_ID_SRAM_V_OFFSET,(0x20 |(gbVScalingup[enID]&0x1))); //level 1 :up 0:down
        ...
        case EN_DRV_HVSP_IQ_V_Tbl3:
            ...
            Drv_HVSP_SetCMDQTrigType(stCmdTrigCfg);
            DRV_HVSP_MUTEX_UNLOCK();
            Hal_HVSP_SRAM_Dump(enID+HVSP_ID_SRAM_V_OFFSET,(0x30 |(gbVScalingup[enID]&0x1))); //level 1 :up 0:down
...
void Drv_HVSP_SetLDCONOFF(MS_BOOL bEn)
{
    ...
    if(Hal_HVSP_GetLDCPathSel()!= bEn)
    {
        ...
        {
            MsOS_WaitEvent(Drv_SCLIRQ_Get_IRQ_SYNCEventID(), E_SCLIRQ_EVENT_FRMENDSYNC, &u32Events, E_OR, 100); // get status: FRM END
        }
...
MS_BOOL Drv_HVSP_SetPatTgen(MS_BOOL bEn, ST_DRV_HVSP_PAT_TGEN_CONFIG *pCfg)
{
    ...
    if(bEn)
    {
        ...
        if(pCfg)
        {
            ...
            {
                Hal_HVSP_SetPatTgVtt(1800); //rotate
            }
