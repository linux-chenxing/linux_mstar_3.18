...
{
    if(MsOS_GetEvent(_s32SYNCEventId) & E_SCLIRQ_EVENT_CMDQDONE)
    {
        return gbInBlanking;
    }
    else if(gbInBlanking && (MsOS_GetEvent(_s32SYNCEventId) & E_SCLIRQ_EVENT_CMDQGOING))
    {
        return 0;
    }
    else
...
static int SCLIRQ_DazaIST(void *arg)
{
    ...
    if(enDAZAEvent &E_SCLIRQ_DAZA_BCLOSELDCP)
    {
        ...
        Hal_SCLDMA_SetSC1DMAEn(E_SCLDMA_IMI_W,0);
        Hal_SCLDMA_SetSC1DMAEn(E_SCLDMA_IMI_W,1);
        Hal_SCLIRQ_Set_Reg(REG_SCL0_01_L, BIT3, BIT3);
        Hal_SCLIRQ_Set_Reg(REG_SCL0_01_L, 0, BIT3);
...
static int SCLIRQ_CMDQIST(void *arg)
{
    ...
    {
        Drv_HVSP_SetSCIQHSRAM(E_HVSP_ID_1);
        MsOS_ClearEventIRQ(_s32SYNCEventId,(E_SCLIRQ_EVENT_SC1HSRAMSET));
    }
    if(u32Events & E_SCLIRQ_EVENT_SC1VSRAMSET)
    {
        Drv_HVSP_SetSCIQVSRAM(E_HVSP_ID_1);
        MsOS_ClearEventIRQ(_s32SYNCEventId,(E_SCLIRQ_EVENT_SC1VSRAMSET));
    }
    if(u32Events & E_SCLIRQ_EVENT_SC2HSRAMSET)
    {
        Drv_HVSP_SetSCIQHSRAM(E_HVSP_ID_2);
        MsOS_ClearEventIRQ(_s32SYNCEventId,(E_SCLIRQ_EVENT_SC2HSRAMSET));
    }
    if(u32Events & E_SCLIRQ_EVENT_SC2VSRAMSET)
    {
        Drv_HVSP_SetSCIQVSRAM(E_HVSP_ID_2);
        MsOS_ClearEventIRQ(_s32SYNCEventId,(E_SCLIRQ_EVENT_SC2VSRAMSET));
    }
    if(u32Events & E_SCLIRQ_EVENT_SC3HSRAMSET)
    {
        Drv_HVSP_SetSCIQHSRAM(E_HVSP_ID_3);
        MsOS_ClearEventIRQ(_s32SYNCEventId,(E_SCLIRQ_EVENT_SC3HSRAMSET));
    }
    if(u32Events & E_SCLIRQ_EVENT_SC3VSRAMSET)
    {
        Drv_HVSP_SetSCIQVSRAM(E_HVSP_ID_3);
        MsOS_ClearEventIRQ(_s32SYNCEventId,(E_SCLIRQ_EVENT_SC3VSRAMSET));
    }
...
{
    static MS_BOOL bVsyncWithVend = 0;
    static MS_BOOL bLDCEvenOpen = 0;
    static MS_BOOL bLDCEvenOpenCount = 0;
    ...
    if(bVsyncWithVend>3 && (Hal_SCLIRQ_Get_RegVal(REG_SCL0_03_L)&BIT12))
    {
...
{
    MS_U32 u32Events;
    MsOS_WaitEvent(Drv_SCLIRQ_Get_IRQ_SYNCEventID(), E_SCLIRQ_EVENT_CMDQDONE, &u32Events, E_OR, 100); // get status: CMDQ Done
    DRV_SCLIRQ_MUTEX_LOCK_ISR();
...
MS_BOOL Drv_SCLIRQ_Init(ST_SCLIRQ_INIT_CONFIG *pCfg)
{
    ...
    if(MsOS_AttachInterrupt(pCfg->u32CMDQIRQNUM, (InterruptCb)_Drv_SCLIRQ_CMDQ_isr ,NULL, "CMDQINTR"))
    {
        DRV_SCLIRQ_ERR(printf("[DRVSCLIRQ]%s(%d):: Request IRQ Fail\n", __FUNCTION__, __LINE__));
        return FALSE;
    }
