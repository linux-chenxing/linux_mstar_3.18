...
static int SCLIRQ_IST(void *arg)
{
    ...
    while(gstThreadCfg.flag == 1)
    {
        ...
        _Drv_SCLIRQ_SetSingleModeDMAInfo(E_SCLDMA_2_FRM2_W,u32Events);
...
irqreturn_t  _Drv_SCLIRQ_isr(int eIntNum, void* dev_id)
{
    ...
    if(_bSCLIRQ_Suspend)
    {
        Hal_SCLIRQ_Set_Clear(EN_SCLIRQ_SCTOP_0,0xFFFFFFFFFF, 1);
    }
    ...
    if(u64Flag & SCLIRQ_MSK_SC2_FRM_W_ACTIVE_N)
    {
        ...
        _Drv_SCLIRQ_DMAActive_NHandler(E_SCLDMA_2_FRM2_W);
    }
    // SC1 SNP
