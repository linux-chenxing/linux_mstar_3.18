        ...
        if (MsOS_clk_get_enable_count(stclk->fclk1) == 0)
        {
...
#endif
            {
                if (NULL != (stclock = MsOS_clk_get_parent_by_index(stclk->fclk1, 0)))
                {
                    ...
                    //printf("[SCLDMA]flag:%ld index:%hx\n",MsOS_clk_get_rate(stclk->fclk1),regclk);
                }
                else
                {
...
void Hal_SCLDMA_SetSC1DMAConfig(ST_SCLDMA_RW_CONFIG stSCLDMACfg)
{
    ...
    {
        W2BYTEMSK(REG_SCL_DMA0_1B_L, Hal_GetHVSPOutputVSize(E_SCLDMA_ID_1_W), 0xFFFF);
        W2BYTEMSK(REG_SCL_DMA1_4B_L, Hal_GetHVSPOutputVSize(E_SCLDMA_ID_1_W), 0xFFFF);
        W2BYTEMSK(REG_SCL_DMA0_4B_L, Hal_GetHVSPOutputVSize(E_SCLDMA_ID_1_W), 0xFFFF);
    }
...
void Hal_SCLDMA_SetSC2DMAConfig(ST_SCLDMA_RW_CONFIG stSCLDMACfg)
{
    ...
    if(Hal_GetHVSPOutputHSize(E_SCLDMA_ID_2_W)==stSCLDMACfg.u16Width ||(Hal_GetHVSPOutputHSize(E_SCLDMA_ID_2_W) == 0))
    {
        ...
        W2BYTEMSK(REG_SCL_DMA0_7A_L, stSCLDMACfg.u16Width, 0xFFFF);
    }
    else
    {
        W2BYTEMSK(REG_SCL_DMA0_62_L, Hal_GetHVSPOutputHSize(E_SCLDMA_ID_2_W), 0xFFFF);
        W2BYTEMSK(REG_SCL_DMA1_62_L, Hal_GetHVSPOutputHSize(E_SCLDMA_ID_2_W), 0xFFFF);
        W2BYTEMSK(REG_SCL_DMA0_7A_L, Hal_GetHVSPOutputHSize(E_SCLDMA_ID_2_W), 0xFFFF);
    }
    if(Hal_GetHVSPOutputVSize(E_SCLDMA_ID_2_W) == stSCLDMACfg.u16Height ||(Hal_GetHVSPOutputVSize(E_SCLDMA_ID_2_W) == 0))
    {
        ...
        W2BYTEMSK(REG_SCL_DMA0_7B_L, stSCLDMACfg.u16Height,0xFFFF);
    }
    else
    {
        W2BYTEMSK(REG_SCL_DMA0_63_L, Hal_GetHVSPOutputVSize(E_SCLDMA_ID_2_W), 0xFFFF);
        W2BYTEMSK(REG_SCL_DMA1_63_L, Hal_GetHVSPOutputVSize(E_SCLDMA_ID_2_W), 0xFFFF);
        W2BYTEMSK(REG_SCL_DMA0_7B_L, Hal_GetHVSPOutputVSize(E_SCLDMA_ID_2_W), 0xFFFF);
    }
}
