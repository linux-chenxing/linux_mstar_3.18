...
MS_U16 Hal_VIP_GetWDRTvalue(MS_U8 u8sec, MS_U16 u16tcount, MS_U32 u32Sram)
{
    MS_U16 u16tvalue;
    MS_U8 *p8buffer;
    MS_U32 u32Val = 0;
...
void Hal_VIP_SRAMDumpWDR(MS_U32 u32reg, MS_U32 u32Sram)
{
    MS_U8  u8sec = 0;
    ...
    for(u8sec=0;u8sec<WDR_SRAM_NUM;u8sec++)
    {
        ...
        for(u16addr=0;u16addr<u16addrmax;u16addr++)
        {
            ...
            Hal_VIP_WriteReg(u32reg+2, (MS_U16)u16tvalue, 0x3FF);//data
            Hal_VIP_WriteReg(u32reg+4, u16addr|BIT8, 0xFFFF);//wen
...
MS_U16 Hal_VIP_GetGammaYUVTvalue(MS_U16 u16sec, MS_U16 u16tcount,MS_U32 u32Sram)
{
    MS_U16 u16tvalue;
    MS_U8 *p8buffer;
    if(u32Sram)
    {
        p8buffer= (MS_U8 *)u32Sram;
        u16tvalue = (MS_U16)(*(p8buffer+u16tcount) | (*(p8buffer+u16tcount+1)<<8));
    }
    else
    {
        switch(u16sec)
...
MS_BOOL Hal_VIP_SRAMDumpGammaYUV(EN_VIP_SRAM_DUMP_TYPE endump,MS_U32 u32Sram,MS_U32 u32reg)
{
    MS_U16  u16sec = 0;
    MS_U8  bRet = 1;
    MS_U16  u16chkodd = 0;
    ...
    for(u16addr=0;u16addr<u16size;u16addr++)
    {
        ...
        u16tvalueeven = Hal_VIP_GetGammaYUVTvalue(u16sec, u16tcount,u32Sram);
        u16tvalueodd = Hal_VIP_GetGammaYUVTvalue(u16sec, u16tcount+2,u32Sram);
        if(bRet)
        {
...
MS_BOOL Hal_VIP_GetSRAMDumpGammaYUVCallback(EN_VIP_SRAM_DUMP_TYPE endump,MS_U32 u32Sram,MS_U32 u32reg)
{
    MS_U16  u16sec = 0;
    u8 flag = 0;
...
MS_BOOL Hal_VIP_GetSRAMDumpGammaYUV(EN_VIP_SRAM_DUMP_TYPE endump,MS_U32 u32Sram,MS_U32 u32reg)
{
    MS_U16  u16sec = 0;
    u8 flag = 0;
    MS_U16 u16addr = 0,u16tcount = 0,u16readdata = 0,u16readdataodd = 0,u16tvalueodd = 0,u16tvalueeven = 0,u16RegMask,u16size;
...
MS_U16 Hal_VIP_GetGammaRGBTvalue(MS_U16 u16sec, MS_U16 u16tcount,MS_U32 u32Sram,MS_U8  u8type)
{
    MS_U16 u16tvalue;
    MS_U8 *p8buffer;
    if(u32Sram)
...
MS_U8 Hal_VIP_SRAMDumpGammaRGB(EN_VIP_SRAM_DUMP_TYPE endump,MS_U32 u32Sram,MS_U32 u32reg)
{
    MS_U16  u16sec = 0;
    MS_U8  u8type = 0;
    MS_U8  bRet = 1;
...
MS_BOOL Hal_VIP_GetSRAMDumpGammaRGBCallback(EN_VIP_SRAM_DUMP_TYPE endump,u32 u32Sram,u32 u32reg)
{
    MS_U16  u16sec = 0;
    MS_U8  u8type = 0;
    MS_U8 flag = 0;
    ...
    for(u16addr=0;u16addr<u16size;u16addr++)
    {
        ...
        if(u16readdata != u16tvalueeven)
        {
            ...
            SCL_DBGERR("[ISRCheck]RGBGamma:%hhx read val:%hx odd val:%hx ckodd val:%hx\n"
                ,u16sec+u8type,u16readdata,u16tvalueodd,u16chkodd);
            if(!bRet)
            {
                Hal_VIP_SetWriteRegType(1);
                flag = gbCMDQ;
                ...
                Hal_VIP_WriteReg(u32reg+2, (u16)u16tvalueeven, 0xFFF);//data
                Hal_VIP_WriteReg(u32reg, ((u16addr)|((u16)u16sec<<8)|BIT11), 0xFFFF);//wen
                Hal_VIP_WriteReg(u32reg, ((u16addr)|((u16)u16sec<<8)|BIT12), 0xFFFF);//ren
                Hal_VIP_SetWriteRegType(0);
            }
        }
        Hal_VIP_WriteReg(u32reg, u16addr, 0x007F);//addr
        ...
        if(u16readdata != u16tvalueodd)
        {
            ...
            SCL_DBGERR("[ISRCheck]RGBGamma:%hhx read val:%hx odd val:%hx ckeven val:%hx\n"
                ,u16sec+u8type,u16readdata,u16chkodd,u16tvalueeven);
            if(!bRet)
            {
                Hal_VIP_SetWriteRegType(1);
                u16readdata=R2BYTE(u32reg+4);
                ...
                Hal_VIP_WriteReg(u32reg+2, (u16)u16tvalueodd, 0xFFF);//data
                Hal_VIP_WriteReg(u32reg, ((u16addr)|(((u16)u16sec+1)<<8)|BIT11), 0xFFFF);//wen
                Hal_VIP_WriteReg(u32reg, ((u16addr)|(((u16)u16sec+1)<<8)|BIT12), 0xFFFF);//ren
                Hal_VIP_SetWriteRegType(0);
            }
        }
        u16tcount+=4;
...
MS_BOOL Hal_VIP_GetSRAMDumpGammaRGB(EN_VIP_SRAM_DUMP_TYPE endump,MS_U32 u32Sram,MS_U32 u32reg)
{
    MS_U16  u16sec = 0;
    MS_U8  u8type = 0;
    u8 flag = 0;
...
MS_BOOL Hal_VIP_SetSRAMDump(EN_VIP_SRAM_DUMP_TYPE endump,MS_U32 u32Sram,MS_U32 u32reg)
{
...
MS_BOOL Hal_VIP_SRAM_Dump(EN_VIP_SRAM_DUMP_TYPE endump,MS_U32 u32Sram)
{
    MS_U16 u16clkreg;
    MS_BOOL bRet;
