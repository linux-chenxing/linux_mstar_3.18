...
#define HAL_HVSP_RATIO(input, output)           ((MS_U32)((MS_U64)((MS_U32)(input) * 1048576) / (MS_U32)(output)))
...
void Hal_HVSP_PriMask_Width(MS_U16 u16W)
{
    if(Is_Reg_Type(E_HVSP_CMD_TRIG_NONE) || Is_IP_First_Set(E_HVSP_ID_1) || Is_CannotUseCMDQToSetReg())
    {
        W2BYTEMSK(REG_VIP_SCNR_15_L, u16W-1, 0xFFFF);
    }
    else
    {
...
void Hal_HVSP_PriMask_Height(MS_U16 u16H)
{
    if(Is_Reg_Type(E_HVSP_CMD_TRIG_NONE) || Is_IP_First_Set(E_HVSP_ID_1) || Is_CannotUseCMDQToSetReg())
    {
        W2BYTEMSK(REG_VIP_SCNR_16_L, u16H-1, 0xFFFF);
    }
    else
    {
...
void Hal_HVSP_SetHVSPOutputSize(EN_HVSP_ID_TYPE enID, MS_U16 u16Width, MS_U16 u16Height)
{
    ...
    if(Is_Reg_Type(E_HVSP_CMD_TRIG_NONE) || Is_IP_First_Set(enID)|| Is_CannotUseCMDQToSetReg())
    {
        ...
        if(enID ==E_HVSP_ID_1)
        {
            W2BYTEMSK(REG_SCL_DMA0_1A_L, u16Width, 0xFFFF);
            W2BYTEMSK(REG_SCL_DMA0_1B_L, u16Height,0xFFFF);
            W2BYTEMSK(REG_SCL_DMA1_4A_L, u16Width, 0xFFFF);
            W2BYTEMSK(REG_SCL_DMA1_4B_L, u16Height,0xFFFF);
            W2BYTEMSK(REG_SCL_DMA0_4A_L, u16Width, 0xFFFF);
            W2BYTEMSK(REG_SCL_DMA0_4B_L, u16Height,0xFFFF);
        }
        else if(enID ==E_HVSP_ID_2)
        {
            W2BYTEMSK(REG_SCL_DMA0_62_L, u16Width, 0xFFFF);
            W2BYTEMSK(REG_SCL_DMA0_63_L, u16Height,0xFFFF);
            W2BYTEMSK(REG_SCL_DMA1_62_L, u16Width, 0xFFFF);
            W2BYTEMSK(REG_SCL_DMA1_63_L, u16Height,0xFFFF);
        }
    }
    else
    {
    ...
    if (u16SrcWidth<=1920)//for LDC issue
    {
        switch(enISPClkType)
        {
            ...
            case EN_HVSP_ISPCLK_144M:
                if(gstHalSrcSize.u16Height*gstHalSrcSize.u16Width >= 4500000 &&gstHalSrcSize.bSet)
                {
                    u16CropHeightLim = (u16DspHeight/50)*46;//90% 1.1X
                }
                else
                {
                    u16CropHeightLim = (u16DspHeight/20)*15;//75% 1.33X
...
EN_DRV_HVSP_OSD_LOC_TYPE Hal_HVSP_GetOSDLocate(EN_HVSP_ID_TYPE enID)
{
    ...
    if(enID == E_HVSP_ID_2)
    {
        bEn = (R2BYTE(REG_SCL2_64_L)& BIT1)>>1;
    }
    else if(enID == E_HVSP_ID_3)
    {
        bEn = (R2BYTE(REG_SCL2_68_L)& BIT1)>>1;
    }
    else
    {
...
{
    W2BYTEMSK(REG_VIP_SCNR_12_L, u16Y, 0xFFFF);
}
void Hal_HVSP_PriMask_ColorU(MS_U16 u16U)
{
    W2BYTEMSK(REG_VIP_SCNR_13_L, u16U, 0xFFFF);
}
void Hal_HVSP_PriMask_ColorV(MS_U16 u16V)
{
    W2BYTEMSK(REG_VIP_SCNR_14_L, u16V, 0xFFFF);
}
...
void Hal_HVSP_SetPrv2CropOnOff(MS_BOOL bEn)
{
    if(!LOCK_FB())
    {
        ...
        if(bEn)
        {
            if(Is_Reg_Type(E_HVSP_CMD_TRIG_NONE) || Is_IP_First_Set(E_HVSP_ID_1)|| Is_CannotUseCMDQToSetReg())
            {
                W2BYTEMSK(REG_VIP_MCNR_01_L, bEn ? BIT0 : 0, BIT0);
                if(!(R2BYTE(REG_SCL_DNR1_01_L)&BIT0) && bEn)
                {
            ...
            {
                _Hal_HVSP_Write_Cmd(REG_VIP_MCNR_01_L, bEn ? BIT0 : 0, BIT0);
                if(!(R2BYTE(REG_SCL_DNR1_01_L)&BIT0) && bEn)
                {
...
void Hal_HVSP_PriMask_bEn(MS_BOOL bEn)
{
    ...
    else if(bEn == EN_HVSP_TRIGGER_OFF)
    {
        ...
        else
        {
            ...
            {
                MsOS_WaitEvent(Drv_SCLIRQ_Get_IRQ_SYNCEventID(),
                    (E_SCLIRQ_EVENT_FRMENDSYNC), &u32Events, E_AND, 200); // get status: FRM END
            }
    ...
    {
        _Hal_HVSP_Write_Cmd(REG_VIP_SCNR_10_L,bEn ? BIT0 : 0, BIT0);
    }
}
