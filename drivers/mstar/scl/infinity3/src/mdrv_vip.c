...
void _MDrv_VIP_SetDAZATaskWork(VIPDAZAEvent enEvent)
{
    ...
    bret = MsOS_SetTaskWork(_MVIP_TASK);
...
void _Mdrv_VIP_ModifyMCNRAlreadySetting(ST_MDRV_VIP_SETPQ_CONFIG stSetPQCfg, EN_VIP_RESET_TYPE enRe)
{
...
void _MDrv_VIP_PrepareCheckSetting(EN_VIP_MDRV_CONFIG_TYPE enVIPtype)
{
    ...
    if(_IsCMDQAlreadySetting())
    {
        ...
        {
            _Mdrv_VIP_ModifyMCNRAlreadySetting(stSetPQCfg,EN_VIP_RESET_ZERO);
        }
        else
        {
    ...
    else if(_IsAutoSetting())
    {
        if(enVIPtype ==EN_VIP_MDRV_MCNR_CONFIG)
        {
            _Mdrv_VIP_ModifyMCNRAlreadySetting(stSetPQCfg,EN_VIP_FILL_GOLBAL_FULL);
        }
        else
        {
...
void _MDrv_VIP_SetPQByType(MS_U8 u8PQIPIdx, MS_U8 *pData)
{
    ...
    stSetPQCfg = _MDrv_VIP_FillPQCfgByType(u8PQIPIdx,pData,u16Structsize);
...
void _MDrv_VIP_SetMCNR(ST_MDRV_VIP_MCNR_CONFIG *pstCfg)
{
    ...
    _MDrv_VIP_SetPQByType(PQ_IP_MCNR_Main,(unsigned char *)pstCfg->u32Viraddr);
}
static int VIP_DazaIST(void *arg)
{
    ...
    while(1)
    {
        ...
        if(genVipDazaEvent)
        {
            ...
            {
                MsOS_WaitEvent(Drv_SCLIRQ_Get_IRQ_SYNCEventID(), E_SCLIRQ_EVENT_ISPFRMEND, &u32Events, E_OR, 200); // get status: FRM END
            ...
            else if(enDAZAEvent &E_VIP_DAZA_BGMA2C)
            {
                ...
                if(Drv_SCLIRQ_GetIsBlankingRegion())
                {
                    ...
                    _MDrv_VIP_SetPQByType(u16AIPsheet,(MS_U8 *)gstSupCfg.staip[EN_VIP_MDRV_AIP_GM10TO12].u32Viraddr);
            ...
            if(enDAZAEvent &E_VIP_DAZA_BNLM)
            {
                ...
                if(Drv_SCLIRQ_GetIsBlankingRegion())
                {
                    ...
                    MDRv_PQ_Set_CmdqCfg(0,0,0,1);
                    _MDrv_VIP_SetPQByType(PQ_IP_NLM_Main,(MS_U8 *)&gstSupCfg.stnlm.stNLM);
            ...
            else if(enDAZAEvent &E_VIP_DAZA_BGMC2A)
            {
                ...
                if(Drv_SCLIRQ_GetIsBlankingRegion())
                {
                    ...
                    MDRv_PQ_Set_CmdqCfg(0,0,0,1);
                    _Drv_CMDQ_WriteRegWithMaskDirect(REG_SCL0_01_L, BIT3, BIT3);
                    _MDrv_VIP_SetPQByType(u16AIPsheet,(MS_U8 *)gstSupCfg.staip[EN_VIP_MDRV_AIP_GM12TO10].u32Viraddr);
                    _Drv_CMDQ_WriteRegWithMaskDirect(REG_SCL0_01_L, 0, BIT3);
            ...
            if(enDAZAEvent &E_VIP_DAZA_BXNR)
            {
                ...
                if(Drv_SCLIRQ_GetIsBlankingRegion())
                {
                    ...
                    MDRv_PQ_Set_CmdqCfg(0,0,0,1);
                    _MDrv_VIP_SetPQByType(u16AIPsheet,(MS_U8 *)gstSupCfg.staip[EN_VIP_MDRV_AIP_XNR].u32Viraddr);
...
unsigned char _MDrv_VIP_SetBypassIP(unsigned long bBypass,EN_VIP_MDRV_CONFIG_TYPE enVIPtype)
{
    ...
    {
        case EN_VIP_MDRV_MCNR_CONFIG:
                stSetBypassCfg = MDrv_VIP_FillBasicStructSetPQCfg(enVIPtype,NULL);
                if(bBypass)
                {
                    _Mdrv_VIP_ModifyMCNRAlreadySetting(stSetBypassCfg,EN_VIP_RESET_ZERO);
                }
                else
                {
...
unsigned long _MDrv_VIP_FillSettingBuffer(MS_U16 u16PQIPIdx, MS_U32 u32pstViraddr, MS_U32 *u32gpViraddr)
{
    if(_IsNotToCheckPQorCMDQmode())
    {
        ...
        {
            if(MsOS_copy_from_user((void *)u32Viraddr, (void  __user *)u32ViraddrOri, u16StructSize))
            {
...
//  Function
//-------------------------------------------------------------------------------------------------
ST_MDRV_VIP_SETPQ_CONFIG MDrv_VIP_FillBasicStructSetPQCfg(EN_VIP_MDRV_CONFIG_TYPE enVIPtype,void *pPointToCfg)
{
    ST_MDRV_VIP_SETPQ_CONFIG stSetPQCfg;
...
void MDrv_VIP_FreeMemory(void)
{
    ...
    for(enVIPtype = 0;enVIPtype<EN_VIP_MDRV_AIP_NUM;enVIPtype++)
    {
...
unsigned char MDrv_VIP_Init(ST_MDRV_VIP_INIT_CONFIG *pCfg)
{
    ...
    _MVIP_TASK  = MsOS_CreateTask((TaskEntry)VIP_DazaIST,(MS_U32)NULL,TRUE,(char*)"VIPDazaTask");
    if (_MVIP_TASK == -1)
    {
...
unsigned char MDrv_VIP_SetPeakingConfig(void *pvCfg)
{
    ...
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_HLPF_Main,PQ_IP_VIP_Peaking_gain_ad_Y_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetDLCConfig(void *pvCfg)
{
    ...
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_DLC_His_range_Main,PQ_IP_VIP_YC_gain_offset_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetLCEConfig(void *pvCfg)
{
    ...
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_LCE_Main,PQ_IP_VIP_LCE_curve_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetIHCConfig(void *pvCfg)
{
    ...
    MDRV_CMDQ_MUTEX_UNLOCK(EN_CMDQ_TYPE_IP0,bRet);
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_IHC_Main,PQ_IP_VIP_IHC_dither_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetICEConfig(void *pvCfg)
{
    ...
    MDRV_CMDQ_MUTEX_UNLOCK(EN_CMDQ_TYPE_IP0,bRet);
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_ICC_Main,PQ_IP_VIP_ICC_dither_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetIHCICCADPYConfig(void *pvCfg)
{
    ...
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_Ymode_Yvalue_ALL_Main,PQ_IP_VIP_Ymode_Yvalue_SETTING_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetIBCConfig(void *pvCfg)
{
    ...
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_IBC_Main,PQ_IP_VIP_IBC_SETTING_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetFCCConfig(void *pvCfg)
{
    ...
    MDRV_CMDQ_MUTEX_UNLOCK(EN_CMDQ_TYPE_IP0,bRet);
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_FCC_T1_Main,PQ_IP_VIP_FCC_T9_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetACKConfig(void *pvCfg)
{
    ...
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_VIP_ACK_Main,PQ_IP_VIP_YCbCr_Clip_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_SetVIPOtherConfig(void *pvCfg)
{
    ...
    MDRV_CMDQ_MUTEX_UNLOCK(EN_CMDQ_TYPE_IP0,bRet);
    if(_MDrv_VIP_For_SetEachPQTypeByIP(PQ_IP_422to444_Main,PQ_IP_VIP_Main,pCfg->stFCfg,p8PQType))
    {
...
unsigned char MDrv_VIP_SetLDCConfig(void *pvCfg)
{
    ...
    if(_IsFrameBufferAllocatedReady())
    {
        ...
        if(Drv_VIP_CheckIPMResolution())
        {
            ...
            Drv_VIP_SetLDCBank_Mode(pCfg->enLDCType);
...
unsigned char MDrv_VIP_SetAIPConfig(ST_MDRV_VIP_AIP_CONFIG stCfg)
{
    ...
    if(!_Is_AIPBypassFlag(stCfg.u16AIPType))
    {
            ...
            stCfg.u16AIPType!=EN_VIP_MDRV_AIP_GM12TO10)
        {
            ...
            _MDrv_VIP_SetPQByType(u16AIPsheet,(MS_U8 *)pvPQSetParameter);
            MDRV_CMDQ_MUTEX_UNLOCK(EN_CMDQ_TYPE_IP0,bRet);
            if(gstSupCfg.staip[stCfg.u16AIPType].u32Viraddr != stCfg.u32Viraddr)
            {
                stCfg.u32Viraddr = (unsigned long)pvPQSetParameter;
                _MDrv_VIP_CopyAIPConfigToGlobal(stCfg.u16AIPType,(void *)&stCfg,sizeof(ST_MDRV_VIP_AIP_CONFIG));
            }
        }
        else if (stCfg.u16AIPType==EN_VIP_MDRV_AIP_XNR)
        {
            ...
            {
                MDRv_PQ_Set_CmdqCfg(0,stCfg.stFCfg.bEn,stCfg.stFCfg.u8framecnt,1);
...
unsigned char MDrv_VIP_SetAIPSRAMConfig(ST_MDRV_VIP_AIP_SRAM_CONFIG stCfg)
{
    ...
    if(MsOS_copy_from_user(pvPQSetParameter, (void  __user *)stCfg.u32Viraddr, u16StructSize))
    {
...
ssize_t MDrv_VIP_ProcShow(char *buf)
{
    ...
    str += MsOS_scnprintf(str, end - str, "   2      VIP Bypass     %s\n", Drv_VIP_GetBypassStatus(EN_VIP_MDRV_CONFIG) ? "ON" :"OFF");
    str += MsOS_scnprintf(str, end - str, "   3          MCNR       %s\n", Drv_VIP_GetBypassStatus(EN_VIP_MDRV_MCNR_CONFIG) ? "Bypass" :"ON");
    str += MsOS_scnprintf(str, end - str, "   5          NLM        %s\n", Drv_VIP_GetBypassStatus(EN_VIP_MDRV_NLM_CONFIG) ? "Bypass" :"ON");
    str += MsOS_scnprintf(str, end - str, "   6          LDC        %s\n", Drv_VIP_GetBypassStatus(EN_VIP_MDRV_LDC_CONFIG) ? "Bypass" :"ON");
