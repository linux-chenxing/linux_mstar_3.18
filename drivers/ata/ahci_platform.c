...
//extern struct ata_host *ata_host_alloc_pinfo(struct platform_device *pdev,const struct ata_port_info * const * ppi, int n_ports);
enum ahci_type {
...
ms_hpriv = devm_kzalloc(dev, sizeof(struct sata_mstar_host_priv), GFP_KERNEL);
...
#endif
	if(id)
	pi=ahci_port_info[id->driver_data];
	else
	pi=ahci_port_info[0];
	...
	//host = ata_host_alloc_pinfo(pdev, ppi, n_ports);//JJFFYY
	if (!host) {
		rc = -ENOMEM;
		goto pdata_exit;
	}
