...
void MHal_LD_Set_SRAM_Power_Down(BOOL bIsSRAMPowerDown)
{
    ...
    if(bIsSRAMPowerDown)
    {
        MHal_LD_Write2ByteMask(REG_LD_FO_SRAM_PD_ENABLE, BIT0, REG_LD_FO_SRAM_PD_ENABLE_MSK);
        MHal_LD_Write2ByteMask(REG_LD_FO_CLOCK_GATE, 0x0000, REG_LD_FO_CLOCK_GATE_MSK);
        MHal_LD_Write2ByteMask(REG_LD_BYPASS_ENABLE, BIT15, REG_LD_BYPASS_ENABLE_MSK);
    }
    else
    {
        MHal_LD_Write2ByteMask(REG_LD_FO_SRAM_PD_ENABLE, 0x00, REG_LD_FO_SRAM_PD_ENABLE_MSK);
        MHal_LD_Write2ByteMask(REG_LD_FO_CLOCK_GATE, BIT12, REG_LD_FO_CLOCK_GATE_MSK);
        MHal_LD_Write2ByteMask(REG_LD_BYPASS_ENABLE, 0x0000, REG_LD_BYPASS_ENABLE_MSK);
    }
#else
    if(bIsSRAMPowerDown)
    {
        MHal_LD_Write2ByteMask(REG_LD_SRAM_PD_ENABLE, (BIT0| BIT1), REG_LD_SRAM_PD_ENABLE_MSK);
        MHal_LD_Write2ByteMask(REG_LD_CLOCK_GATE, 0x0000, REG_LD_CLOCK_GATE_MSK);
        MHal_LD_Write2ByteMask(REG_LD_BYPASS_ENABLE, BIT15, REG_LD_BYPASS_ENABLE_MSK);

    }
    else
    {
        MHal_LD_Write2ByteMask(REG_LD_SRAM_PD_ENABLE, 0x00, REG_LD_SRAM_PD_ENABLE_MSK);
        MHal_LD_Write2ByteMask(REG_LD_CLOCK_GATE, (BIT11| BIT12| BIT13), REG_LD_CLOCK_GATE_MSK);
        MHal_LD_Write2ByteMask(REG_LD_BYPASS_ENABLE, 0x0000, REG_LD_BYPASS_ENABLE_MSK);
    }
#endif
}
