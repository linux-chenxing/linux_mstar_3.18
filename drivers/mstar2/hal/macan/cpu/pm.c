...
void mstar_restore_int_mask(void)
{
    ...
    //mstar_restore_int_mask use str_gic_dist_base to mask irq 29
    val = readl_relaxed(str_gic_dist_base + 0x180 + (29 / 32) * 4);
    val= val | (0x01 << 29 );
    writel_relaxed(val, str_gic_dist_base + 0x180 + (29 / 32) * 4);
}
