...
#define DOVI_BCMS_HEADER_SIZE   31
#define DOVI_BCMS_DEFAULT_SIZE  29509   //based on document "UI_control.pdf"  , released on .......
#define DOVI_BCMS_A_SIZE        DOVI_BCMS_DEFAULT_SIZE
#define DOVI_BCMS_B_SIZE        DOVI_BCMS_DEFAULT_SIZE
#define DOVI_PQ2BT1886_SIZE     16592
#define DOVI_BACKLIGHTLUT_SIZE  4096
#define DOVI_CONFIG_BINARY_SIZE 512
#define DOVI_PQ_TUNING_FORMAT_SIZE (DOVI_BCMS_DEFAULT_SIZE + DOVI_BCMS_A_SIZE \
                                    + DOVI_BCMS_B_SIZE + DOVI_PQ2BT1886_SIZE \
                                    + DOVI_BACKLIGHTLUT_SIZE + DOVI_CONFIG_BINARY_SIZE) //109727
...
//PQ setting
#define DOVI_PQ_CONFIG_BINARY_SIZE 512
...
{  ////// lut hdr( chars+1 for '\0' )
    MS_U32 Version;     // from 4 CC
    MS_S32 LittleEndian; // from 1 CC, b: big or l: little endian
    GmLutType_t Type;     // from 8 CC, ???2???_
    MS_S16 DimC1, DimC2, DimC3; // dim for C1, C2, C3
    MS_S16 IdxMinC1, IdxMaxC1, IdxMinC2, IdxMaxC2, IdxMinC3, IdxMaxC3;  // the min, max idx

    ////// lut data
    MS_U16 *LutMap; // the LUT, type depends on version and type
} GmLut_t;


typedef struct DoVi_BCMS_Header_t_
{
...
#define int32_t MS_S32
#define int16_t MS_S16
#define uint16_t MS_U16
#define uint32_t MS_U32

typedef struct DoVi_PQ_control_t_
{

    MS_BOOL bDovi_UsePQen;
...
#pragma pack(push)
#pragma pack(1)
typedef struct DoVi_TgtGDCfg_t_
{
    int32_t   gdEnable;
    uint32_t  gdWMin;
    uint32_t  gdWMax;
...
typedef struct DoVi_TargetDisplayConfig_t_
{
    ...
    uint16_t TuningMode;
    int16_t  reserved[10];
    uint16_t BackLightUI;
    uint16_t BrightnessUI;
    uint16_t ContrastUI;
    int16_t  MidPQBiasLut[14];
    int16_t  SaturationGainBiasLut[14];
    int16_t  ChromaWeightBiasLut[14];
    int16_t  reserved0Lut[14];
    int16_t  reserved1Lut[14];
    int16_t  reserved2Lut[14];
    DoVi_TgtGDCfg_t gdConfig;
}DoVi_TargetDisplayConfig_t;
#pragma pack(pop)
