...
static MS_BOOL bGOP_OSDB[MAX_GOP_SUPPORT];
static MS_U16 u16GOP_OPMux[MAX_GOP_SUPPORT];
...
MS_BOOL MHAL_GFLIP_SDRHDR_CTRL(MS_BOOL bHDR_STATUS, MS_BOOL gop_csc_en);
void OSDB_BLENING_ENABLE(MS_U16 u16OP_Mux, MS_BOOL bEnable);
EXPORT_SYMBOL(MHAL_GFLIP_SDRHDR_CTRL);
...
MS_U8 MHal_GFLIP_GetBankOffset(MS_U32 u32GopIdx, MS_U16 u16BankIdx)
{
    ...
    else if(u32GopIdx == GFLIP_GOP_IDX_MIXER)
        return (MS_U8)(GFLIP_GOP_MIXER_BANKID);
    return 0;
}
...
{
    MS_U8 u8index = 0x0;
    MS_U16 u16GOP_OPMUX_tmp = 0x0;
    MS_U16 u16GOP_OPMux_OSDB_tmp=0x0;
    MS_U16 u16GOPMixerPortEna=0x0;
    MS_U16 u16GOPdst = 0x0;
    ...
    if(bHDR_STATUS == TRUE)
    {
        ...
        /////////////////////////////////////////////////
        MHal_GFLIP_ReadGopReg(0x0, 0x0, REG_GOP_MUX, &u16GOP_OPMUX_tmp);
        for(u8index = 0; u8index < MAX_GOP_SUPPORT; u8index++)
        {
            MHal_GFLIP_ReadGopReg(u8index, GFLIP_GOP_BANK_IDX_0, REG_GOP_CTRL1, &u16GOPdst);
            u16GOPdst = (u16GOPdst & GOP_DST_MASK);
            if(u16GOPdst == GFLIP_GOP_DST_OP0)
            {
                //enable bank force write
                MHal_GFLIP_WriteGopReg(u8index, GFLIP_GOP_BANK_IDX_0, REG_GOP_BAK_FORCE_WRITE_REG, GOP_BAK_WRITE, GOP_BAK_WRITE);

                //HDR need RGB domain
                if(gop_csc_en == TRUE)
                    MHal_GFLIP_WriteGopReg(u8index, GFLIP_GOP_BANK_IDX_0, REG_GOP4G_CTRL0, 0x0, GOP_COLOR_MASK);
                else
                    MHal_GFLIP_WriteGopReg(u8index, GFLIP_GOP_BANK_IDX_0, REG_GOP4G_CTRL0, GOP_COLOR_MASK, GOP_COLOR_MASK);

                //enable GOP handshake mode
                MHal_GFLIP_WriteGopReg(u8index, GFLIP_GOP_BANK_IDX_0, REG_GOP_HANDSHAKE, GOP_HANDSHAKE_MASK, GOP_HANDSHAKE_MASK);

                //disable bank force write
                MHal_GFLIP_WriteGopReg(u8index, GFLIP_GOP_BANK_IDX_0, REG_GOP_BAK_FORCE_WRITE_REG, 0x0, GOP_BAK_WRITE);

                if(u8index == (u16GOP_OPMUX_tmp&GOP_OP_MUX0))
                    u16GOP_OPMux[u8index] = 0;
                else if(u8index == (u16GOP_OPMUX_tmp&GOP_OP_MUX1))
                    u16GOP_OPMux[u8index] = 1;
                else if(u8index == (u16GOP_OPMUX_tmp&GOP_OP_MUX2))
                    u16GOP_OPMux[u8index] = 2;
                else if(u8index == (u16GOP_OPMUX_tmp&GOP_OP_MUX3))
                    u16GOP_OPMux[u8index] = 3;
                else if(u8index == (u16GOP_OPMUX_tmp&GOP_OP_MUX4))
                ...
                switch(u16GOP_OPMux[u8index])
                {
                    case 0:
                    {
                        u16GOP_OPMux_OSDB_tmp = SC_REG(GOP_SC_GOPBLENDING_L);
                    ...
                    case 2:
                    {
                        u16GOP_OPMux_OSDB_tmp = SC_REG(GOP_SC_GOPBLENDING_H);
        ...
        u8GOP_HDR_OPMux_GOP = (u16GOP_OPMUX_tmp&GFLIP_GOP_HDR_MUX_MASK)>>GFLIP_GOP_HDR_MUX_SHIFT;

        //Set one OP mux for Mixer HDR
        if(GFLIP_GOP_HDR_MUX == OP_MUX_0)
            u16GOP_OPMUX_tmp = (u16GOP_OPMUX_tmp&(~GOP_OP_MUX0)) | GFLIP_GOP_MIXER_MUX << GOP_OP_MUX0_SHIFT;
        else if(GFLIP_GOP_HDR_MUX == OP_MUX_1)
            u16GOP_OPMUX_tmp = (u16GOP_OPMUX_tmp&(~GOP_OP_MUX1)) | GFLIP_GOP_MIXER_MUX << GOP_OP_MUX1_SHIFT;
        else if(GFLIP_GOP_HDR_MUX == OP_MUX_2)
            u16GOP_OPMUX_tmp = (u16GOP_OPMUX_tmp&(~GOP_OP_MUX2)) | GFLIP_GOP_MIXER_MUX << GOP_OP_MUX2_SHIFT;
        else if(GFLIP_GOP_HDR_MUX == OP_MUX_3)
            u16GOP_OPMUX_tmp = (u16GOP_OPMUX_tmp&(~GOP_OP_MUX3)) | GFLIP_GOP_MIXER_MUX << GOP_OP_MUX3_SHIFT;
        else if(GFLIP_GOP_HDR_MUX == OP_MUX_4)
            u16GOP_OPMUX_tmp = (u16GOP_OPMUX_tmp&(~GOP_OP_MUX4)) | GFLIP_GOP_MIXER_MUX << GOP_OP_MUX4_SHIFT;

        OSDB_BLENING_ENABLE(GFLIP_GOP_HDR_MUX,TRUE);

        MHal_GFLIP_WriteGopReg(0x0, GFLIP_GOP_BANK_IDX_0, REG_GOP_BANK_SEL_EX, 0x0, GOP_OP_MUX_DBF_EN);//disable opmux double buffer
        MHal_GFLIP_WriteGopReg(0x0, GFLIP_GOP_BANK_IDX_0, REG_GOP_MUX, u16GOP_OPMUX_tmp, GOP_OP_MUX_MASK);

        MHal_GFLIP_WriteGopReg(GFLIP_GOP_IDX_MIXER, 0x0, GOP_MIXER_L_REG, GOP_MIXER_ALPHA_INV|GOP_MIXER_VDE_SEL, GOP_HDR_BYPASS|GOP_MIXER_ALPHA_INV|GOP_MIXER_VDE_SEL);
        MHal_GFLIP_WriteGopReg(GFLIP_GOP_IDX_MIXER, 0x0, GOP_MIXER_H_REG, u16GOPMixerPortEna, GOP_MIXER_VALID_EN_MASK);
        MHal_GFLIP_WriteGopReg(0x0, GFLIP_GOP_BANK_IDX_0, REG_GOP_BANK_SEL_EX, GOP_MIXER_WR_ACK, GOP_MIXER_WR_ACK); //gop mixer write ack
         ...
         for(u8index=0;u8index<MAX_GOP_SUPPORT;u8index++)
         {
            if(bGOP_OSDB[u8index]==TRUE)
            {
                if(GFLIP_GOP_HDR_MUX == u16GOP_OPMux[u8index])
                    continue;
    ...
    else
    {
        ...
        MHal_GFLIP_WriteGopReg(GFLIP_GOP_IDX_MIXER, 0x0, GOP_MIXER_L_REG, GOP_HDR_BYPASS, GOP_HDR_BYPASS);
        MHal_GFLIP_WriteGopReg(0x0, GFLIP_GOP_BANK_IDX_0, REG_GOP_BANK_SEL_EX, GOP_MIXER_WR_ACK, GOP_MIXER_WR_ACK); //gop mixer write ack
    }
...
void OSDB_BLENING_ENABLE(MS_U16 u16OP_Mux, MS_BOOL bEnable)
{
    ...
    switch(u16OP_Mux)
    {
        case OP_MUX_0:
        {
            ...
            if (bEnable == TRUE)
            {
                u16GOP_OPMux_OSDB_tmp = ((u16GOP_OPMux_OSDB_tmp&(~GOP_MUX0_OSDB_MASK)) | GOP_MUX0_OSDB_MASK);
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            else
            {
                u16GOP_OPMux_OSDB_tmp = (u16GOP_OPMux_OSDB_tmp&(~GOP_MUX0_OSDB_MASK));
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            break;
        }
        case OP_MUX_1:
        {
            ...
            if (bEnable == TRUE)
            {
                u16GOP_OPMux_OSDB_tmp = ((u16GOP_OPMux_OSDB_tmp&(~GOP_MUX1_OSDB_MASK)) | GOP_MUX1_OSDB_MASK);
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            else
            {
                u16GOP_OPMux_OSDB_tmp = (u16GOP_OPMux_OSDB_tmp&(~GOP_MUX1_OSDB_MASK));
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            break;
        }
        case OP_MUX_2:
        {
            ...
            if (bEnable == TRUE)
            {
                u16GOP_OPMux_OSDB_tmp = ((u16GOP_OPMux_OSDB_tmp&(~GOP_MUX2_OSDB_MASK)) | GOP_MUX2_OSDB_MASK);
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            else
            {
                u16GOP_OPMux_OSDB_tmp = (u16GOP_OPMux_OSDB_tmp&(~GOP_MUX2_OSDB_MASK));
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            break;
        }
        case OP_MUX_3:
        {
            ...
            if (bEnable == TRUE)
            {
                u16GOP_OPMux_OSDB_tmp = ((u16GOP_OPMux_OSDB_tmp&(~GOP_MUX3_OSDB_MASK)) | GOP_MUX3_OSDB_MASK);
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            else
            {
                u16GOP_OPMux_OSDB_tmp = (u16GOP_OPMux_OSDB_tmp&(~GOP_MUX3_OSDB_MASK));
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            break;
        }
        case OP_MUX_4:
        {
            u16GOP_OPMux_OSDB_tmp = SC_REG(GOP_SC_GOPBLENDING_EX);
            if (bEnable == TRUE)
            {
                u16GOP_OPMux_OSDB_tmp = ((u16GOP_OPMux_OSDB_tmp&(~GOP_MUX4_OSDB_MASK)) | GOP_MUX4_OSDB_MASK);
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            else
            {
                u16GOP_OPMux_OSDB_tmp = (u16GOP_OPMux_OSDB_tmp&(~GOP_MUX4_OSDB_MASK));
                SC_REG(GOP_SC_GOPBLENDING_L) = u16GOP_OPMux_OSDB_tmp;
            }
            break;
        }
