...
{
    MHal_LD_WriteByteMask(REG_LD_LSFOM, u8Mode << REG_LD_LSFOM_SHIFT, REG_LD_LSFOM_MSK);
}
...
void MHal_LD_SetTempFilterMaxSpeed(U16 u16Val)
{
    MHal_LD_Write2Byte(REG_LD_TEMPMAXSPEED,u16Val);
}

void MHal_LD_SetLSFEncodingEn(BOOL bLSFEncodingEn)
{
    //h20 [0]reg_ld_coefdec
    MHal_LD_WriteByteMask( 0x134E40, (U16)bLSFEncodingEn, 0x0001);

}
void MHal_LD_SetCompLowGainEn(BOOL bCompLowGainEn)
{
    //h20 [2]reg_ld_comp_low_gain_en
    MHal_LD_WriteByteMask( 0x134E40, ((U16)bCompLowGainEn)<<2, 0x0004);
}

void MHal_LD_SetSaturationEn(BOOL bSaturationEn)
{
    //h50 [7]reg_sat_en
    MHal_LD_WriteByteMask( 0x134EA0, ((U16)bSaturationEn<<7), 0x0080);
}

void MHal_LD_SetSatA(U8 u8Reg_sat_a)
{
    //h51 [7:0]reg_sat_a
    MHal_LD_WriteByteMask( 0x134EA2, u8Reg_sat_a, 0x00FF);
}
void MHal_LD_SetSatC(U16 u16Reg_sat_c)
{
    //h55 [11:0]reg_sat_c
    MHal_LD_Write2ByteMask( 0x134EAA, u16Reg_sat_c, 0x0FFF);
}
void MHal_LD_SetSatCSft(U8 u8Reg_sat_c_sft)
{
    //h56 [3:0]reg_sat_c_sft
    MHal_LD_WriteByteMask( 0x134EAC, u8Reg_sat_c_sft, 0x000F);
}
...
{
    MHal_LD_WriteRegBit(REG_LD_LSFOM, TRUE, REG_LD_REPORTWIN_SWAP_BIT);
}
...
void MHal_LD_ClearReportWindowSwap(void)
{
    MHal_LD_WriteRegBit(REG_LD_LSFOM, FALSE, REG_LD_REPORTWIN_SWAP_BIT);
}

//for DMA to use
void MHal_LD_SetBlGlobe(U8 u8GlobeWeight)
{
    MHal_LD_WriteByte(REG_BL_GLOBE , u8GlobeWeight);
}

//FIXME : We should use ENUM
//Mode 0 :8BIT
//Mode 1: 16BIT
//Mode 2: 12BIT
//Mode 3: 4BIT
//Mode 4: 10BIT
void MHal_LD_SetBlGlobeMode(U8 u8ModeIdx)
{
    switch(u8ModeIdx) {
    case  0: {
        MHal_LD_WriteByteMask(REG_BL_GLOBE_MOD,  0x0, REG_BL_GLOBE_MOD_MSK);
        break;
    }
    case 1: {
        MHal_LD_WriteByteMask(REG_BL_GLOBE_MOD,  0x1, REG_BL_GLOBE_MOD_MSK);
        MHal_LD_WriteByteMask(REG_BL_DATA_ALIG,  0x0,REG_BL_DATA_ALIG_MSK);
        break;
    ...
    case 3: {
        MHal_LD_WriteByteMask(REG_BL_GLOBE_MOD,  0x2, REG_BL_GLOBE_MOD_MSK);
        MHal_LD_WriteByteMask(REG_BL_DATA_ALIG,  0x0,REG_BL_DATA_ALIG_MSK);

        break;
    }
    case 4: {
        MHal_LD_WriteByteMask(REG_BL_GLOBE_MOD,  0x3, REG_BL_GLOBE_MOD_MSK);
        MHal_LD_WriteByteMask(REG_BL_DATA_ALIG,  0x0,REG_BL_DATA_ALIG_MSK);
...
void MHal_LD_SetBlWidthDMA(U8 u8Width)
{
     MHal_LD_WriteByteMask(REG_BL_WIDTH_DMA,u8Width,REG_BL_WIDTH_DMA_MASK);
}
void MHal_LD_SetBlHeightDMA(U8 u8Height)
{
    MHal_LD_WriteByteMask(REG_BL_HEIGHT_DMA,u8Height,REG_BL_HEIGHT_DMA_MASK);
}

void MHal_LD_SetLEDBufBaseOffset(U32 u32Addr)
{
    MHal_LD_Write4Byte(REG_BASEADDR_LED_OFFSET,u32Addr&REG_BASEADDR_LED_OFFSET_BITS);
}

void MHal_LD_SetBlDepack16BitMode(BOOL bEnable)
{
    MHal_LD_WriteRegBit(REG_BL_DEPACK16B_MOD, bEnable, REG_BL_DEPACK16B_MOD_BIT);
}

void MHal_LD_SetSpiDataInv(U8 u8invert)
{
     MHal_LD_WriteByteMask(REG_SPI_DATA_INV,u8invert<<REG_SPI_DATA_INV_SHIFT,REG_SPI_DATA_INV_MSK);
}


BOOL MHal_LD_SetMIUPackOffset(U8 u8DMAIndex, U8 u8Value)
{

    if(u8Value>MIU_PACK_NUM_MAX) {
        return FALSE;
    }

    switch (u8DMAIndex) {
    case 0: {
        MHal_LD_Write2ByteMask(REG_MIU_PACK_OFFSET_0, u8Value, REG_MIU_PACK_OFFSET_0_MSK);
        break;
    }
    case 1: {
        MHal_LD_Write2ByteMask(REG_MIU_PACK_OFFSET_1, u8Value, REG_MIU_PACK_OFFSET_0_MSK);
        break;
    }
    case 2: {
        MHal_LD_Write2ByteMask(REG_MIU_PACK_OFFSET_2, u8Value, REG_MIU_PACK_OFFSET_0_MSK);
        break;
    }
    case 3: {
        MHal_LD_Write2ByteMask(REG_MIU_PACK_OFFSET_3, u8Value, REG_MIU_PACK_OFFSET_0_MSK);
        break;
    }
    default: {
        return FALSE;
...
BOOL MHal_LD_SetMIUPackLength(U8 u8DMAIndex, U8 u8Value)
{
    ...
    switch (u8DMAIndex) {
    case 0: {
        MHal_LD_Write2ByteMask(REG_MIU_PACK_LENGTH_0, u8Value, REG_MIU_PACK_LENGTH_0_MSK);
        break;
    }
    case 1: {
        MHal_LD_Write2ByteMask(REG_MIU_PACK_LENGTH_1, u8Value, REG_MIU_PACK_LENGTH_1_MSK);
        break;
    }
    case 2: {
        MHal_LD_Write2ByteMask(REG_MIU_PACK_LENGTH_2, u8Value, REG_MIU_PACK_LENGTH_2_MSK);
        break;
    }
    case 3: {
        MHal_LD_Write2ByteMask(REG_MIU_PACK_LENGTH_3, u8Value, REG_MIU_PACK_LENGTH_3_MSK);
        break;
    }
    default: {
        return FALSE;
...
BOOL MHal_LD_SetDmaEnable(U8 u8DMAIndex, BOOL bValue)
{

    switch (u8DMAIndex) {
    case 0: {
        MHal_LD_WriteRegBit(REG_DMA0_EN, bValue, REG_DMA_EN_BIT);
        break;
    }
    case 1: {
        MHal_LD_WriteRegBit(REG_DMA1_EN, bValue, REG_DMA_EN_BIT);
        break;
    }
    case 2: {
        MHal_LD_WriteRegBit(REG_DMA2_EN, bValue, REG_DMA_EN_BIT);
        break;
    }
    case 3: {
        MHal_LD_WriteRegBit(REG_DMA3_EN, bValue, REG_DMA_EN_BIT);
        break;
    }
    default: {
        return FALSE;
...
BOOL MHal_LD_SetYoffEnd(U8 u8DMAIndex, U8 u8Value)
{
    if(u8Value>REG_DMA_YOFF_END_MSK) {
        return FALSE;
    }
    switch (u8DMAIndex) {
    case 0: {
        MHal_LD_Write2ByteMask(REG_DMA_YOFF_END_0, u8Value, REG_DMA_YOFF_END_MSK);
        break;
    }
    case 1: {
        MHal_LD_Write2ByteMask(REG_DMA_YOFF_END_1,u8Value, REG_DMA_YOFF_END_MSK);
        break;
    }
    case 2: {
        MHal_LD_Write2ByteMask(REG_DMA_YOFF_END_2, u8Value, REG_DMA_YOFF_END_MSK);
        break;
    }
    case 3: {
        MHal_LD_Write2ByteMask(REG_DMA_YOFF_END_3, u8Value, REG_DMA_YOFF_END_MSK);
        break;
    }
    default: {
        return FALSE;
...
// LD2 h25[8] dummy
void MHal_LD_Set_ResetGammaTable_En(BOOL bEn)
{
    MHal_LD_WriteRegBit(REG_LD_SWRSTGAMMAEN, bEn, REG_LD_SWRSTGAMMAEN_BIT);
}
BOOL MHal_LD_Get_ResetGammaTable_En(void)
{
    return ( MHal_LD_ReadRegBit(REG_LD_SWRSTGAMMAEN, REG_LD_SWRSTGAMMAEN_BIT))?TRUE:FALSE;
}
// LD2 h25[9] dummy
void MHal_LD_Set_ResetGammaTable_Mode(BOOL bMode)
{
    MHal_LD_WriteRegBit(REG_LD_SWRSTGAMMAMODE, bMode, REG_LD_SWRSTGAMMAMODE_BIT);
}
BOOL MHal_LD_Get_ResetGammaTable_Mode(void)
{
    return ( MHal_LD_ReadRegBit(REG_LD_SWRSTGAMMAMODE, REG_LD_SWRSTGAMMAMODE_BIT))?TRUE:FALSE;
}
// LD2 h25[10] dummy
void MHal_LD_Set_ResetGammaTable_RW(BOOL bRW)
{
   MHal_LD_WriteRegBit(REG_LD_SWRSTGAMMARW, bRW, REG_LD_SWRSTGAMMARW_BIT);
 }
BOOL MHal_LD_Get_ResetGammaTable_RW(void)
{
    return ( MHal_LD_ReadRegBit(REG_LD_SWRSTGAMMARW, REG_LD_SWRSTGAMMARW_BIT))?TRUE:FALSE;
}
// h26[7:0] dummy : index 0~255
void MHal_LD_Set_ResetGammaTable_Index(U8 u8TblIndex)
{
    MHal_LD_WriteByte(REG_LD_SWRSTGAMMAINDEX, u8TblIndex);
}
U8 MHal_LD_Get_ResetGammaTable_Index(void)
{
    return MHal_LD_ReadByte(REG_LD_SWRSTGAMMAINDEX);
}
// h26[15:8] dummy : data 0~255
void MHal_LD_Set_ResetGammaTable_Data(U8 u8TblData)
{
    MHal_LD_WriteByte(REG_LD_SWRSTGAMMADATA, u8TblData);
}
U8 MHal_LD_Get_ResetGammaTable_Data(void)
{
    return MHal_LD_ReadByte(REG_LD_SWRSTGAMMADATA);
}

void MHal_LD_Set_SRAM_Power_Down(BOOL bIsSRAMPowerDown)
{
    ;
}
#endif
