...
void piu_timer_mdelay(unsigned int mdelay)
{
    unsigned int time_out;
    unsigned int reg_temp1;
    unsigned int reg_temp2;
    //PIU timer1
    time_out = mdelay*12*1000;
    *(volatile unsigned short *)(mstar_pm_base + (0x003040 << 1))=0;
    *(volatile unsigned short *)(mstar_pm_base + (0x003040 << 1))=1;
    reg_temp1 = *(volatile unsigned short *)(mstar_pm_base + (0x003048 << 1));
    reg_temp1 |= (*(volatile unsigned short *)(mstar_pm_base + (0x00304a << 1)) << 16);
    while(reg_temp2 < (reg_temp1+time_out))
    {
        reg_temp2 = *(volatile unsigned short *)(mstar_pm_base + (0x003048 << 1));
        reg_temp2 |= (*(volatile unsigned short *)(mstar_pm_base + (0x00304a << 1)) << 16);
    }
    *(volatile unsigned short *)(mstar_pm_base + (0x003040 << 1))=0;
}
EXPORT_SYMBOL(piu_timer_mdelay);
...
extern u32 SC_MULT;
extern u32 SC_SHIFT;
extern unsigned int early_query_frequency(void);
