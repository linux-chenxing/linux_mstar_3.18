...
irqreturn_t _MHAL_MBXINT_INTHandler(int irq, void *dev_id)
{
    switch(irq)
    {
        ...
        case E_FIQ_INT_8051_TO_MIPS:
            ...
            break;
		case E_FIQEXPL_HST1TO2: /* sec_r2 to arm */
...
MBX_Result _MHAL_MBXINT_SetHostCPU(MBX_CPU_ID eHostCPU)
{
	...
	s32Err = request_irq(E_FIQEXPL_HST1TO2, _MHAL_MBXINT_INTHandler, irqflags, "MBX_FIQ_R2toMIPS", NULL);
    if(s32Err<0)
    {
...
void MHAL_MBXINT_DeInit (MBX_CPU_ID eHostCPU)
{
    ...
    MHAL_MBXINT_Enable(E_FIQEXPL_HST1TO2, FALSE);
    free_irq(E_FIQEXPL_HST1TO2, NULL);
    _MHAL_MBXINT_ClearIntFlg(E_FIQEXPL_HST1TO2);
}
...
MBX_Result MHAL_MBXINT_Suspend(MBX_CPU_ID eHostCPU)
{
    ...
    if(_MHAL_MBXINT_IsIntFlgSet(E_FIQEXPL_HST1TO2)){
        MHAL_MBXINT_Enable(E_FIQEXPL_HST1TO2,FALSE);
        disable_irq(E_FIQEXPL_HST1TO2);
    }
    return E_MBX_SUCCESS;
}
//-------------------------------------------------------------------------------------------------
...
MBX_Result MHAL_MBXINT_Resume(MBX_CPU_ID eHostCPU)
{
    ...
    if(_MHAL_MBXINT_IsIntFlgSet(E_FIQEXPL_HST1TO2)){
        MHAL_MBXINT_Enable(E_FIQEXPL_HST1TO2, TRUE);
        enable_irq(E_FIQEXPL_HST1TO2);
    }
    return E_MBX_SUCCESS;
}
