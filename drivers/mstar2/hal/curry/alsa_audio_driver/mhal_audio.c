 ...
 * ============================================================================
 */
#define _MAD_CHIP_MODEL    "MAD CURRY"
...
#define _MAD_PCM_CAPTURE3_BASE_OFFSET   (0x93000 + _MAD_DSP2_TO_COMMON_DRAM_OFFSET)
...
static int _mhal_alsa_pcm_capture3_init(void)
{
	...
	if ((pcm_capture->initialized_status != MAD_TRUE) || (pcm_capture->str_mode_info.status != E_RESUME)) {
		...
		if (g_pcm_capture3_base_va == 0) {
			g_pcm_capture3_base_va = (ptrdiff_t)ioremap_nocache(audio_pcm_capture_base_ba, pcm_capture->buffer.size);
			if (g_pcm_capture3_base_va == 0) {
