...
static void __init chip_early_init(void)
{
#ifdef CONFIG_MP_TRUSTZONE_PL310
	printk("\033[0;32;31m [Ian] %s %d PL310 Enable SMC call OPTEE \033[m\n",__func__,__LINE__);
	__asm__ volatile( //Use SMC call OPTEE to open PL310 Enable bit
		".arch_extension sec\n\t"
		"ldr r0, =0x30000000\n\t" //MSTAR_SERVICE | MSTAR_PL310_ENABLE
		"smc #0\n\t"
	);
#endif
...
#ifdef CONFIG_MP_TRUSTZONE_PATCH
	//do nothing because PL310 init move to chip_early_init when CONFIG_MP_TRUSTZONE_PATCH enable
#else
#ifdef CONFIG_CACHE_L2X0
	{
		void __iomem * l2x0_base
...
#endif
#endif //CONFIG_MP_TRUSTZONE_PATCH
