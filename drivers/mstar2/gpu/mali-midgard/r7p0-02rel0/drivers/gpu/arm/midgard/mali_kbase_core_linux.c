...
/* MStar: module parameter to show GPU clock */
#ifdef MSTAR
...
/* MStar: module parameter to show GPU utilization */
...
/* MStar: module parameter to enable/disable skipping jobs */
#if MSTAR_SKIP_JOBS_SWITCH
int mali_skip_jobs = 0;
module_param(mali_skip_jobs, int, S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH); /* rw-rw-r-- */
MODULE_PARM_DESC(mali_skip_jobs, "Skip jobs in device driver");
#endif
...
/** Show callback for the @c dvfs boost sysfs file.
 *
 * This function is called to get the contents of the @c dvfs_boost
 * sysfs file.
 *
 * @param devThe device this sysfs file is for
 * @param attrThe attributes of the sysfs file
 ...
 * @return The number of bytes output to @c buf.
 **/
static ssize_t show_dvfs_boost(struct device *dev, struct device_attribute *attr, char *const buf)
{
	struct kbase_device *kbdev = to_kbase_device(dev);
	ssize_t ret;
	...
	ret = scnprintf(buf, PAGE_SIZE, "%d\n", kbdev->dvfs.boost_enabled);

	return ret;
}

/** Store callback for the @c dvfs boost sysfs file.
 *
 * This function is called when the @c dvfs_enable_boost sysfs file is
 * written to. It determines the written value is valid and perform
 * corresponding dvfs enabling switch.
 *
 * @param devThe device with sysfs file is for
 ...
 * @return @c count if the function succeeded. An error code on failure.
 **/
static ssize_t set_dvfs_boost(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev = to_kbase_device(dev);
	u32 boost_enabled = 0;
	...
	boost_enabled = kbdev->dvfs.boost_enabled;
	...
	if (boost_enabled == 0)
	{
		kbdev->dvfs.disable_boost(kbdev);
	}
	else
	{
		kbdev->dvfs.enable_boost(kbdev);
	}

	return count;
}

/** The sysfs file @c dvfs_enable_boost
 *
 * This is used for obtaining information about the DVFS boost enabling status,
 * switching whether to enable DVFS or not.
 **/
static DEVICE_ATTR(dvfs_enable_boost, S_IRUGO | S_IWUSR, show_dvfs_boost, set_dvfs_boost);

#if CONFIG_MALI_MIDGARD_DVFS_FREQ_ADJUSTABLE
/**
 * set_dvfs_max_freq - Store callback for the dvfs_max_freq sysfs file.
 * @dev:   The device with sysfs file is for
 * @attr:  The attributes of the sysfs file
 * @buf:   The value written to the sysfs file
 * @count: The number of bytes written to the sysfs file
 *
 * This function is called when the dvfs_max_freq sysfs file is written to. It
 * checks the data written, and if valid updates the gpu maximum frequency,
 *
 * Return: @c count if the function succeeded. An error code on failure.
 */
static ssize_t set_dvfs_max_freq(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	int ret;
	...
	ret = kstrtoint(buf, 0, &dvfs_max_freq);
	if (ret) {
		dev_err(kbdev->dev, "Couldn't process dvfs_max_freq write operation.\n");
		return -EINVAL;
	}

	if (dvfs_max_freq == kbdev->dvfs.dvfs_max_freq) {
		dev_err(kbdev->dev, "Ignore dvfs_max_freq write operation.\n"
			"%d MHz is the same with original value of dvfs_max_freq\n", kbdev->dvfs.dvfs_max_freq);
		return -EINVAL;
	}

	if ((dvfs_max_freq < kbdev->dvfs.dvfs_min_freq) || (dvfs_max_freq > MALI_MAX_FREQ)) {
		dev_err(kbdev->dev, "Ignore dvfs_max_freq write operation.\n"
			"%d MHz is out of boundary of [%d,%d]\n", dvfs_max_freq, kbdev->dvfs.dvfs_min_freq, MALI_MAX_FREQ);
		return -EINVAL;
	}

	kbdev->dvfs.dvfs_max_freq = dvfs_max_freq;
	dev_dbg(kbdev->dev, "dvfs_max_freq: %d MHz\n", dvfs_max_freq);

	if (mali_gpu_clock > dvfs_max_freq) {
		dev_err(kbdev->dev, "clock_down.\n"
			"mali_gpu_clock: %d > dvfs_max_freq: %d\n", mali_gpu_clock, dvfs_max_freq);
...
/**
 * show_dvfs_max_freq - Show callback for the dvfs_max_freq sysfs entry.
 * @dev:  The device this sysfs file is for.
 * @attr: The attributes of the sysfs file.
 * @buf:  The output buffer to receive the GPU information.
 *
 ...
 * Return: The number of bytes output to buf.
 */
static ssize_t show_dvfs_max_freq(struct device *dev,
		struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret;
...
static DEVICE_ATTR(dvfs_max_freq, S_IRUGO | S_IWUSR, show_dvfs_max_freq, set_dvfs_max_freq);

/**
 * set_dvfs_min_freq - Store callback for the dvfs_min_freq sysfs file.
 * @dev:   The device with sysfs file is for
 * @attr:  The attributes of the sysfs file
 * @buf:   The value written to the sysfs file
 * @count: The number of bytes written to the sysfs file
 *
 * This function is called when the dvfs_min_freq sysfs file is written to. It
 * checks the data written, and if valid updates the gpu minimum frequency,
 *
 * Return: @c count if the function succeeded. An error code on failure.
 */
static ssize_t set_dvfs_min_freq(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev;
	int ret;
	...
	ret = kstrtoint(buf, 0, &dvfs_min_freq);
	if (ret) {
		dev_err(kbdev->dev, "Couldn't process dvfs_min_freq write operation.\n");
		return -EINVAL;
	}

	if (dvfs_min_freq == kbdev->dvfs.dvfs_min_freq) {
		dev_err(kbdev->dev, "Ignore dvfs_min_freq write operation.\n"
			"%d MHz is the same with original value of dvfs_min_freq\n", kbdev->dvfs.dvfs_min_freq);
		return -EINVAL;
	}

	if ((dvfs_min_freq > kbdev->dvfs.dvfs_max_freq) || (dvfs_min_freq < MALI_MIN_FREQ)) {
		dev_err(kbdev->dev, "Ignore dvfs_min_freq write operation.\n"
			"%d MHz is out of boundary of [%d,%d]\n", dvfs_min_freq, MALI_MIN_FREQ, kbdev->dvfs.dvfs_max_freq);
		return -EINVAL;
	}

	kbdev->dvfs.dvfs_min_freq = dvfs_min_freq;
	dev_dbg(kbdev->dev, "dvfs_min_freq: %d MHz\n", dvfs_min_freq);

	if (mali_gpu_clock < dvfs_min_freq) {
		dev_err(kbdev->dev, "gpu_clock_up.\n"
			"mali_gpu_clock: %d < dvfs_min_freq: %d\n", mali_gpu_clock, dvfs_min_freq);
...
/**
 * show_dvfs_min_freq - Show callback for the dvfs_min_freq sysfs entry.
 * @dev:  The device this sysfs file is for.
 * @attr: The attributes of the sysfs file.
 * @buf:  The output buffer to receive the GPU information.
 *
 ...
 * Return: The number of bytes output to buf.
 */
static ssize_t show_dvfs_min_freq(struct device *dev,
		struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret;
...
static DEVICE_ATTR(dvfs_min_freq, S_IRUGO | S_IWUSR, show_dvfs_min_freq, set_dvfs_min_freq);
#endif /* CONFIG_MALI_MIDGARD_DVFS_FREQ_ADJUSTABLE */
...
static struct attribute *kbase_attrs[] = {
	...
	&dev_attr_dvfs_enable_boost.attr,
#if CONFIG_MALI_MIDGARD_DVFS_FREQ_ADJUSTABLE
	&dev_attr_dvfs_max_freq.attr,
	&dev_attr_dvfs_min_freq.attr,
#endif /* CONFIG_MALI_MIDGARD_DVFS_FREQ_ADJUSTABLE */
#endif
