...
/**
 * struct kbase_uk_mem_umm_import - User/Kernel space data exchange structure
 * @header:         UK structure header
 * @base:           the address of the memory to import
 * @size:           the size of the memory to import
 * @is_bus_address: is the address a bus address
 * @fd:             file descriptor of the imported memory
 *
 * This structure is used when performing a call to import memory.
 */

#if MSTAR_UMM_IMPORT
struct kbase_uk_mem_umm_import {
	union uk_header header;
	/* IN */
	u32 base;
	u32 size;
...
#if MSTAR_UMM_IMPORT
	KBASE_FUNC_MEM_UMM_IMPORT = (UK_FUNC_ID + 37),
#endif
