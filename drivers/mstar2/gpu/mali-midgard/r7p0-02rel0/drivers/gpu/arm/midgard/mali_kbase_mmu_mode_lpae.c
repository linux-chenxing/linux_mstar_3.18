...
{
	/* MIU0/MIU1 */
	if (bus_addr < 0xE0000000ULL)
	{
		return (bus_addr - 0x20000000ULL);
	}

	/* MIU_TLB0 */
	if ((bus_addr & 0x10000000ULL) == 0)
	{
		return (bus_addr - 0xA0000000ULL);
	}
...
{
	/* MIU0 */
	if (miu_addr < 0x40000000ULL)
	{
		return (miu_addr + 0x20000000ULL);
	}

	/* MIU_TLB0 */
	if (miu_addr < 0x80000000ULL)
	{
		return (miu_addr + 0xA0000000ULL);
	}

	/* MIU1 */
	if (miu_addr < 0xC0000000ULL)
	{
		return (miu_addr + 0x20000000ULL);
	}
...
#ifdef MSTAR
	setup->transtab = (u64)BUS_TO_MIU_ADDRESS(kctx->pgd) &
#else
