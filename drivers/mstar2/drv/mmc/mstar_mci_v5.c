...
#if defined(CONFIG_PROFILE_EMMC_IO_LENGTH)
#define eMMC_WR_length_COUNT    2048
static U32 gu32_read_blk_cnt[eMMC_WR_length_COUNT+1];
static U32 gu32_write_blk_cnt[eMMC_WR_length_COUNT+1];
static U32 gu32_read_exceed2k_cnt =0;
static U32 gu32_write_exceed2k_cnt =0;
#endif
...
static void mstar_mci_send_data(struct work_struct *work)
{
    ...
    else
    {
        ...
        if(pData_st->flags & MMC_DATA_WRITE)
        {
            if(pData_st->blocks <= eMMC_WR_length_COUNT)
               gu32_write_blk_cnt[pData_st->blocks]++;
            else
               gu32_write_exceed2k_cnt++;
        }
        else if(pData_st->flags & MMC_DATA_READ)
        {
            if(pData_st->blocks <= eMMC_WR_length_COUNT)
               gu32_read_blk_cnt[pData_st->blocks]++;
            else
               gu32_read_exceed2k_cnt++;
        }
        #endif
...
#if defined(CONFIG_PROFILE_EMMC_IO_LENGTH)
static ssize_t eMMC_IO_length_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
     U32 u32_i;

     printk("\n");
     printk("R Blk Cnt |   IO Cnt\n");
     for(u32_i=1;u32_i<=eMMC_WR_length_COUNT;u32_i++)
     {
         if((u32_i %4)==1)
         {
             printk("     %4u", u32_i);
         ...
         if(u32_i < eMMC_WR_length_COUNT)
             printk("%8u, ", gu32_read_blk_cnt[u32_i]);
         else
             printk("%8u", gu32_read_blk_cnt[u32_i]);
         if((u32_i %4)==0)
            printk("\n");
     }
     printk("\n read blks exceed 2k cnt: ");
     printk("%8Xh\n",gu32_read_exceed2k_cnt);

     printk("W Blk Cnt |   IO Cnt\n");
     for(u32_i=1;u32_i<=eMMC_WR_length_COUNT;u32_i++)
     {
         if((u32_i %4)==1)
         {
         ...
         if(u32_i < eMMC_WR_length_COUNT)
             printk("%8u, ", gu32_write_blk_cnt[u32_i]);
         else
             printk("%8u", gu32_write_blk_cnt[u32_i]);
         if((u32_i %4)==0)
            printk("\n");
     }
     printk("\n write blks exceed 2k blks cnt: ");
     printk("%8u\n",gu32_write_exceed2k_cnt);

     return 0;
}

DEVICE_ATTR(eMMC_IO_length,
            S_IRUSR,
            eMMC_IO_length_show,
            NULL);
#endif
...
static s32 mstar_mci_probe(struct platform_device *pDev_st)
{
    ...
    #if defined(CONFIG_PROFILE_EMMC_IO_LENGTH)
    memset((U8*)gu32_read_blk_cnt, 0, sizeof(U32)*(eMMC_WR_length_COUNT+1));
    memset((U8*)gu32_write_blk_cnt, 0, sizeof(U32)*(eMMC_WR_length_COUNT+1));
    #endif
    LABEL_END:
