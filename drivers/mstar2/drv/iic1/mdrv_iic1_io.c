...
{
    .s32MajorIIC = MDRV_MAJOR_IIC1,
    .s32MinorIIC = MDRV_MINOR_IIC1,
    .cDevice =
    {
        .kobj = {.name= MOD_IIC1_NAME, },
        .owner = THIS_MODULE,
    },
    .IICFop =
    {
...
static ssize_t _MDrv_IIC_Read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	{
		u32RetCountIIC = MDrv_HWI2C1_ReadBytes(IIC_ReadParam.u8SlaveIdIIC,IIC_ReadParam.u8AddrSizeIIC, IIC_ReadParam.u8AddrIIC, IIC_ReadParam.u32DataSizeIIC,IIC_ReadParam.u8pbufIIC);
	}
	else
	{
...
static ssize_t _MDrv_IIC_Write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    if( IIC_WriteParam.u8IdIIC <= IIC_NUM_OF_HW )
	{
		for(j = 0; j < 10; j++)
		{
			...
			u32RetCountIIC = MDrv_HWI2C1_WriteBytes(IIC_WriteParam.u8SlaveIdIIC, IIC_WriteParam.u8AddrSizeIIC, IIC_WriteParam.u8AddrIIC, IIC_WriteParam.u32DataSizeIIC, IIC_WriteParam.u8pbufIIC);
			if(u32RetCountIIC == -1)
			{
				MDrv_HW_IIC1_Init();	
			}
...
#endif
{
    ...
    //PROBE_IO_ENTRY(MDRV_MAJOR_IIC1, _IOC_NR(cmd));
    ...
    switch(cmd)
    {
        case MDRV_IIC_INIT:
            ...
            {
                //PROBE_IO_EXIT(MDRV_MAJOR_IIC1, _IOC_NR(cmd));
 	            return -EFAULT;
            }
			MDrv_HW_IIC1_Init();
            break;
        case MDRV_IIC_CLOCK:
            ...
            if( IIC_param.u8IdIIC <= IIC_NUM_OF_HW )
			{
				...
				MDrv_HWI2C1_SetClk(IIC_param.u8ClockIIC);
			}
			else
			{
...
static int __init mod_iic_init(void)
{
    ...
    if (IICDev.s32MajorIIC)
    {
        ...
        s32Ret = register_chrdev_region(dev, MOD_IIC_DEVICE_COUNT, MOD_IIC1_NAME);
    }
    else
    {
        s32Ret = alloc_chrdev_region(&dev, IICDev.s32MinorIIC, MOD_IIC_DEVICE_COUNT, MOD_IIC1_NAME);
    ...
    cls = class_create(THIS_MODULE, MOD_IIC1_NAME);
    device = device_create(cls, NULL, dev, NULL, MOD_IIC1_NAME);
...
MODULE_AUTHOR("MSTAR");
MODULE_DESCRIPTION("IIC1 driver");
MODULE_LICENSE("GPL");
