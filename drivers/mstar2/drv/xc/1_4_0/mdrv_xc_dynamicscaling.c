...
MS_BOOL KApi_XC_SetDynamicScaling(EN_KDRV_SC_DEVICE u32DeviceID, K_XC_DynamicScaling_Info *pstDSInfo,MS_U32 u32DSInfoLen, EN_KDRV_WINDOW  eWindow)
{
...
#ifdef SUPPORT_KERNEL_DS
    printk("\033[1;32m[%s:%d]u32DeviceID=%d , u32DSInfoLen = %d, eWindow =%d\033[m\n",__FUNCTION__,__LINE__,u32DeviceID,u32DSInfoLen,eWindow);
    printk("\033[1;32m[%s:%d]%d,%d,%d,%d,%d,%d,%d\033[m\n",__FUNCTION__,__LINE__,
    ...
    printk("\033[1;32m[%s:%d]0x%llx,\033[m\n",__FUNCTION__,__LINE__,
            pstDSInfo->u64DS_Info_BaseAddr);

    if(u32DSInfoLen != sizeof(K_XC_DynamicScaling_Info))
    {
        printk("\033[1;32m[%s:%d] ############################ warning , size are differnet!! %d , %d\033[m\n",__FUNCTION__,__LINE__,
            u32DSInfoLen,sizeof(K_XC_DynamicScaling_Info));
#ifdef PATCH_FOR_DS_STRUCT_NOT_SYNC
        printk("\033[1;32m[%s:%d] Force entry setting now, need refine\033[m\n",__FUNCTION__,__LINE__);
#else
        return FALSE;
#endif
    }
...
#ifdef      SUPPORT_TCH
        KApi_DS_set_client(u32DeviceID, E_DS_CLIENT_HDR, 360 * (MS_MLOAD_BUS_WIDTH / MS_MLOAD_CMD_LEN_64BITS));
#else
