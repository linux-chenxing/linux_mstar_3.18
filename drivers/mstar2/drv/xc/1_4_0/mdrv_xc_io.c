...
int _MDrv_XC_IOC_CFDControl(struct file *filp, unsigned long arg)
{
    ...
    if (enCtrlType == E_KDRV_XC_CFD_CTRL_GET_HDMI_STATUS)
    {
        if (copy_to_user(((ST_KDRV_XC_CFD_CONTROL_INFO __user *)arg)->pParam, pu8Data, stKdrvCFDCtrlInfo.u32ParamLen))
        {
            //free buffer
            if(pu8Data)
                kfree(pu8Data);
    ...
    if (enCtrlType == E_KDRV_XC_CFD_CTRL_GET_STATUS || enCtrlType == E_KDRV_XC_CFD_CTRL_GET_OSD_PROCESS_CONFIGS)
    {
