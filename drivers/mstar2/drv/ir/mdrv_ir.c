...
#include CONFIG_ENABLE_CUSTOMER_IR_PATH_STRING

#ifndef IR_TYPE_SEL
// If CONFIG_ENABLE_CUSTOMER_IR_PATH=y, define IR_TYPE_SEL in IR header files.
// Please re-check your IR header file.
#error
#endif
...
#elif (defined(IR_TYPE_HWRC5_EX) && IR_TYPE_SEL==IR_TYPE_HWRC5_EX)
#include "IR_MSTAR_HWRC5.h"
...
//#define MULTI_PROTOCOL_DUMP_SHOTVALUE //open to get shotvalue list to analysis during development of new protocol
#ifdef IR_MODE_SEL
#undef IR_MODE_SEL
...
#define REPEAT_PRESS_SPEED_TIME 300
#define REPEAT_PRESS_SPEED_COUNT 2
#define REPEAT_FOLLOW_SPEED_TIME 50
static unsigned long  _ulPrevKeyTime;
#endif
#endif
...
inline static void _MDrv_MultiProtocol_StartTimer(void)
{
    ...
    timer_pending(&_key_MultiProtocol_timer);
    mod_timer(&_key_MultiProtocol_timer, jiffies + msecs_to_jiffies((pstProtocol->u32Timeout/1000)));
}
...
static void _MDrv_MultiProtocol_Cb(unsigned long params)
{
    ...
    else
    {
        ...
        if(_eCurentProtocol==E_IR_PROTOCOL_RC6)
        {
...
static irqreturn_t  MDrv_IR_ISRParseKey(void)
{
...
#ifdef MULTI_PROTOCOL_DUMP_SHOTVALUE
    ...
    if(_MulProtCommon_GetShotDataSize()>80)
    {
...
SW_ISR_LAST:
    ...
    BOOL bHaveKey = FALSE;
    bHaveKey = _MDrv_IR_GetKey(&u8Key, &u8System, &u8RepeatFlag);
    if (bHaveKey) {
        if (down_trylock(&fantasy_protocol.sem) == 0) {
            if (fantasy_protocol.data == 0) {
#if defined(IR_TYPE_RCMM) && (IR_TYPE_SEL == IR_TYPE_RCMM)
...
#if 0
    static U8 _u8RepeatPresCount = 0;
#endif
#endif
    ...
    else
    {
        ...
        *pu8Flag = (REG(REG_IR_SHOT_CNT_H_FIFO_STATUS) & IR_RPT_FLAG)? 1 : 0;
        ...
        if ( (*pu8Flag == 1) && ( *pu8Key == _u8PrevKeyCode ))
        {
            ...
            {
                if( (i - _ulPrevKeyRepeatTime) > REPEAT_PRESS_SPEED_TIME)
                {
                    _ulPrevKeyRepeatTime = i;
                    _u8RepeatPresCount ++;
                    if(_u8RepeatPresCount >= REPEAT_PRESS_SPEED_COUNT)
                    {
                        _u8RepeatPresCount = 0;
            ...
            {
                if( (i - _ulPrevKeyRepeatTime) > REPEAT_FOLLOW_SPEED_TIME)
                {
                    _ulPrevKeyRepeatTime = i;
                    bRet = TRUE;
                }
...
static BOOL _MDrv_IR_GetKey(U8 *pu8Key, U8 *pu8System, U8 *pu8Flag)
{
    ...
    *pu8Key = u8KeyCmd;
#elif (IR_TYPE_SEL == IR_TYPE_HWRC5_EX)
    u8KeyCmd_EX = (REG(REG_IR_RC_KEY_COMMAND_ADD)&0x7f00UL)>>8;//RC5: {repeat,1'b0,command[13:8]} reg[15:8]
    *pu8Key = u8KeyCmd_EX;
#endif
