...
#ifdef CONFIG_MSTAR_CHIP
static int tz_set_log_level(uint32_t level)
{
	struct smc_param param = { 0 };
	printk("\033[0;32;31m [Ian] %s %d Set Log Level %d\033[m\n",__func__,__LINE__,level);
	param.a0 = 0xb2005858;
	param.a1 = level;
	tee_smc_call(&param);
...
static int tz_pm51ctl(uint32_t cmd)
{
	struct smc_param param = { 0 };
	printk("\033[0;32;31m [Walker] %s %d PM51CTL %d\033[m\n",__func__,__LINE__,cmd);
	param.a0 = 0xb200685C;
	param.a1 = cmd;
	tee_smc_call(&param);
	...
	while ((unsigned long)log_buff_read_point != log_buff_write_point) {
		if (isascii(*(log_buff_read_point)) && *(log_buff_read_point) != '\0') {
			if (log_count >= MAX_PRINT_SIZE) {
				pr_err("%s %d buf is over MAX_PRINT_SIZE \n",__func__,__LINE__);
				log_count = 0;
			} else {
				log_buf[log_count] = *(log_buff_read_point);
				log_count++;
				if (*(log_buff_read_point) == '\n') {
					if (log_count >= MAX_PRINT_SIZE-1)
						pr_err("%s %d buf is over MAX_PRINT_SIZE \n",__func__,__LINE__);
					else {
						log_buf[log_count] = '\0';
						pr_info("%s", log_buf);
