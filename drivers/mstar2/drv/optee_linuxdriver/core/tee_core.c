	...
	case TEE_SET_LOG_LEVEL:
		{
			...
			if (copy_from_user(&k_level, (void *)u_arg, sizeof(uint32_t))) {
				printk("\033[0;32;31m [Ian] %s %d Error copy_from_user\033[m\n",__func__,__LINE__);
				break;
			}
			tee = ctx->tee;
			tee->ops->set_log_level(k_level);
			break;
		}
#endif
	case TEE_PM51CTL_IOC:
		{
			...
			if (copy_from_user(&cmd, (void *)u_arg, sizeof(uint32_t))) {
				printk("\033[0;32;31m [Walker] %s %d Error copy_from_user\033[m\n",__func__,__LINE__);
				break;
			}
			tee = ctx->tee;
			ret = tee->ops->pm51ctl(cmd);
			break;
		}
	default:
		ret = -ENOSYS;
                                    ...
                                    size_t count, loff_t *ppos)
{
	...
	printk("\033[0;32;31m [Ian] %s %d set_log_level %d\033[m\n",__func__,__LINE__,loglevel);
	...
	if(tee->ops->set_log_level)
		tee->ops->set_log_level(loglevel);
