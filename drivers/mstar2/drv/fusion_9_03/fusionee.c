...
#ifdef CONFIG_COMPAT
    if (type == FMT_CALL )
       msg_size = sizeof(CompatFusionCallMessage);
    if (type == FMT_CALL3)
       msg_size = sizeof(CompatFusionCallMessage3);
    if (type == FMT_LEAVE)
       msg_size = sizeof(FusionID);
#endif
...
#ifdef CONFIG_COMPAT
           if (header->msg_type==FMT_CALL)
           {
              CompatFusionCallMessage cfcm;
              FusionCallMessage *fcm =  (FusionCallMessage *)msg_data;
              ...
              memcpy( header + 1, &cfcm, msg_size );
           }
           else if ( header->msg_type== FMT_CALL3)
           {
              CompatFusionCallMessage3 cfcm3;
              FusionCallMessage3 *fcm3 =  (FusionCallMessage3 *)msg_data;
              cfcm3.handler = (compat_uptr_t)fcm3->handler;
              cfcm3.ctx = (compat_uptr_t)fcm3->ctx;
              cfcm3.caller = fcm3->caller;
              cfcm3.call_arg = fcm3->call_arg;
              cfcm3.call_ptr = (compat_uptr_t)fcm3->call_ptr;
              cfcm3.call_length = fcm3->call_length;
              cfcm3.ret_length = fcm3->ret_length;
              cfcm3.serial = fcm3->serial;
              memcpy( header + 1, &cfcm3, msg_size );
           }
           else
 #endif
                      ...
                      Fusionee * fusionee, void *buf, int buf_size, bool block)
{
     ...
     while (fusionee->packets.count && ((Packet *) fusionee->packets.items)->flush) {
		...
		{
		 CompatFusionCallMessage cfcm;
		 FusionReadMessage  *header=(FusionReadMessage *)packet->buf;
         FusionCallMessage *fcm ; 
         if (header->msg_type==FMT_CALL) {
                FusionCallMessage *fcm = (FusionCallMessage *)(packet->buf+sizeof(FusionReadMessage));
                cfcm.handler = (compat_uptr_t)fcm->handler;
				cfcm.ctx = (compat_uptr_t)fcm->ctx;
				cfcm.caller = fcm->caller;
				cfcm.call_arg = fcm->call_arg;
				cfcm.call_ptr = (compat_uptr_t)fcm->call_ptr;
				cfcm.serial = fcm->serial;
                memcpy(packet->buf+sizeof(FusionReadMessage),&cfcm,sizeof(CompatFusionCallMessage));
         }
		}
		#endif
		#endif
