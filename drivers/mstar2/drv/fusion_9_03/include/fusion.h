...
#define FUSION_API_MICRO_PROVIDED 3
...
typedef struct {
     ...
     int                      call_arg;      /* optional int argument */
     compat_uptr_t            ptr;           /* optional argument, data will be appended (copy) to message and passed to callee */
     unsigned int             length;
...
typedef struct {
     ...
     compat_uptr_t            ret_ptr;       /* pointer to return buffer */
     unsigned int             ret_length;    /* maximum (input) and actual (output) length of return buffer */

     FusionCallExecFlags      flags;         /* execution flags */
     unsigned int             serial;        /* with FCEF_RESUMABLE used for EINTR handling, intialise with zero!!! */
} CompatFusionCallExecute3;
#endif
...
typedef struct {
     ...
     compat_uptr_t            ptr;           /* pointer to return buffer */
     unsigned int             length;        /* length of return buffer */
} CompatFusionCallReturn3;
#endif
...
typedef struct {
     ...
     CompatFusionID                 fusion_id;     /* [output] owner of the call */
} CompatFusionCallGetOwner;
#endif
...
typedef struct {
     int                      call_id;       /* [input] call from which to get the owner */
     CompatFusionID                 fusion_id;     /* [input] fusionee to set quota for */
     unsigned int             limit;         /* [input] max number of pending calls, 0 = always blocked */
} CompatFusionCallSetQuota;
#endif
...
typedef struct {
     ...
     CompatFusionID                 fusion_id;
     unsigned int             permissions;
} CompatFusionEntryPermissions;
#endif
...
#define COMPAT_FUSION_SYNC                          _IO (FT_LOUNGE,    0x06)

#define COMPAT_FUSION_ENTRY_ADD_PERMISSIONS         _IOW(FT_LOUNGE,    0x07, CompatFusionEntryPermissions)

#define COMPAT_FUSION_SHM_GET_BASE                  _IOR(FT_LOUNGE,    0x08, compat_ulong_t)

#define COMPAT_FUSION_GET_FUSIONEE_INFO             _IOR(FT_LOUNGE,    0x09, CompatFusionGetFusioneeInfo)
...
#define COMPAT_FUSION_CALL_EXECUTE2                 _IOW(FT_CALL,      0x04, CompatFusionCallExecute2)
#define COMPAT_FUSION_CALL_EXECUTE3                 _IOW(FT_CALL,      0x05, CompatFusionCallExecute3)
#define COMPAT_FUSION_CALL_RETURN3                  _IOW(FT_CALL,      0x06, CompatFusionCallReturn3)
#define COMPAT_FUSION_CALL_GET_OWNER                _IOW(FT_CALL,      0x07, CompatFusionCallGetOwner)
#define COMPAT_FUSION_CALL_SET_QUOTA                _IOW(FT_CALL,      0x08, CompatFusionCallSetQuota)
...
#define COMPAT_FUSION_REF_CATCH                     _IOW(FT_REF,       0x0C, int)
#define COMPAT_FUSION_REF_THROW                     _IOW(FT_REF,       0x0D, FusionRefThrow)
#define COMPAT_FUSION_REF_SET_SYNC                  _IOW(FT_REF,       0x0E, int)
...
#define COMPAT_FUSION_SHMPOOL_GET_BASE              _IOR(FT_SHMPOOL,   0x05, compat_ulong_t)
