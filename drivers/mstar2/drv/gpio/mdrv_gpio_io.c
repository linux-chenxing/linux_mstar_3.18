...
#if defined(CONFIG_COMPAT)
static long Compat_MDrv_GPIO_Ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
#endif
...
static GPIO_ModHandle_t GPIODev=
{
    ...
    .GPIOFop =
    {
        ...
        #endif 
		#if defined(CONFIG_COMPAT)
		.compat_ioctl = Compat_MDrv_GPIO_Ioctl,
		#endif
...
#if defined(CONFIG_COMPAT)
static long Compat_MDrv_GPIO_Ioctl(struct file *filp, unsigned int cmd, unsigned long arg){
	GPIO_PRINT("%s is invoker, cmd = %x, arg = %hhu\n", __FUNCTION__, cmd, arg);
...
static int mod_gpio_init(void)
{
    ...
    cls = class_create(THIS_MODULE, MOD_GPIO_NAME);
    device = device_create(cls, NULL, dev, NULL, MOD_GPIO_NAME);
...
static int mstar_gpio_drv_probe(struct platform_device *pdev)
{
    ...
    if( !(pdev->name) || strcmp(pdev->name,"gpio")
        || pdev->id!=0)
    {
        retval = -ENXIO;
    }
...
static struct of_device_id mstargpio_of_device_ids[] = {
