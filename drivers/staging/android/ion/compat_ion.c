...
#endif
struct compat_ion_cust_import_data {    
    compat_pid_t pid;
    compat_int_t fd;  
    compat_int_t newfd;
};
...
#define COMPAT_ION_IOC_CACHE_FLUSH		_IOW(ION_IOC_MAGIC, 10, \
				      struct compat_ion_cache_flush_data)
#endif
#define COMPAT_ION_IOC_CUST_IMPORT		_IOWR(ION_IOC_MAGIC, 11, \
				      struct compat_ion_cust_import_data)
...
static int compat_get_ion_cust_import_data(
    struct compat_ion_cust_import_data __user *data32,
			struct ion_cust_import_data __user *data)
{
	compat_pid_t p;
	compat_int_t i;
	int err;

	err = get_user(p,&data32->pid);
	err |= put_user(p,&data->pid);	
	err |= get_user(i,&data32->fd);
	err |= put_user(i,&data->fd);
	return err;
}

static int compat_put_ion_cust_import_data(
			struct compat_ion_cust_import_data __user *data32,
			struct ion_cust_import_data __user *data)
{
	compat_int_t i;
	...
	err = get_user(i, &data->newfd);
	err |= put_user(i, &data32->newfd);
	return err;
}
...
#endif
	case COMPAT_ION_IOC_CUST_IMPORT:
	{		
		struct compat_ion_cust_import_data __user *data32;
		struct ion_cust_import_data __user *data;
		int err;
		...
		err = compat_get_ion_cust_import_data(data32, data);
		if (err)
			return err;
		ret = filp->f_op->unlocked_ioctl(filp, ION_IOC_CUST_IMPORT,
							(unsigned long)data);
		err = compat_put_ion_cust_import_data(data32, data);
		return ret ? ret : err;
	}
