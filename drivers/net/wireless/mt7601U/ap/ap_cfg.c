	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
    ...
    // not accept ',' separated command
    this_char = pIoctlCmdStr->u.data.pointer;
   ...
   INT IEEE8021X = 0;
	...
	case OID_802_11_AUTHENTICATION_MODE:
        ...
        case RT_OID_802_11_RSSI:
			ulInfo = pAd->ApCfg.RssiSample.LastRssi0;
	...
	case RT_OID_802_11_PER_BSS_STATISTICS:
		{
			...
			copy = copy_to_user(wrq->u.data.pointer, pMbssStat, wrq->u.data.length);
		...
		case OID_APCLI_CONNECT_STATUS:
			{
				...
				DBGPRINT(RT_DEBUG_ERROR, ("==>OID_APCLI_CONNECT_STATUS\n"));
    ...
    IN  PSTRING          arg)
{
	...
	if (strcmp(arg, "Monitor") == 0)
	{
		...
		DBGPRINT(RT_DEBUG_OFF, ("%s():BW_%s, CtrlChannel(%d), CentralChannel(%d) \n", 
		...
		csr.field.TsfSyncMode = 3;
		RTMP_IO_WRITE32(pAd, BCN_TIME_CFG, csr.word);
		OPSTATUS_CLEAR_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED);

		pAd->ApCfg.BssType = BSS_INFRA;
		RTMP_OS_NETDEV_SET_TYPE(pAd->net_dev, ARPHRD_ETHER);
		DBGPRINT(RT_DEBUG_TRACE, ("===>Set_NetworkType_Proc::(APmode)\n"));
	}
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	if (bIsPrintAllRF)
	{
		...
		RtmpDrvAllRFPrint(NULL, (IN UINT32 *)msg, strlen(msg));
		/* Copy the information into the user buffer */
...
/*	msg = (PSTRING)kmalloc(sizeof(CHAR)*(2048), MEM_ALLOC_FLAG); */
	...
	wps_state = pAd->ApCfg.ApCliTab[0].WscControl.WscState;
	wps_status = pAd->ApCfg.ApCliTab[0].WscControl.WscStatus;
	...
	if ((wps_state == WSC_STATE_OFF) || (wps_status & 0xff00))
	{
		if (wps_status == STATUS_WSC_CONFIGURED)
		{
			...
			sprintf(msg+strlen(msg), "WPS_STATUS=WPS_SUCCESS\n");
		}
		else if ((wps_status == STATUS_WSC_NOTUSED))
		{
			//sprintf(extra+strlen(extra), "WPS not used.\n");
			sprintf(msg+strlen(msg), "WPS_STATUS=WPS_NOT_USED\n ");
		}
	...
	{
		//sprintf(msg+strlen(msg), "%s WPS Proceed. Please wait... \n");
		sprintf(msg+strlen(msg), "WPS Proceed. Please wait... \n");
	}
...
{	
	...
	/*&& (pAd->ed_false_cca_cnt > pAd->ed_learning_time_threshold)
	 && pAd->ed_vga_at_lowest_gain */  //falseCCA is low
	)
	{
