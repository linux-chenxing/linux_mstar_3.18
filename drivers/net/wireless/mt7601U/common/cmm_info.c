	...
	IN PSTRING arg)
{
    ...
    DBGPRINT_S(RT_DEBUG_TRACE, ("==>%s() Set RTDebugFunc = 0x%lu\n", __FUNCTION__, RTDebugFunc));
	...
	IN  UINT32		MsgLen)
{
	...
	CHAR        SsidDoubleQuotes[MAX_LEN_OF_SSID +5];
	STRING		SecurityStr[32] = {0};
	NDIS_802_11_ENCRYPTION_STATUS	ap_cipher = Ndis802_11EncryptionDisabled;
	NDIS_802_11_AUTHENTICATION_MODE	ap_auth_mode = Ndis802_11AuthModeOpen;

		/*Channel*/
		sprintf(msg+strlen(msg),"%-8d", pBss->Channel);
		...
		if (Rssi >= -50)
			...
			Rssi_Quality = 0;
		sprintf(msg+strlen(msg),"%-8d", Rssi);
	...
	else
	{
		...
		sprintf(SsidDoubleQuotes, "\"%s\"", Ssid);
		sprintf(msg+strlen(msg),"%-36s", SsidDoubleQuotes);
	...
	// Encryption Type for Alphanetworks Site Survey Result
	if(strcmp(GetEncryptType((CHAR)ap_cipher), "TKIPAES") == 0)
	{
		sprintf(msg+strlen(msg), "%-12s", "AES");
	}
	else
	{
		sprintf(msg+strlen(msg), "%-12s", GetEncryptType((CHAR)ap_cipher));
	}
					        
	// Authentication Mode for Alphanetworks Site Survey Result
	if(strcmp(GetAuthMode((CHAR)ap_auth_mode), "WPAPSK") == 0)
	{
		sprintf(msg+strlen(msg), "%-20s", "WPA-PSK");
	}
	else if(strcmp(GetAuthMode((CHAR)ap_auth_mode), "WPA2PSK") == 0)
	{
		sprintf(msg+strlen(msg), "%-20s", "WPA2-PSK");
	}
	else if(strcmp(GetAuthMode((CHAR)ap_auth_mode), "WPA1PSKWPA2PSK") == 0)
	{
		sprintf(msg+strlen(msg), "%-20s", "WPA2-PSK");
	}
	else
	{
		sprintf(msg+strlen(msg),"%-20s", GetAuthMode((CHAR)ap_auth_mode));
	}
		...
		*/
		...
		if (pBss->BssType == BSS_ADHOC)
			sprintf(msg+strlen(msg),"%-3s", "Adhoc");
		else
			sprintf(msg+strlen(msg),"%-3s", "Infra");
