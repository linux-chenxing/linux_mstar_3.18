...
#include "rt_config.h"
#include <stdarg.h>
extern void MT7601_dynamic_vga_reset_default(IN PRTMP_ADAPTER pAd);
extern void MT7601_dynamic_vga_tuning(IN PRTMP_ADAPTER pAd);
	...
	IN PVOID SystemSpecific3) 
{
	...
	/* batch update CPU index to avoid RX coherent */

	RTMP_SEM_LOCK(&pAd->RxRingLock);
	if (IS_MT7601(pAd))
	{
		RTMP_RX_RING *pRxRing = &pAd->RxRing;
		int ring_size = RX_RING_SIZE;

		if ((pRxRing->RxSwReadIdx > pRxRing->RxCpuIdx) && ((pRxRing->RxSwReadIdx - pRxRing->RxCpuIdx) > 1))
	    {
	        //printk("%s: RxSwReadIdx %d, RxCpuIdx %d\n", __FUNCTION__, pRxRing->RxSwReadIdx, pRxRing->RxCpuIdx);
	        pRxRing->RxCpuIdx = (pRxRing->RxSwReadIdx == 0) ? (ring_size-1) : (pRxRing->RxSwReadIdx-1);
	    }
		else if ((pRxRing->RxSwReadIdx < pRxRing->RxCpuIdx) && (pRxRing->RxSwReadIdx - pRxRing->RxCpuIdx + ring_size) > 1)
		{
			//printk("%s: RxSwReadIdx %d, RxCpuIdx %d\n", __FUNCTION__, pRxRing->RxSwReadIdx, pRxRing->RxCpuIdx);
			pRxRing->RxCpuIdx = (pRxRing->RxSwReadIdx == 0) ? (ring_size-1) : (pRxRing->RxSwReadIdx-1);
		}
