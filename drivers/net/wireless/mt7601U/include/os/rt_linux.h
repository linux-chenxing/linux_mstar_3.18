...
#else
#define AP_PROFILE_PATH			"/ext/etc/Wireless/RT2870AP/RT2870AP.dat"
#define AP_RTMP_FIRMWARE_FILE_NAME "/ext/etc/Wireless/RT2870AP/RT2870AP.bin"
#endif
 ...
 ***********************************************************************************/
#define MEM_ALLOC_FLAG      (GFP_KERNEL) /*(GFP_DMA | GFP_ATOMIC) */
...
#define OS_PKT_COPY(_pkt)		skb_copy(RTPKT_TO_OSPKT(_pkt), GFP_KERNEL)
...
#define RTUSB_ALLOC_URB(iso)		usb_alloc_urb(iso, GFP_KERNEL)
...
#endif
#else
#define RTUSB_URB_ALLOC_BUFFER(_dev, _size, _dma)	kmalloc(_size, GFP_KERNEL)
...
#define RTUSB_URB_ALLOC_BUFFER(pUsb_Dev, BufSize, pDma_addr)         		kmalloc(BufSize, GFP_KERNEL)
