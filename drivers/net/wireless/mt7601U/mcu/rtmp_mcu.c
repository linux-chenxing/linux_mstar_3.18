...
{
	return RTUSBMultiWrite_nBytes(pAd, Offset, (UCHAR *)Data, Cnt * 4, 64); 
}
...
VOID ChipOpsMCUHook(PRTMP_ADAPTER pAd, enum MCU_TYPE MCUType)
{
	...
	if (MCUType == ANDES) 
	{
		...
		pChipOps->RandomWrite = (INT (*)(struct _RTMP_ADAPTER *, RTMP_REG_PAIR *, UINT32 ))AndesRandomWrite;
		pChipOps->RFRandomWrite = (INT (*)(struct _RTMP_ADAPTER *, BANK_RF_REG_PAIR *, UINT32 ))AndesRFRandomWrite;
