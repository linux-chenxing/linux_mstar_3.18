...
//	CFG80211_CB *p80211CB = NULL;
	...
	MSC_DBG("[MSC] set monitor channel: chl, chan_id:%d, width:%d, chan_flags:0x%x\n",
...
//	RTMP_DRIVER_80211_CHAN_SET(pAd, &ChanInfo);
	...
	CFG80211DRV_OpsSetChannel(pAd, &ChanInfo);
...
int sc_plt_leave_monitor_mode(void *priv)
{
	...
	/* set channel */
	ChanInfo.ChanId = 1;
	ChanInfo.IfType = RT_CMD_80211_IFTYPE_AP;
	ChanInfo.ChanType = RT_CMD_80211_CHANTYPE_HT20;
	ChanInfo.MonFilterFlag= APNORMAL;
