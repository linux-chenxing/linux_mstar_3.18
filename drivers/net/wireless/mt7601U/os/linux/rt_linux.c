	...
	IN PNDIS_PACKET pPacket)
{
	...
	if (skb_tailroom(skb) < TKIP_TX_MIC_SIZE) {
		...
		newskb = skb_copy_expand(skb, skb_headroom(skb), TKIP_TX_MIC_SIZE, GFP_KERNEL);
	...
	IN UINT32 ext_tail_len)
{
	...
	if (skb_cloned(skb) || (skb_headroom(skb) < ext_head_len)
		...
		/* alloc a new skb and copy the packet */
		newskb = skb_copy_expand(skb, head_len, tail_len, GFP_KERNEL);
...
NDIS_STATUS AdapterBlockAllocateMemory(VOID *handle, VOID **ppAd, UINT32 SizeOfpAd)
{
	...
	/* get offset for sk_buff */
	{
		...
		pPkt = kmalloc(sizeof (struct sk_buff), GFP_KERNEL);
		if (pPkt == NULL) {
			*ppAd = NULL;
			return NDIS_STATUS_FAILURE;
		}
	...
	IN UINT32 BufLen)
{
	...
	if (IS_ERR(file_w)) {
		...
		if (file_w->f_op && file_w->f_op->write) {
			...
			file_w->f_op->write(file_w, (const char __user *)pBuf, BufLen, &file_w->f_pos);
		}
		filp_close(file_w, NULL);
	}
	set_fs(orig_fs);
...
{
	return skb_copy(RTPKT_TO_OSPKT(pNetPkt), GFP_KERNEL);
}
