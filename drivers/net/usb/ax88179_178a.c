...
#include "chip_int.h"
static const struct cpumask *ori_cpumask;
#endif
...
#endif
#if defined(CONFIG_MP_PLATFORM_GIC_SET_MULTIPLE_CPUS) && defined(CONFIG_MP_PLATFORM_INT_1_to_1_SPI)
	ori_cpumask = get_cpu_mask(get_cpu()); put_cpu();
	//FIXME: It should get hcd's IRQ number here, but so far, it can't. This is workaround.
	irq_set_affinity_hint(E_IRQHYPH_USB30_SS, cpu_online_mask);
	irq_set_affinity(E_IRQHYPH_USB30_SS, cpu_online_mask);
#endif
	return 0;
...
#if defined(CONFIG_MP_PLATFORM_GIC_SET_MULTIPLE_CPUS) && defined(CONFIG_MP_PLATFORM_INT_1_to_1_SPI)
	//FIXME: It should get hcd's IRQ number here, but so far, it can't. This is workaround.
	irq_set_affinity(E_IRQHYPH_USB30_SS, ori_cpumask);
#endif
}
